BGOpen("sc000",1);
ChLayout(1);
ChEye(22,0);
ChMouth(22,0);
ChMotion(22,0);
ScrFadeIn(0);
ChMotion(22,3,1);
VoicePlay("F022200003_22_000");
MsgDisp("Hikaru","Hey, Mari, did you know?
About that legendary rumour at the church!");
MsgDisp("主人公","Which one?");
ChEye(22,0);
ChMouth(22,3);
ChMotion(22,0,1);
VoicePlay("F022200003_22_010");
MsgDisp("Hikaru","That couples who confess and marry at the
church, will give birth to twins!");
MsgDisp("主人公","Ehh, really∋");
ChEye(22,3);
ChMouth(22,3);
ChMotion(22,4,1);
ChEyeOpenLevel(22,0);
VoicePlay("F022200003_22_020");
MsgDisp("Hikaru","Our Mama said it.
It must be true♪");
MsgDisp("主人公","Wow...
Did both of your parents study at Haba High,
｛ひかる＊｝?");
ChEye(22,3);
ChMouth(22,3);
ChEyeOpenLevel(22,0);
ChMouthOpenLevel(22,0);
VoicePlay("F022200003_22_030");
MsgDisp("Hikaru","......");
ChEye(22,3);
ChMouth(22,2);
ChEyeOpenLevel(22,10);
VoicePlay("F022200003_22_040");
MsgDisp("Hikaru","...Huh?");
ChEye(22,4);
ChMouth(22,2);
ChMotion(22,2,1);
VoicePlay("F022200003_22_050");
MsgDisp("Hikaru","That's not right～ Papa definitely studied
abroad in Paris, and Hikaru feels like Mama
said she went to an all-girls high school.");
ChEye(22,2);
ChMouth(22,1);
ChMotion(22,5,1);
VoicePlay("F022200003_22_060");
MsgDisp("Hikaru","...Eh, wait a second...
That means Haba High used to be an all girls
school a long time ago～∋");
MsgDisp("主人公","(｛ひかる＊｝ is confused.
I think ｛ひかる＊｝'s Mother was
probably just joking...)");
MsgClose();
ScrFadeOut(0,0);
