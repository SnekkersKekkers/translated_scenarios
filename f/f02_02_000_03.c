BGMStop();
BGOpen("sc000",1);
ChEye(2,0);
ChMouth(2,3);
ChMotion(2,3,1);
ChLayout(1);
ScrFadeIn(0);
VoicePlay("F020200003_02_000");
MsgDisp("Sassa","I heard another story about the church.");
MsgDisp("主人公","Hehe, the one without the princess?");
ChEye(2,0);
ChMouth(2,3);
ChMotion(2,4);
VoicePlay("F020200003_02_010");
MsgDisp("Sassa","Yeah, about Mikage-sensei.
There's a rumor he's gathering ostriches and
breeding them.");
MsgDisp("主人公","Huh?
He's keeping ostriches?");
ChEye(2,0);
ChMouth(2,3);
ChMotion(2,0);
VoicePlay("F020200003_02_020");
MsgDisp("Sassa","He's planning an ostrich race scam.");
MsgDisp("主人公","Ostrich race?");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0);
VoicePlay("F020200003_02_030");
MsgDisp("Sassa","Yeah, so he's gathering ostriches of superior
breeding.");
ChEye(2,1);
ChMouth(2,3);
ChMotion(2,4);
VoicePlay("F020200003_02_040");
MsgDisp("Sassa","But...if the ostriches are too slow, I heard
Mikage-sensei eats them.");
MsgDisp("主人公","Huh∋");
ChEye(2,1);
ChMouth(2,3);
ChMotion(2,2);
VoicePlay("F020200003_02_050");
MsgDisp("Sassa","So every night, there's an ostrich who
breathes its last breath to the sound of
Mikage-sensei smacking his lips.");
ChEye(2,0);
ChMouth(2,3);
ChMotion(2,4);
VoicePlay("F020200003_02_060");
MsgDisp("Sassa","Haha, well, they're just rumors.
I feel bad for Mikage-sensei, you know?
But they're interesting.");
MsgDisp("主人公","(...It went like that.
We left school happily talking.)");
