BGOpen("sc230",0);
ChLayout(1);
ScrFadeIn(0);
VoicePlay("F100430001_22_000");
MsgDisp("Hikaru?","Eh～∋
Minoru-kun, you're quitting?");
VoicePlay("F100430001_04_000");
MsgDisp("Nanatsumori?","No...
I haven't decided yet.");
MsgDisp("主人公","(Hm?
This voice...)");
VoicePlay("F100430001_22_010");
MsgDisp("Hikaru?","Then what are you going to do with that
wig and outfit?
Are you going to throw it all away?");
VoicePlay("F100430001_04_010");
MsgDisp("Nanatsumori?","Hikaru-san, if there's anything you like
I'll give it to you——");
MsgClose();
BGMPlay("BGM_HANYOU_A",0.01);
ChNanaType(0);
ChOpen(4,254,0,0,0,#1,#1,0,1);
ChOpen(22,254,0,0,0,#1,#1,0,2);
SEPlay("EV_SE_FOOT_WALK_COME_ALONE_SHORT",0.3,0.5);
SEPlay("EV_SE_544",0);
ChEye(4,5);
ChMouth(4,5);
ChMotion(4,5,1);
ChEye(22,3);
ChMouth(22,5);
ChMotion(22,5,1);
ChEyeOpenLevel(22,10);
MsgDisp("主人公","Hey, what are you two talking about?");
ChEye(22,0);
ChMouth(22,0);
ChEye(4,2);
ChMouth(4,1);
ChMotion(4,1,1);
VoicePlay("F100430001_04_020");
MsgDisp("Nanatsumori","No...");
ChEye(22,0);
ChMouth(22,0);
ChMotion(22,4,1);
VoicePlay("F100430001_22_020");
MsgDisp("Hikaru","Hey, Mari would be lonely too right?
Nanako——");
ChEye(4,5);
ChMouth(4,1);
ChMotion(4,4,1);
VoicePlay("F100430001_04_030");
MsgDisp("Nanatsumori","Shh, shhhhh∈");
ChEye(22,3);
ChMouth(22,5);
ChMotion(22,5,1);
ChEyeOpenLevel(22,10);
VoicePlay("F100430001_22_030");
MsgDisp("Hikaru","Eh...no way?
Mari doesn't know∋");
ChEye(4,1);
ChMouth(4,1);
ChMotion(4,2,1);
ChMouthOpenLevel(4,0);
VoicePlay("F100430001_04_040");
MsgDisp("Nanatsumori","......");
ChEye(22,4);
ChMouth(22,2);
ChMotion(22,2,1);
VoicePlay("F100430001_22_040");
MsgDisp("Hikaru","Oh no...
I thought she found out long ago and were
having this kind of girl talk.");
MsgDisp("主人公","(Girl talk with ｛七ツ森＊｝?)");
ChEye(4,0);
ChMouth(4,2);
ChMotion(4,0,1);
VoicePlay("F100430001_04_050");
MsgDisp("Nanatsumori","Hikaru-san, it's time to tone it down...");
ChEye(22,4);
ChMouth(22,2);
ChMotion(22,1,1);
VoicePlay("F100430001_22_050");
MsgDisp("Hikaru","How bothersomeー...");
ChEye(22,3);
ChMouth(22,3);
ChMotion(22,3,1);
ChEyeOpenLevel(22,0);
VoicePlay("F100430001_22_060");
MsgDisp("Hikaru","I mean, Mari. you're too oblivious～!");
ChEye(4,2);
ChMouth(4,4);
ChMotion(4,2,1);
VoicePlay("F100430001_04_060");
MsgDisp("Nanatsumori","Thanks to that, I was saved...");
MsgDisp("主人公","(What do you mean I'm oblivious...)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
