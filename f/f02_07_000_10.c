BGMStop();
BGOpen("sc000",1);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0,1);
ChLayout(1);
ScrFadeIn(0);
MsgDisp("主人公","It's the cultural festival soon.
I wonder if I'll be alright
for the school play...");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,5);
VoicePlay("F020700010_07_000");
MsgDisp("Mikage","What, you're worried about that?
It'll definitely go well. I guarantee it.");
MsgDisp("主人公","That's a lot of faith 
you have in me.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,4);
VoicePlay("F020700010_07_010");
MsgDisp("Mikage","I've been with you guys practicing 
and preparing from the very beginning,
so I'm telling you.");
MsgDisp("主人公","Yes. From stage preparations 
to the script and acting, you supported
us with everything,｛御影＊＊｝.");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,3);
VoicePlay("F020700010_07_020");
MsgDisp("Mikage","Right?
So if I say it's okay, it's okay.
Don't worry, just do what you can.");
MsgDisp("主人公","Yes...
But if I fail, I'll cause
trouble for everyone..");
ChEye(7,4);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("F020700010_07_030");
MsgDisp("Mikage","You've been working hard for dear life 
with  your fellow classmates, right?
So no matter what happens during the real
thing, it will become the best memory.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("F020700010_07_040");
MsgDisp("Mikage","So there's no longer a route for failure.
You are guaranteed to receive
one of life's treaures.");
MsgDisp("主人公","Is that so...");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,2);
VoicePlay("F020700010_07_050");
MsgDisp("Mikage","That's right.
Now it's up to you to decide
what shape of success you're going for.");
MsgDisp("主人公","(I want to get the best possible result,
not only for everyone, but also
for ｛御影＊＊｝ who had helped
with all of the preparations.)");
