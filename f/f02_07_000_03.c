BGMStop();
BGOpen("sc000",1);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0,1);
ChLayout(1);
ScrFadeIn(0);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("F020700003_07_000");
MsgDisp("Mikage","Hey, there seems to be more and more tales
surrounding that church legend, right?");
MsgDisp("主人公","What kind of tales?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,5);
VoicePlay("F020700003_07_010");
MsgDisp("Mikage","That, well.
That I'm raising ostriches over there.");
MsgDisp("主人公","Ehhh∋ Is that true?");
ChEye(7,1);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("F020700003_07_020");
MsgDisp("Mikage","I am not raising ostriches there.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,3);
VoicePlay("F020700003_07_030");
MsgDisp("Mikage","What's the deal with it being an ostrich?
If it was a cow, horse or sheep I could kind
of get it.");
MsgDisp("主人公","Do you have any clue as to why?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("F020700003_07_040");
MsgDisp("Mikage","I like ostrich meat.
I'm curious about it, and I've even eaten it
at home.");
MsgDisp("主人公","Eh? Is it tasty?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("F020700003_07_050");
MsgDisp("Mikage","Ostrich meat is all lean, low in calories,
and high in protein, making it an extremely
healthy ingredient, ya know?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,5);
VoicePlay("F020700003_07_060");
MsgDisp("Mikage","I'll say stuff like that when I'm asked.");
MsgDisp("主人公","Then, that rumour was definitely made by
someone who heard ｛Mikage＊＊｝.");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("F020700003_07_070");
MsgDisp("Mikage","That's not all.
Apparently the ostriches started breeding and
escaping.");
MsgDisp("主人公","T-That's not true, right?");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("F020700003_07_080");
MsgDisp("Mikage","Yeah. But if that kind of talk comes up in
the staff room, it'll reach Vice Principal
Himuro's ears, right? It's scary.");
MsgDisp("主人公","Ehh? But, it's a lie right?");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,3);
VoicePlay("F020700003_07_090");
MsgDisp("Mikage","Sigh...there's a grain of truth even in
rumours.
How troubling.");
MsgDisp("主人公","(I wonder if ｛Mikage＊＊｝ is up to
something in the church...)");
