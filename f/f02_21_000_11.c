BGOpen("sc000",1);
ChLayout(1);
MsgClose();
ChEye(21,0);
ChMouth(21,0);
ChMotion(21,7);
ScrFadeIn(0);
MsgDisp("主人公","It's almost winter break soon.");
ChEye(21,0);
ChMouth(21,4);
ChMotion(21,2);
VoicePlay("F022100011_21_000");
MsgDisp("Michiru","This year is ending huh...
It went by in the blink of an eye.");
MsgDisp("主人公","That's right...");
ChEye(21,0);
ChMouth(21,0);
ChMotion(21,0);
VoicePlay("F022100011_21_010");
MsgDisp("Michiru","So? Did you have 
a fulfilling year Mari?");
ChEye(21,3);
ChMouth(21,3);
ChMotion(21,3);
VoicePlay("F022100011_21_020");
MsgDisp("Michiru","I had such a fulfilling year that
I'm scared of what next year will bring.");
MsgDisp("主人公","Hehe!");
ChEye(21,4);
ChMouth(21,3);
ChMotion(21,4);
VoicePlay("F022100011_21_030");
MsgDisp("Michiru","Mari. Thank you for being my friend this
year too.");
MsgDisp("主人公","（｛みちる＊｝……）");
MsgClose();
ScrFadeOut(0,0);
