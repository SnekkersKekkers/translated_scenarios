MsgDisp("主人公","｛みちる＊｝, 
what is your dream for the future?");
ChEye(21,0);
ChMouth(21,4);
ChMotion(21,2);
VoicePlay("F032100000_21_330");
MsgDisp("Michiru","My dream for the future....");
MsgDisp("主人公","I mean, maybe a top model 
or a fashion designer or something?");
ChEye(21,0);
ChMouth(21,0);
ChMotion(21,0);
VoicePlay("F032100000_21_340");
MsgDisp("Michiru","I wonder about that? ");
MsgDisp("主人公","？");
ChEye(21,4);
ChMouth(21,4);
ChMotion(21,4);
VoicePlay("F032100000_21_350");
MsgDisp("Michiru","I thought that until middle school.
But... it's a little different now.");
ChEye(21,0);
ChMouth(21,0);
ChMotion(21,0);
VoicePlay("F032100000_21_360");
MsgDisp("Michiru","Hey. Mari.
Thanks to you, I can see reality now.");
MsgDisp("主人公","Thanks to me?");
ChEye(21,0);
ChMouth(21,0);
ChMotion(21,3);
ChEyeOpenLevel(21,0);
VoicePlay("F032100000_21_370");
MsgDisp("Michiru","Yes.
Until now, I've gone with the flow,
living freely without knowing any
hardship.");
ChEye(21,0);
ChMouth(21,3);
ChMotion(21,0);
VoicePlay("F032100000_21_380");
MsgDisp("Michiru","But, ever since I became friends
with Mari, there's been a link between 
this dream-like world and reality. 
Everyday is super exciting.");
ChEye(21,4);
ChMouth(21,3);
ChMotion(21,4);
VoicePlay("F032100000_21_390");
MsgDisp("Michiru","That's why, I've started thinking
seriously about my own future... I'm still
in the middle of thinking about it though?");
MsgDisp("主人公","(Hehe!
I'm also looking forward to 
｛みちる＊｝'s dream')");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEyeOpenLevel(21,-1);
