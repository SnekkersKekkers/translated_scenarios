ChLayout(2);
BGOpen("sc310",0);
ScrFadeIn(0);
MsgDisp("主人公","(Now, the next class is...
Ah, over there is...——)");
SEPlay("EV_SE_FOOT_WALK_KEEP_ALONE");
MsgClose();
ScrFadeOut(0);
BGOpen("sc510",0);
SEWait();
MsgClose();
BGMPlay("BGM_HANYOU_A",0.01);
ChOpen(21,254,0,0,0,#1,#1,0,1,0,30);
ChOpen(7,254,0,0,0,#1,#1,0,0,0,30);
ChOpen(22,254,0,0,3,#1,#1,0,2,0,30);
ScrFadeIn(0);
MsgDisp("主人公","If it's these three then...");
ChEye(21,0);
ChMouth(21,0);
ChMotion(21,3);
VoicePlay("F100730002_21_000");
MsgDisp("Michiru","That's it.");
ChEye(22,0);
ChMouth(22,3);
ChMotion(22,5,1);
VoicePlay("F100730002_22_000");
MsgDisp("Hikaru","I won't let you get away today!");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("F100730002_07_000");
MsgDisp("Mikage","That's kind of scary.
I'm wearing a classy suit today, so there
shouldn't be a problem, right?");
ChEye(21,0);
ChMouth(21,4);
ChMotion(21,4);
VoicePlay("F100730002_21_010");
MsgDisp("Michiru","Yeah, that's right.
There's no problem, though..");
ChEye(22,0);
ChMouth(22,2);
ChMotion(22,0,1);
ChEyeOpenLevel(22,10);
VoicePlay("F100730002_22_010");
MsgDisp("Hikaru","Hmm, it's actually boring that there's no
problem.
Right, Mari?");
MsgDisp("主人公","Eh, um...
I think it's good that he looks
teacher-like...");
ChEye(22,0);
ChEye(21,0);
ChMouth(21,0);
ChMotion(21,0);
VoicePlay("F100730002_21_020");
MsgDisp("Michiru","Right, he seems like
a normal teacher.");
ChEye(22,0);
ChMouth(22,3);
ChMotion(22,4,1);
ChEyeOpenLevel(22,6);
VoicePlay("F100730002_22_020");
MsgDisp("Hikaru","Hey are you okay with being ordinary?
Mikage-sensei.");
ChEye(7,0);
ChMouth(7,4);
ChMotion(7,1);
VoicePlay("F100730002_07_010");
MsgDisp("Mikage","Hm? It's good that there's no problem, but
it's kind of troubling if normal is
boring.");
ChEye(21,0);
ChMouth(21,3);
ChMotion(21,3);
VoicePlay("F100730002_21_030");
MsgDisp("Michiru","Right?
Mikage-sensei can't be ordinary.");
ChEye(22,3);
ChMouth(22,3);
ChMotion(22,3,1);
VoicePlay("F100730002_22_030");
MsgDisp("Hikaru","In that case, leave it to us!
I'll ask Uncle about the perfect fashion
for Mikage-sensei♪");
ChEye(7,0);
ChMouth(7,4);
ChMotion(7,0);
VoicePlay("F100730002_07_020");
MsgDisp("Mikage","...Uncle?");
MsgDisp("主人公","Ehh!　Goro-sensei∋");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,3);
VoicePlay("F100730002_07_030");
MsgDisp("Mikage","Oh, the world-famous fashion leader
Hanatsubaki Goro sensei. That's amazing.
I'm looking forward to it.");
ChEye(21,3);
ChMouth(21,3);
ChMotion(21,0);
VoicePlay("F100730002_21_040");
MsgDisp("Michiru","We're also looking forward to it.");
ChEye(21,0);
ChMouth(21,3);
ChEye(22,3);
ChMouth(22,3);
ChMotion(22,4,1);
VoicePlay("F100730002_22_040");
MsgDisp("Hikaru","Yeah, I'm sure uncle will like
Mikage-sensei▼
right, Mari?");
MsgDisp("主人公","Y-Yeah.");
MsgDisp("主人公","(Goro-sensei's fashion is quite original
though...?
Will ｛御影＊＊｝ be alright...)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
