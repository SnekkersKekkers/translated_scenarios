BGOpen("sc520",1);
ChLayout(1);
ScrFadeIn(0);
MsgClose();
BGMPlay("BGM_HANYOU_A",0.01);
ChOpen(22,254,0,0,0,#1,#1,0,1);
ChOpen(21,254,0,0,0,#1,#1,0,2);
ChEye(22,0);
ChMouth(22,3);
ChMotion(22,3,1);
VoicePlay("F102230002_22_000");
MsgDisp("Hikaru","Ah, Mari!");
ChEye(21,0);
ChMouth(21,3);
ChMotion(21,3);
VoicePlay("F102230002_21_000");
MsgDisp("Michiru","You're right on time again.");
MsgDisp("主人公","Hehe, are you checking character bento
answers again?");
ChEye(21,0);
ChMouth(21,4);
ChMotion(21,4);
VoicePlay("F102230002_21_010");
MsgDisp("Michiru","Right.
Hey...if you still can't guess it today, I
guess I'll go to a cooking class...");
ChEye(22,4);
ChMouth(22,0);
ChMotion(22,3,1);
VoicePlay("F102230002_22_010");
MsgDisp("Hikaru","Now, now!
Then, shall we go ahead with the answer～?");
ChEye(21,0);
ChMouth(21,4);
ChMotion(21,0);
VoicePlay("F102230002_21_020");
MsgDisp("Michiru","Go ahead.");
ChEye(22,4);
ChMouth(22,0);
ChMotion(22,4,1);
VoicePlay("F102230002_22_020");
MsgDisp("Hikaru","Today...");
ChEye(22,3);
ChMouth(22,3);
ChMotion(22,5,1);
ChEyeOpenLevel(22,0);
VoicePlay("F102230002_22_030");
MsgDisp("Hikaru","It's a skull bear!");
ChEye(21,3);
ChMouth(21,3);
ChMotion(21,3);
VoicePlay("F102230002_21_030");
MsgDisp("Michiru","Correct!
I'm glad...");
ChMotion(22,3,1);
ChEyeOpenLevel(22,#1);
MsgDisp("主人公","｛みちる＊｝, congratulations!");
ChEye(21,0);
ChMouth(21,3);
ChMotion(21,0);
VoicePlay("F102230002_21_040");
MsgDisp("Michiru","Thank you, Mari.");
ChEye(22,2);
ChMouth(22,3);
ChMotion(22,4,1);
VoicePlay("F102230002_22_040");
MsgDisp("Hikaru","It was Hikaru who guessed though...
Well, that's fine～");
ChEye(22,4);
ChMouth(22,2);
ChMotion(22,2,1);
ChEyeOpenLevel(22,10);
VoicePlay("F102230002_22_050");
MsgDisp("Hikaru","...You know, if it's a brown animal with
its bones exposed, there's only one thing
that comes to mind...");
MsgDisp("主人公","Eh...?");
ChEye(22,0);
ChMouth(22,3);
ChMotion(22,1,1);
VoicePlay("F102230002_22_060");
MsgDisp("Hikaru","It's nothing?
Big sis, we're graduating from the animal
series! What should we do next～?");
ChEye(21,0);
ChMouth(21,0);
ChMotion(21,3);
VoicePlay("F102230002_21_050");
MsgDisp("Michiru","Then...how about flowers?");
ChEye(22,3);
ChMouth(22,3);
ChMotion(22,3,1);
VoicePlay("F102230002_22_070");
MsgDisp("Hikaru","Uwah...that seems even harder!");
MsgDisp("主人公","(Hehe! These two really get along.)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
