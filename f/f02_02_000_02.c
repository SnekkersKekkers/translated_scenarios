BGMStop();
BGOpen("sc000",1);
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0,1);
ChLayout(1);
ScrFadeIn(0);
VoicePlay("F020200002_02_000");
MsgDisp("Sassa","There's a lot of legends about that
church.
A lot of them are like stories.");
MsgDisp("主人公","Yeah, people find them amusing.
But the story about the prince and 
the princess is the most famous.");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,4);
VoicePlay("F020200002_02_010");
MsgDisp("Sassa","From what I heard, 
there's no more prince or princess.
Instead, there's Mikage-sensei.");
MsgDisp("主人公","｛御影＊＊｝?");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0);
VoicePlay("F020200002_02_020");
MsgDisp("Sassa","Yeah, Mikage-sensei. You know, he's the
advisor for the track and field club,
but he doesn't show up to practices.");
ChEye(2,0);
ChMouth(2,3);
ChMotion(2,4);
VoicePlay("F020200002_02_030");
MsgDisp("Sassa","But I heard he's assembling a team
of exceptional players and putting them
in special training at the church.");
MsgDisp("主人公","Huh?
｛颯砂＊＊｝, are you one of them? ");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0);
VoicePlay("F020200002_02_040");
MsgDisp("Sassa","No, it's a rumor.");
ChEye(2,0);
ChMouth(2,3);
ChMotion(2,4);
VoicePlay("F020200002_02_050");
MsgDisp("Sassa","I heard there's a cloud of dust near
the church. At the center of it, you can
see Mikage-sensei, happily nodding.");
MsgDisp("主人公","A dust cloud...
Like when someone's running really fast?");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0);
VoicePlay("F020200002_02_060");
MsgDisp("Sassa","Come on.
It's not a manga, so no matter how fast
you run, a dust cloud won't appear.");
MsgDisp("主人公","(...We had fun going home,
while discussing that story.)");
