BGMStop();
BGOpen("sc000",1);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
ChLayout(1);
ScrFadeIn(0);
MsgDisp("主人公","There are way too many stories about that
legendary church on school grounds.
It's impossible to get the full picture.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("F020600003_06_000");
MsgDisp("Himuro","I heard something.");
MsgDisp("主人公","Oh, what?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("F020600003_06_010");
MsgDisp("Himuro","About what my relative is doing in
there.");
MsgDisp("主人公","By relative, do you mean Vice Principal
Himuro?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("F020600003_06_020");
MsgDisp("Himuro","Yes. There are all sorts of guesses,
from secret experiments to
black magic rituals.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,1);
ChEyeOpenLevel(6,0);
VoicePlay("F020600003_06_030");
MsgDisp("Himuro","So, useless gossip.");
MsgDisp("主人公","Vice principal Himuro and the church...
yeah, there's no connection there.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("F020600003_06_040");
MsgDisp("Himuro","Are you sure about that?");
MsgDisp("主人公","Eh, why?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("F020600003_06_050");
MsgDisp("Himuro","That guy, he's kind of a a romantic at
heart.");
MsgDisp("主人公","Huh?");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,3);
VoicePlay("F020600003_06_060");
MsgDisp("Himuro","So he might believe in the legend of the
prince and the princess or something.");
MsgDisp("主人公","Eh∋");
MsgDisp("主人公","(...We had fun talking about stuff
like that on our way home.)");
