BGOpen("sc710",0);
ChLayout(1);
MsgClose();
ChOpen(22,254,0,3,3,0,-1,0,0);
ScrFadeIn(0);
VoicePlay("F112210000_22_000");
MsgDisp("Hikaru","Everything good, everythings good～");
MsgDisp("主人公","Oh, ｛ひかる＊｝.
What's with the good mood?");
ChEye(22,0);
ChMouth(22,3);
ChMotion(22,4,1);
VoicePlay("F112210000_22_010");
MsgDisp("Hikaru","Ah, Mari!
Just now, someone gave Hikaru 
a takoyaki～");
MsgDisp("主人公","Was it from a guy?");
ChEye(22,3);
ChMouth(22,3);
ChMotion(22,2,1);
ChEyeOpenLevel(22,0);
VoicePlay("F112210000_22_020");
MsgDisp("Hikaru","Ew, no way～!");
ChEye(22,0);
ChMouth(22,3);
ChMotion(22,4,1);
VoicePlay("F112210000_22_030");
MsgDisp("Hikaru","At the fortune-telling corner,
there's a girl who told Hikaru's 
fortune.");
MsgDisp("主人公","Hm?");
ChEye(22,0);
ChMouth(22,3);
ChMotion(22,0,1);
VoicePlay("F112210000_22_040");
MsgDisp("Hikaru","Hikaru went to get her fortune told,
then saw the girl's face, and realized.
That girl's in love with someone～");
ChMotion(22,4,1);
VoicePlay("F112210000_22_050");
MsgDisp("Hikaru","So, Hikaru will give you advice.
Then, you'll thank her～!");
MsgDisp("主人公","S-Sure.");
ChEye(22,3);
ChMouth(22,3);
ChMotion(22,3,1);
VoicePlay("F112210000_22_060");
MsgDisp("Hikaru","Hikaru hopes that girl's love 
comes true.");
MsgDisp("主人公","(｛ひかる＊｝ seems more
like a fortune teller...");
MsgClose();
ScrFadeOut(0,0);
ChClose(22,0,0);
