MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
BGOpen("ne620",0);
BGMPlay("BGM_C05_HIIRAGI_B",0.01);
ChLayout(1);
MsgClose();
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0,1);
ScrFadeIn(0);
ChEye(5,3);
VoicePlay("B360518001_05_000");
MsgDisp("Hiiragi","Phew, that wasn't bad.
Thank you for joining me.");
MsgDisp("主人公","Thank you too.");
ChEye(5,4);
ChMotion(5,4);
VoicePlay("B360518001_05_010");
MsgDisp("Hiiragi","I received this from the supporters of the
theatre company.");
VoicePlay("B360518001_05_020");
MsgDisp("Hiiragi","Only two.");
MsgDisp("主人公","I see.
I wanted ｛Mikage＊＊｝ and ｛Himuro＊＊｝ to
see it too.");
ChEye(5,0);
ChMotion(5,0);
VoicePlay("B360518001_05_030");
MsgDisp("Hiiragi","Yeah.
But the four of us lined up watching a movie
together would be quite the funny sight.");
MsgDisp("主人公","Does it really?");
ChEye(5,4);
VoicePlay("B360518001_05_040");
MsgDisp("Hiiragi","Yeah, just imagine it.");
ChMotion(5,4);
VoicePlay("B360518001_05_050");
MsgDisp("Hiiragi","Mikage-sensei, me, you, and Inori-kun all
lined up with serious expressions...");
MsgDisp("主人公","...Ahahaha.");
ChMotion(5,0);
VoicePlay("B360518001_05_060");
MsgDisp("Hiiragi","Right?");
VoicePlay("B360518001_48_000");
MsgDisp("Staff","I'm sorry.
We'll clean it up right away.");
ChEye(5,3);
ChMouth(5,2);
ChMotion(5,4);
VoicePlay("B360518001_05_070");
MsgDisp("Hiiragi","Ah, yes.");
ChEye(5,4);
ChMouth(5,4);
VoicePlay("B360518001_05_080");
MsgDisp("Hiiragi","...It's getting late.
I'll take my time to reflect on the movie on
my way home.");
MsgDisp("主人公","Yeah!");
BGMStop();
MsgClose();
ScrFadeOut(0);
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0);
ChCheek(5,0);
