BGOpen("fp610",0);
BGMPlay("BGM_PLACE_SKI_SKAT",0.01);
ChLayout(2);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,4,1);
ChEye(5,2);
ChMouth(5,4);
ChMotion(5,0,1);
ChEyeOpenLevel(5,9);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,7);
ScrFadeIn(0);
VoicePlay("B330C30301_07_000");
MsgDisp("Mikage","Phew...
We skated a lot today～!");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
VoicePlay("B330C30301_06_000");
MsgDisp("Himuro","I don't even know how 
many laps we all did.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0,1);
MsgDisp("主人公","Hehe, yeah.");
ChEye(5,2);
ChMouth(5,2);
ChMotion(5,0,1);
ChEyeOpenLevel(5,0);
VoicePlay("B330C30301_05_000");
MsgDisp("Hiiragi","Ah...");
MsgDisp("主人公","｛柊＊＊＊｝, are you alright?");
ChEye(5,2);
ChMouth(5,4);
ChMotion(5,0);
ChEyeOpenLevel(5,8);
VoicePlay("B330C30301_05_010");
MsgDisp("Hiiragi","Yes.
But we kept doing the same 
rotation each time.");
ChEyeOpenLevel(5,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B330C30301_06_010");
MsgDisp("Himuro","I was curious about it too, 
are we inclined to go counter-clockwise?");
ChMotion(5,4);
ChEyeOpenLevel(5,9);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("B330C30301_07_010");
MsgDisp("Mikage","Yeah, it seems that going left is easier 
due to the structure of the human body.
It's the same in track and field, right?");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,3);
VoicePlay("B330C30301_06_020");
MsgDisp("Himuro","Heeh...
As expected of the track-and-field 
club's phantom advisor.");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,2);
VoicePlay("B330C30301_07_020");
MsgDisp("Mikage","Yeah.
Wanna go the opposite way next?");
ChMouth(5,2);
ChEyeOpenLevel(5,0);
MsgDisp("主人公","No you can't!");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(5,0);
ChMouth(5,0);
ChMotion(5,7);
ChEyeOpenLevel(5,#1);
ChMouthOpenLevel(5,#1);
ChCheek(5,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,7);
ChEyeOpenLevel(6,#1);
ChMouthOpenLevel(6,#1);
ChCheek(6,0);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,7);
ChEyeOpenLevel(7,#1);
ChMouthOpenLevel(7,#1);
ChCheek(7,0);
