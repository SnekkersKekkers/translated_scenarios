MsgDisp("主人公","｛Mikage＊＊｝, recently, you're around
｛Hiiragi＊＊＊｝ and ｛Himuro＊＊｝ a lot, but what
sort of things do you talk about?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,3);
VoicePlay("B070700001_07_250");
MsgDisp("Mikage","Surprisingly, Inori's the one leading the
conversation, you know?");
MsgDisp("主人公","Is that right?");
ChEye(7,4);
ChMouth(7,3);
ChMotion(7,1);
VoicePlay("B070700001_07_260");
MsgDisp("Mikage","Yeah, while he's chatting with Yanosuke,
he'll try to probe into the student council's
activities.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("B070700001_07_270");
MsgDisp("Mikage","He'll also discreetly try to get me to reveal
information about school events.");
MsgDisp("主人公","Hehe, how clever.
That's just like ｛Himuro＊＊｝.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,5);
VoicePlay("B070700001_07_280");
MsgDisp("Mikage","Yeah, but Yanosuke's onto him too, so he'll
quietly tell me on the side that he \"mixed in
some wrong info.\"");
MsgDisp("主人公","Wow... ｛Hiiragi＊＊＊｝ is pretty funny.
Does ｛Himuro＊＊｝ fall for it?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,4);
VoicePlay("B070700001_07_290");
MsgDisp("Mikage","Who knows, he might also enjoy that sort
of stuff?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("B070700001_07_300");
MsgDisp("Mikage","Furthermore, the thing they're most invested
in is you.");
MsgDisp("主人公","Ehh∋");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,3);
VoicePlay("B070700001_07_310");
MsgDisp("Mikage","They can't stand that I'm the only one with
info on you. Even when I tell them it's
because I'm your homeroom teacher.");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,1);
VoicePlay("B070700001_07_320");
MsgDisp("Mikage","Yanosuke will go, \"That's hardly fair\" and
Inori says stuff like \"How childish\".
Nonsensical stuff like that.");
MsgDisp("主人公","What information do you have on me?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,5);
VoicePlay("B070700001_07_330");
MsgDisp("Mikage","Mostly grades～
Well, I have to keep confidentiality.
Don't worry, focus on your studies.");
MsgDisp("主人公","(Ehh...
What exactly are those three talking about
when it comes to me?)");
BGMStop();
MsgClose();
ScrFadeOut(0);
ChEyeOpenLevel(7,#1);
ChMouthOpenLevel(7,#1);
ChCheek(7,0);
