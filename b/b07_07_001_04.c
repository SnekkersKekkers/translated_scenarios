MsgDisp("主人公","｛御影＊＊｝,
recently, you're around ｛柊＊＊＊｝ and
｛氷室＊＊｝ a lot, but what sort 
of things do you talk about?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,3);
VoicePlay("B070700001_07_250");
MsgDisp("Mikage","Surprisingly Inori is
the one doing the talking?");
MsgDisp("主人公","Is that right?");
ChEye(7,4);
ChMouth(7,3);
ChMotion(7,1);
VoicePlay("B070700001_07_260");
MsgDisp("Mikage","Yeah, whilst he talks to Yanosuke,
he'll try to probe into the
student council's activities.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("B070700001_07_270");
MsgDisp("Mikage","He'll also discreetly try to 
get me to reveal information
about school events.");
MsgDisp("主人公","Hehe, how clever.
That's just like ｛氷室＊＊｝.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,5);
VoicePlay("B070700001_07_280");
MsgDisp("Mikage","Yeah, but Yanosuke also knows so, 
he'll quietly tell me that he
\"added some slight 
misinformation\" on the side.");
MsgDisp("主人公","Heeh...｛柊＊＊＊｝ is funny. 
Is ｛氷室＊＊｝ fooled?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,4);
VoicePlay("B070700001_07_290");
MsgDisp("Mikage","Who knows, he might also 
enjoy that sort of stuff?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("B070700001_07_300");
MsgDisp("Mikage","Furthermore, the thing they're 
most invested in is you.");
MsgDisp("主人公","Ehh∋");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,3);
VoicePlay("B070700001_07_310");
MsgDisp("Mikage","They can't forgive that I'm the only one
who has information on you. Even if I 
say it's because I'm your homeroom 
teacher, they can't accept that.");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,1);
VoicePlay("B070700001_07_320");
MsgDisp("Mikage","Yanosuke will go, \"That's hardly fair,\"
and Inori will normally say something 
like \"How childish\",
nonsensical stuff like that.");
MsgDisp("主人公","What information do you have on me?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,5);
VoicePlay("B070700001_07_330");
MsgDisp("Mikage","Mostly grades～
Well, I have to keep confidentiality.
Don't worry, focus on your studies.");
MsgDisp("主人公","(Ehh...What exactly are those
three talking about 
when it comes to me?)");
BGMStop();
MsgClose();
ScrFadeOut(0);
ChEyeOpenLevel(7,#1);
ChMouthOpenLevel(7,#1);
ChCheek(7,0);
