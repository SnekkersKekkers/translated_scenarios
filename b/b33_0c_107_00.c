ChClose(5,0,0);
ChClose(6,0,0);
ChClose(7,0,0);
BGOpen("tr400",0);
ScrFadeIn(0);
Wait(40,1);
ScrFadeOut(1,0,40);
BGOpen("tr450",0);
ScrFadeIn(1,40);
Wait(60,1);
ScrFadeOut(1,0,40);
MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
BGOpen("tr450",0);
BGMPlay("BGM_PLACE_AMUSEMENT",0.01);
ChLayout(2);
ChOpen(5,255,0,4,4,-1,-1,0,3);
ChOpen(6,255,2,2,0,0,-1,0,3);
ChOpen(7,255,2,2,2,0,-1,0,3);
ScrFadeIn(0);
MsgDisp("主人公","Umm...
I want to ride it one last time.");
ChEye(7,2);
ChMouth(7,2);
ChMotion(7,4);
VoicePlay("B330C10700_07_000");
MsgDisp("Mikage","Uh...");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,5);
VoicePlay("B330C10700_06_000");
MsgDisp("Himuro","Eh...");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("B330C10700_05_000");
MsgDisp("Hiiragi","Yes, let's do that.");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,4);
ChEyeOpenLevel(7,0);
VoicePlay("B330C10700_07_010");
MsgDisp("Mikage","I-Ill leave the rest to you Yanosuke.");
ChEye(6,2);
ChMouth(6,2);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("B330C10700_06_010");
MsgDisp("Himuro","I'll watch Yonosuke-senpai's heroic 
performance with my own eyes.");
ChEye(5,3);
ChMouth(5,2);
MsgDisp("主人公","What's up with ｛氷室＊＊｝ and 
｛御影＊＊｝?");
ChEye(6,2);
ChMouth(6,0);
ChMotion(6,4);
VoicePlay("B330C10700_06_020");
MsgDisp("Himuro","Can't you see that we've been 
defeated by your spinning?");
MsgDisp("主人公","Maybe I spun it 
a little too much...");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,2);
ChEyeOpenLevel(7,8);
VoicePlay("B330C10700_07_020");
MsgDisp("Mikage","A little, yeah.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
ChEyeOpenLevel(6,8);
VoicePlay("B330C10700_06_030");
MsgDisp("Himuro","A little huh...");
MsgDisp("主人公","I'm sorry...
I'll be sure not to spin it again...");
ChEye(5,3);
ChMouth(5,3);
ChMotion(5,0);
VoicePlay("B330C10700_05_010");
MsgDisp("Hiiragi","Why stop?
Let's spin until the handle falls off.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,3);
ChEyeOpenLevel(7,0);
VoicePlay("B330C10700_07_030");
MsgDisp("Mikage","Oohhー.
I think I'm fallimg for you, Yanosuke...");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,3);
VoicePlay("B330C10700_06_040");
MsgDisp("Himuro","Yes.");
ChEye(5,3);
ChMouth(5,3);
ChMotion(5,4);
VoicePlay("B330C10700_05_020");
MsgDisp("Hiiragi","Then, let's go.");
MsgDisp("主人公","Eh∋");
MsgClose();
ScrFadeOut(0,0);
VoicePlay("B330C10700_05_030");
MsgDisp("Hiiragi","Well, let's enjoy 
ourselves to the fullest?");
MsgDisp("主人公","Yeah...!");
MsgDisp("主人公","(I'm happy but, 
Is it really okay to leave ｛氷室＊＊｝
and ｛御影＊＊｝ behind?)");
BGMStop();
MsgClose();
ChEye(5,0);
ChMouth(5,0);
ChMotion(5,7);
ChEyeOpenLevel(5,-1);
ChMouthOpenLevel(5,-1);
ChCheek(5,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,7);
ChEyeOpenLevel(6,-1);
ChMouthOpenLevel(6,-1);
ChCheek(6,0);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,7);
ChEyeOpenLevel(7,-1);
ChMouthOpenLevel(7,-1);
ChCheek(7,0);
