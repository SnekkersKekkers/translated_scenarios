ChClose(1,0,0);
ChClose(3,0,0);
ChClose(4,0,0);
BGOpen("tr400",0);
ScrFadeIn(0);
Wait(40,1);
ScrFadeOut(1,0,40);
BGOpen("tr430",0);
BGMPlay("BGM_PLACE_AMUSEMENT",0.01);
ChLayout(2);
ChNanaType(0);
ChOpen(4,255,0,0,0,#1,#1,0,3);
ChOpen(1,255,0,0,2,#1,#1,0,3);
ChOpen(3,255,0,0,0,#1,#1,0,3);
ScrFadeIn(0);
ChEye(1,4);
VoicePlay("B330B30801_01_000");
MsgDisp("Kazama","Ferris wheels are a date staple.");
ChEye(4,3);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("B330B30801_04_000");
MsgDisp("Nanatsumori","Heeh, is that so?");
ChEye(1,0);
ChMotion(1,1);
VoicePlay("B330B30801_01_010");
MsgDisp("Kazama","It is.
Look, it's a closed room.");
MsgDisp("主人公","Hehe!");
ChEye(3,0);
ChMouth(3,4);
ChMotion(3,0,1);
VoicePlay("B330B30801_03_000");
MsgDisp("Honda","Ryo-kun, what's wrong?");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0);
ChEyeOpenLevel(1,8);
VoicePlay("B330B30801_01_020");
MsgDisp("Kazama","What?");
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,1,1);
VoicePlay("B330B30801_03_010");
MsgDisp("Honda","Ah, did you want 
to ride alone with someone?
The ferris wheel that is.");
ChMotion(1,0,1);
ChEyeOpenLevel(1,0);
Wait(8,0);
ChEyeOpenLevel(1,#1);
ChEye(4,1);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("B330B30801_04_010");
MsgDisp("Nanatsumori","Dahon is blunt.");
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,4,1);
VoicePlay("B330B30801_03_020");
MsgDisp("Honda","Is that so, 
you should've said earlier.");
ChEye(4,0);
ChMouth(4,0);
ChEye(1,4);
ChMotion(1,0);
VoicePlay("B330B30801_01_030");
MsgDisp("Kazama","...Eh?");
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,3,1);
ChEyeOpenLevel(3,0);
VoicePlay("B330B30801_03_030");
MsgDisp("Honda","Alone with Ryo-kunー.
Seems interesting!");
ChEye(4,0);
ChMouth(4,3);
ChEye(1,0);
ChMouth(1,1);
ChMotion(1,1);
ChEyeOpenLevel(1,8);
VoicePlay("B330B30801_01_040");
MsgDisp("Kazama","Why is that?
Let's ride is a four.");
ChEye(4,3);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("B330B30801_04_020");
MsgDisp("Nanatsumori","Are you going to ignore
Honda's consideration?
That's cold.");
ChMouth(1,2);
ChEyeOpenLevel(1,0);
MsgDisp("主人公","Hehe, 
let's all ride it together?");
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("B330B30801_04_030");
MsgDisp("Nanatsumori","If you say so, sure.
Let's all go on it as four.");
MsgDisp("主人公","(Hehe! But I might've 
wanted to see ｛風真＊＊｝ and
｛本多＊＊｝'s couple photo together?')");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,7);
ChEyeOpenLevel(1,#1);
ChMouthOpenLevel(1,#1);
ChCheek(1,0);
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,7);
ChEyeOpenLevel(3,#1);
ChMouthOpenLevel(3,#1);
ChCheek(3,0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,7);
ChEyeOpenLevel(4,#1);
ChMouthOpenLevel(4,#1);
ChCheek(4,0);
