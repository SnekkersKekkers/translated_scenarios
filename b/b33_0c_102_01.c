BGOpen("wf310",0);
BGMPlay("BGM_PLACE_MUSE_AQUA_GARDEN",0.01);
ChLayout(2);
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0,1);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,5,1);
ScrFadeIn(0);
VoicePlay("B330C10201_07_000");
MsgDisp("Mikage","Ohh, there's a manta ray over there.");
MsgDisp("主人公","It's quite bit isn't it?");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("B330C10201_05_000");
MsgDisp("Hiiragi","Mikage-sensei, Rays and manta rays are
differen't are they not?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("B330C10201_07_010");
MsgDisp("Mikage","They're different species.
The distinction is simple.
The big ones are manta rays.");
MsgDisp("主人公","Hehe, easy.");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("B330C10201_05_010");
MsgDisp("Hiiragi","Obvious at a glance.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0,1);
VoicePlay("B330C10201_06_000");
MsgDisp("Himuro","Heeh...
This feels like a lesson by Kojiro-sensei.");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("B330C10201_05_020");
MsgDisp("Hiiragi","This definitely feels 
like a class field trip.");
MsgDisp("主人公","Hehe. I guess so.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("B330C10201_07_020");
MsgDisp("Mikage","Well, it is typical to have
a class field trip here.");
ChEye(6,4);
ChMouth(6,0);
ChMotion(6,2);
VoicePlay("B330C10201_06_010");
MsgDisp("Himuro","I see, so this is what it's like...");
ChEye(5,0);
ChMouth(5,2);
ChEye(7,0);
ChMouth(7,4);
ChMotion(7,0);
VoicePlay("B330C10201_07_030");
MsgDisp("Mikage","What's up, Inori.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B330C10201_06_020");
MsgDisp("Himuro","I've never taken Kojiro-sensei's class.");
ChEye(5,4);
ChMouth(5,4);
MsgDisp("主人公","Yeah, this feels like we're
in the same class as ｛氷室＊＊｝ now.");
ChEye(6,4);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B330C10201_06_030");
MsgDisp("Himuro","You and Yanosuke-senpai...");
ChEye(7,0);
ChMouth(7,2);
ChMotion(7,2);
VoicePlay("B330C10201_07_040");
MsgDisp("Mikage","Hm? Yanosuke?");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("B330C10201_05_030");
MsgDisp("Hiiragi","Yes?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("B330C10201_07_050");
MsgDisp("Mikage","Did you invite Inori here so
he could get a taste of 
what that would feel like?");
ChEye(5,2);
ChMouth(5,4);
ChMotion(5,4);
ChEyeOpenLevel(5,9);
ChCheek(5,5);
VoicePlay("B330C10201_05_040");
MsgDisp("Hiiragi","N-No, not at all.
I'm not that clever.");
ChEyeOpenLevel(5,0);
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,4);
ChCheek(6,10);
VoicePlay("B330C10201_06_040");
MsgDisp("Himuro","...Thank you very much.
I'm kind of happy.");
MsgDisp("主人公","(｛氷室＊＊｝
wanted to take classes together...
｛柊＊＊＊｝ is kind isn't he?)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(5,0);
ChMouth(5,0);
ChMotion(5,7);
ChEyeOpenLevel(5,#1);
ChMouthOpenLevel(5,#1);
ChCheek(5,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,7);
ChEyeOpenLevel(6,#1);
ChMouthOpenLevel(6,#1);
ChCheek(6,0);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,7);
ChEyeOpenLevel(7,#1);
ChMouthOpenLevel(7,#1);
ChCheek(7,0);
