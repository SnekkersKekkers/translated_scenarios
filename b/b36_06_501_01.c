MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
BGOpen("ex000",1);
BGMPlay("BGM_C06_INORI_C",0.01);
ChLayout(1);
MsgClose();
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,2,1);
ChEyeOpenLevel(6,0);
ScrFadeIn(0);
VoicePlay("B360650101_06_000");
MsgDisp("Himuro","You're quite popular, huh?");
MsgDisp("主人公","...What do you mean?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,1);
VoicePlay("B360650101_06_010");
MsgDisp("Himuro","The two of them were really fawning over
you today.");
MsgDisp("主人公","Eh!");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("B360650101_06_020");
MsgDisp("Himuro","What? You didn't realise it?
Everyone's eyes were on you.");
MsgDisp("主人公","I guess...?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("B360650101_06_030");
MsgDisp("Himuro","You mean you didn't notice that I was
watching you?");
MsgDisp("主人公","Huh?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360650101_06_040");
MsgDisp("Himuro","I was also watching you.
So I understand what everyone was looking at.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360650101_06_050");
MsgDisp("Himuro","...But I still don't know the most
important part.");
MsgDisp("主人公","Important part...?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360650101_06_060");
MsgDisp("Himuro","You.");
MsgDisp("主人公","Huh, me?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
VoicePlay("B360650101_06_070");
MsgDisp("Himuro","I mean, who you're looking at.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360650101_06_080");
MsgDisp("Himuro","Who are you looking at?
Is it one of those two?
Or someone else?");
MsgDisp("主人公","Right now...
I'm looking at ｛Himuro＊＊｝.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360650101_06_090");
MsgDisp("Himuro","Eh...");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B360650101_06_100");
MsgDisp("Himuro","That's true.
Maybe the one not looking at the person in
front of them just now was me.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0,1);
VoicePlay("B360650101_06_110");
MsgDisp("Himuro","...Thanks.
I almost lost sight of something important.");
MsgDisp("主人公","(｛Himuro＊＊｝...)");
BGMStop();
MsgClose();
ScrFadeOut(0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ChCheek(6,0);
