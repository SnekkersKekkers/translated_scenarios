MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
BGOpen("ex000",1);
BGMPlay("BGM_C05_HIIRAGI_B",0.01);
ChLayout(1);
MsgClose();
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0,1);
ScrFadeIn(0);
ChEye(5,3);
ChMouth(5,2);
ChMotion(5,4);
VoicePlay("B360550101_05_000");
MsgDisp("Hiiragi","Ah, I went too far.");
MsgDisp("主人公","Ehh?");
ChEye(5,2);
ChMouth(5,4);
ChMotion(5,0);
ChEyeOpenLevel(5,9);
VoicePlay("B360550101_05_010");
MsgDisp("Hiiragi","I was thinking of having some tea at a coffee
shop at the shopping district, but...");
ChMotion(5,4);
ChEyeOpenLevel(5,0);
VoicePlay("B360550101_05_020");
MsgDisp("Hiiragi","When I'm talking to you, I lose sight of my
surroundings.
I'm sorry...");
ChEyeOpenLevel(5,9);
MsgDisp("主人公","No problem.
I had a good time talking to you too, so it's
fine.");
ChEye(5,3);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("B360550101_05_030");
MsgDisp("Hiiragi","So, shall we play at this park?");
MsgDisp("主人公","Hehe, okay!");
MsgClose();
ScrFadeOut(0,0);
ChMotion(5,0,1);
MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
BGOpen("ex000",2);
ChEye(5,4);
ChMouth(5,4);
ScrFadeIn(0);
ChEye(5,0);
ChMouth(5,2);
ChMotion(5,4);
VoicePlay("B360550101_05_040");
MsgDisp("Hiiragi","Ah, it's that time already!");
MsgDisp("主人公","Huh?");
ChEye(5,3);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("B360550101_05_050");
MsgDisp("Hiiragi","...Haha, didn't you say the same thing
earlier?");
MsgDisp("主人公","Hehe, that's true.");
ChEye(5,2);
ChMotion(5,4);
ChEyeOpenLevel(5,9);
VoicePlay("B360550101_05_060");
MsgDisp("Hiiragi","Whenever I'm with you, I lose track of time,
no matter where we are.
Let's go home.");
ChMotion(5,0);
ChEyeOpenLevel(5,0);
Wait(12,0);
ChEye(5,4);
VoicePlay("B360550101_05_070");
MsgDisp("Hiiragi","Otherwise, I'm afraid it'll be morning again.");
ChEye(5,3);
VoicePlay("B360550101_05_080");
MsgDisp("Hiiragi","Or maybe I've just grown into an old man?");
MsgDisp("主人公","(｛Hiiragi＊＊＊｝, you sound like Urashima
Taro...)");
BGMStop();
MsgClose();
ScrFadeOut(0);
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0);
ChCheek(5,0);
