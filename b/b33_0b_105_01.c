BGOpen("ne340",0);
ChLayout(2);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0,1);
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,0,1);
ChNanaType(0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
ScrFadeIn(0);
MsgDisp("主人公","...Ah!
ＳＵＰＥＲ ＣＨＡＲＧＥＲ's new album is
already out!");
ChEye(4,1);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("B330B10501_04_000");
MsgDisp("Nanatsumori","That's because it's ＳＵＰＥＲ
ＣＨＡＲＧＥＲ.");
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,1,1);
ChEyeOpenLevel(3,0);
VoicePlay("B330B10501_03_000");
MsgDisp("Honda","Yep yep!
I want to sing it!");
ChEye(3,0);
ChMouth(3,0);
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,1);
VoicePlay("B330B10501_01_000");
MsgDisp("Kazama","Wait a second.
Maybe everyone is thinking that.");
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
VoicePlay("B330B10501_04_010");
MsgDisp("Nanatsumori","Yeah.
...How should we do this?
Rock-paper-scissors?");
ChEye(3,0);
ChMouth(3,4);
ChMotion(3,4,1);
VoicePlay("B330B10501_03_010");
MsgDisp("Honda","Why?
Let's all sing!");
ChEye(1,2);
ChMouth(1,3);
ChMotion(1,4);
VoicePlay("B330B10501_01_010");
MsgDisp("Kazama","That's not right...");
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("B330B10501_04_020");
MsgDisp("Nanatsumori","Yeah.");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0);
MsgDisp("主人公","T-Then why don't we all take turns singing
it?");
ChEye(3,0);
ChMouth(3,4);
ChMotion(3,0,1);
VoicePlay("B330B10501_03_020");
MsgDisp("Honda","I'm fine even with that.
But you'll end up listening to the same
songs 3 times though?");
MsgDisp("主人公","Yeah, that's fine.");
ChEye(1,2);
ChMouth(1,0);
ChMotion(1,0,1);
VoicePlay("B330B10501_01_020");
MsgDisp("Kazama","15 minutes of listening to the same song?
Is that okay?");
MsgDisp("主人公","Yeah.
I look forward to hearing everyone sing!");
ChEye(1,0);
ChEye(3,0);
ChMouth(3,3);
ChMotion(3,3,1);
VoicePlay("B330B10501_03_030");
MsgDisp("Honda","Then, let's decide the order!");
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
VoicePlay("B330B10501_04_030");
MsgDisp("Nanatsumori","Rock-paper scissors?");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0,1);
ChEyeOpenLevel(1,8);
VoicePlay("B330B10501_01_030");
MsgDisp("Kazama","Alright...
I'll win this competition.");
MsgDisp("主人公","(Hehe!
It's turned into a serious game of rock,
paper, scissors at karaoke.)");
MsgClose();
ScrFadeOut(0,0);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,7);
ChEyeOpenLevel(1,#1);
ChMouthOpenLevel(1,#1);
ChCheek(1,0);
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,7);
ChEyeOpenLevel(3,#1);
ChMouthOpenLevel(3,#1);
ChCheek(3,0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,7);
ChEyeOpenLevel(4,#1);
ChMouthOpenLevel(4,#1);
ChCheek(4,0);
