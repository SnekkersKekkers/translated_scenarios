BGOpen("fp310",0);
BGMPlay("BGM_PLACE_MUSE_AQUA_GARDEN",0.01);
ChLayout(2);
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,0,1);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,1,1);
ChNanaType(0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
ScrFadeIn(0);
ChMouth(1,2);
VoicePlay("B330B20301_01_000");
MsgDisp("Kazama","Ah, there's banana's growing.");
ChEye(4,3);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("B330B20301_04_000");
MsgDisp("Nanatsumori","Ohー, they're ripe.");
ChMouth(1,0);
ChMotion(1,0);
MsgDisp("主人公","Hehe!
There's so many growing!");
ChEye(3,4);
ChMouth(3,0);
ChMotion(3,4,1);
VoicePlay("B330B20301_03_000");
MsgDisp("Honda","But, 
they've gotten pretty ripe.");
ChMouth(1,2);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,1,1);
VoicePlay("B330B20301_04_010");
MsgDisp("Nanatsumori","Is that a bad thing?");
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,0,1);
VoicePlay("B330B20301_03_010");
MsgDisp("Honda","The yellow bananas we normally
eat are green at harvest.
If you let them ripen more,
they'll be delicious.");
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
VoicePlay("B330B20301_04_020");
MsgDisp("Nanatsumori","Heeh. 
Then are these ones not delicious?");
ChEye(3,0);
ChMouth(3,4);
ChMotion(3,1,1);
VoicePlay("B330B20301_03_020");
MsgDisp("Honda","If they ripen too much on the
tree, they'll lose their flavor, 
but I wonder about these ones. Ah, you 
can't just pick them as you like okay?");
ChEye(4,0);
ChMouth(4,1);
ChMotion(4,2,1);
VoicePlay("B330B20301_04_030");
MsgDisp("Nanatsumori","I wasn't going to pick them!");
ChMouth(3,0);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0,1);
VoicePlay("B330B20301_01_010");
MsgDisp("Kazama","Then, how about taking a picture?");
ChEye(4,2);
ChMouth(4,0);
ChMotion(4,0,1);
VoicePlay("B330B20301_04_040");
MsgDisp("Nanatsumori","Who would be happy to 
take a picture of a banana?");
MsgDisp("主人公","Hey, it says there's a 
tasting corner over there?");
ChEye(4,3);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("B330B20301_04_050");
MsgDisp("Nanatsumori","Tasting?
Nice.");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("B330B20301_01_020");
MsgDisp("Kazama","It says it's the fruits harvested
at this botanical garden.");
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,3,1);
VoicePlay("B330B20301_03_030");
MsgDisp("Honda","We have to go!
Right?");
MsgDisp("主人公","(I'm sure ｛本多＊＊｝ will start
explaining lots of different things to us
whilst we're sampling the fruit.)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,7);
ChEyeOpenLevel(1,-1);
ChMouthOpenLevel(1,-1);
ChCheek(1,0);
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,7);
ChEyeOpenLevel(3,-1);
ChMouthOpenLevel(3,-1);
ChCheek(3,0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,7);
ChEyeOpenLevel(4,-1);
ChMouthOpenLevel(4,-1);
ChCheek(4,0);
