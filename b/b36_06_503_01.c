MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
BGOpen("ex060",0);
BGMPlay("BGM_PLACE_SHOP",0.01);
ChLayout(1);
MsgClose();
ChEye(6,2);
ChMouth(6,0);
ChMotion(6,2,1);
ChEyeOpenLevel(6,0);
ScrFadeIn(0);
VoicePlay("B360650301_06_000");
MsgDisp("Himuro","Sigh... I'm tired...");
MsgDisp("主人公","Good job today.");
ChEye(6,2);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("B360650301_06_010");
MsgDisp("Himuro","My shoulders are so stiff.
...Why did I decide to hang out with
those guys?");
MsgDisp("主人公","You didn't enjoy it?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360650301_06_020");
MsgDisp("Himuro","It's not that, it's just...");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360650301_06_030");
MsgDisp("Himuro","...I feel on edge.");
MsgDisp("主人公","They seem to get along well...");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,1);
ChEyeOpenLevel(6,0);
VoicePlay("B360650301_06_040");
MsgDisp("Himuro","That's another issue.");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,1);
VoicePlay("B360650301_06_050");
MsgDisp("Himuro","I mean it's not like I'm good friends
with them!");
MsgDisp("主人公","Huh, is that so?");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("B360650301_06_060");
MsgDisp("Himuro","Yeah.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360650301_06_070");
MsgDisp("Himuro","...But it's not like we're on bad
terms either.");
MsgDisp("主人公","Hehe.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,1);
VoicePlay("B360650301_06_080");
MsgDisp("Himuro","What's with that knowing laugh?
You're so annoying.");
MsgDisp("主人公","I'm sorry?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360650301_06_090");
MsgDisp("Himuro","You really don't think it's a bad thing,
do you?
Jeez...");
MsgDisp("主人公","(You're not being honest, huh?)");
BGMStop();
MsgClose();
ScrFadeOut(0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ChCheek(6,0);
