BGOpen("fp200",0);
ChLayout(2);
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0,1);
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,0,1);
ChNanaType(0);
ChEye(4,2);
ChMouth(4,3);
ChMotion(4,0,1);
ScrFadeIn(0);
ChMotion(4,1,1);
VoicePlay("B330B10400_04_000");
MsgDisp("Nanatsumori","This isn't the type of place high school
students would come to hang out...");
ChEye(3,0);
ChMouth(3,3);
ChMotion(3,4,1);
VoicePlay("B330B10400_03_000");
MsgDisp("Honda","Really?
I'm super excited!");
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0,1);
VoicePlay("B330B10400_01_000");
MsgDisp("Kazama","Why?");
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,3,1);
ChEyeOpenLevel(3,0);
ChEye(4,2);
ChMouth(4,1);
VoicePlay("B330B10400_03_010");
MsgDisp("Honda","Just look, in the pond.
It's full of aquatic life.
It's a treasure trove of excitement!");
ChMotion(4,2,1);
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0,1);
ChEyeOpenLevel(1,8);
VoicePlay("B330B10400_01_010");
MsgDisp("Kazama","...Indeed.
I'm sure there are some weird insects in
there too.");
ChMouthOpenLevel(4,0);
VoicePlay("B330B10400_04_010");
MsgDisp("Nanatsumori","......");
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,0,1);
MsgDisp("主人公","｛Nanatsumori＊｝, is something wrong?");
ChEyeOpenLevel(1,0);
ChEye(4,2);
ChMouth(4,4);
VoicePlay("B330B10400_04_020");
MsgDisp("Nanatsumori","No, I just don't want to come across
any bugs.");
ChMouth(1,0);
ChEyeOpenLevel(1,#1);
ChEye(3,0);
ChMouth(3,3);
ChMotion(3,1,1);
VoicePlay("B330B10400_03_020");
MsgDisp("Honda","They're not just in the water, they're in
the grass too!
I wonder where they are...");
ChMouth(1,2);
ChMotion(1,0,1);
ChEye(4,2);
ChMouth(4,1);
ChMotion(4,1,1);
VoicePlay("B330B10400_04_030");
MsgDisp("Nanatsumori","You don't have to look for them!");
ChEye(1,2);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("B330B10400_01_020");
MsgDisp("Kazama","Nanatsumori, you're getting too worked up.
At the most, it's like a pill bug or
something.");
ChEye(4,2);
ChMouth(4,1);
ChMotion(4,2,1);
VoicePlay("B330B10400_04_040");
MsgDisp("Nanatsumori","Your idea of \"at most\" and my idea of \"at
most\" are on totally different levels.");
MsgDisp("主人公","Hehe!");
ChEye(1,3);
ChMouth(1,2);
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,5,1);
ChEye(4,5);
ChMouth(4,5);
ChMotion(4,5,1);
ChMouthOpenLevel(4,10);
VoicePlay("B330B10400_03_030");
MsgDisp("Honda","Ah!");
ChMouth(4,1);
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,3);
VoicePlay("B330B10400_01_030");
MsgDisp("Kazama","Oh, was there something there?");
ChEye(4,2);
ChMotion(4,2,1);
VoicePlay("B330B10400_04_050");
MsgDisp("Nanatsumori","...Stop.");
ChEye(1,3);
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,3,1);
ChEyeOpenLevel(3,0);
VoicePlay("B330B10400_03_040");
MsgDisp("Honda","There was a rice fish!
How rare!
So cute～!");
ChEye(4,4);
ChMouth(4,4);
ChMotion(4,0,1);
ChMouthOpenLevel(4,2);
ChEye(1,4);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("B330B10400_01_040");
MsgDisp("Kazama","Ehhh.
It's been a long time since I've seen a
rice fish.");
ChEye(4,2);
ChMouth(4,3);
MsgDisp("主人公","(Hehe!
In the end, everyone is having fun here!)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,7);
ChEyeOpenLevel(1,#1);
ChMouthOpenLevel(1,#1);
ChCheek(1,0);
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,7);
ChEyeOpenLevel(3,#1);
ChMouthOpenLevel(3,#1);
ChCheek(3,0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,7);
ChEyeOpenLevel(4,#1);
ChMouthOpenLevel(4,#1);
ChCheek(4,0);
