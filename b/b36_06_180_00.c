MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
BGOpen("ne610",0);
BGMPlay("BGM_C06_INORI_C",0.01);
ChLayout(1);
MsgClose();
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,0,1);
ChEyeOpenLevel(6,0);
ScrFadeIn(0);
VoicePlay("B360618000_06_000");
MsgDisp("Himuro","..................");
MsgDisp("主人公","What's up?");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,1);
VoicePlay("B360618000_06_010");
MsgDisp("Himuro","I feel a bit proud.");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,0);
VoicePlay("B360618000_06_020");
MsgDisp("Himuro","I don't think those two would believe that
I'm sneaking around like this.");
MsgDisp("主人公","Eh, sneaking around∋");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B360618000_06_030");
MsgDisp("Himuro","Yeah.
The atmosphere isn't nice if there's too
many people.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360618000_06_040");
MsgDisp("Himuro","Now the real fun can begin.");
MsgDisp("主人公","Y-Yeah.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360618000_06_050");
MsgDisp("Himuro","Are you nervous?");
MsgDisp("主人公","A little......");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("B360618000_06_060");
MsgDisp("Himuro","Hmm.
Then how about we do this?");
MsgDisp("主人公","?");
SEPlay("EV_SE_065",0.2,0.6);
SEPlay("EV_SE_504",0.2,0.6);
SEWait();
MsgDisp("主人公","Eh!
｛氷室＊＊｝, your hand......");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,1);
ChCheek(6,10);
VoicePlay("B360618000_06_070");
MsgDisp("Himuro","I know.
Don't you get it?
I did it on purpose.");
MsgDisp("主人公","O-Oh.");
ChEye(6,2);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("B360618000_06_080");
MsgDisp("Himuro","How dense can you get?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360618000_06_090");
MsgDisp("Himuro","Well, I guess you're confused.
Your face is all red too.");
MsgDisp("主人公","......yours is too, ｛氷室＊＊｝.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,5);
VoicePlay("B360618000_06_100");
MsgDisp("Himuro","Huh?");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,1);
ChEyeOpenLevel(6,0);
VoicePlay("B360618000_06_110");
MsgDisp("Himuro","...... The movie's about to start.
Be quiet.");
MsgDisp("主人公","(Hey......!)");
BGMStop();
MsgClose();
ScrFadeOut(0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ChCheek(6,0);
