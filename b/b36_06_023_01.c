MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
BGOpen("wf230",1);
BGMPlay("BGM_C06_INORI_C",0.01);
ChLayout(1);
MsgClose();
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
ScrFadeIn(0);
VoicePlay("B360602301_06_000");
MsgDisp("Himuro","The sunset is beautiful.");
MsgDisp("主人公","Yeah.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B360602301_06_010");
MsgDisp("Himuro","It's a good thing you didn't go home just
yet, huh?");
MsgDisp("主人公","Hehe, you're right.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B360602301_06_020");
MsgDisp("Himuro","I'm glad too.
Now I have you all to myself.");
MsgDisp("主人公","Eh...?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
VoicePlay("B360602301_06_030");
MsgDisp("Himuro","Playing with everyone isn't as bad as I
thought it would be...");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360602301_06_040");
MsgDisp("Himuro","I'm frustrated I can't talk to you
for long.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360602301_06_050");
MsgDisp("Himuro","Getting cut off, or having space between
us— it drives me up the wall.");
MsgDisp("主人公","Is that so?");
ChEye(6,2);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("B360602301_06_060");
MsgDisp("Himuro","That's how I feel.
It's like I'm the only one struggling.");
MsgDisp("主人公","I'm sorry.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B360602301_06_070");
MsgDisp("Himuro","It's fine.
We're together now.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360602301_06_080");
MsgDisp("Himuro","So for just a bit more...can we stay like
this until the sun goes down?");
MsgDisp("主人公","Yes, of course.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B360602301_06_090");
MsgDisp("Himuro","Okay.");
BGMStop();
MsgClose();
ScrFadeOut(0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ChCheek(6,0);
