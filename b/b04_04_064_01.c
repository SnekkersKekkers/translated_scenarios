BGOpen("wf700",2);
ChLayout(1);
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,0);
ScrFadeIn(0);
VoicePlay("B040406401_04_000");
MsgDisp("Nanatsumori","I sure love festivals.");
MsgDisp("主人公","Hm?");
VoicePlay("B040406401_04_010");
MsgDisp("Nanatsumori","Everyone has the same objective, 
they dress the same, they're 
all enjoying this same place. ");
ChEye(4,4);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("B040406401_04_020");
MsgDisp("Nanatsumori","School is the same but, 
I mean...I'm a silent, reticent 
existence there.");
MsgDisp("主人公","I see.");
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("B040406401_04_030");
MsgDisp("Nanatsumori","But, here, 
I can cast away that shield.
I don't really even care
if I'm revealed to be Ｎａｎａ.");
MsgDisp("主人公","Eh?
Is that really okay?");
ChMotion(4,1,1);
VoicePlay("B040406401_04_040");
MsgDisp("Nanatsumori","It's alright.
I can dodge it just by saying \"I'm
in the middle of work right now\".");
MsgDisp("主人公","I see...
then, what is it now?");
ChEye(4,4);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("B040406401_04_050");
MsgDisp("Nanatsumori","Of course it's private.
I'm on a date after all.");
MsgDisp("主人公","!
Y-yeah...");
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("B040406401_04_060");
MsgDisp("Nanatsumori","You're the one who asked a leading 
question and now you're embarassed?
How funny.");
MsgDisp("主人公","Hey,｛七ツ森＊｝!");
SEPlay("EV_SE_022",1);
SEWait();
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
VoicePlay("B040406401_04_070");
MsgDisp("Nanatsumori","Oh.
Is the fireworks display starting?");
MsgDisp("主人公","Seems like it.");
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("B040406401_04_080");
MsgDisp("Nanatsumori","Then, shall we head now?");
MsgDisp("主人公","Yeah!");
MsgClose();
ScrFadeOut(0,0);
ChClose(4,0,0);
