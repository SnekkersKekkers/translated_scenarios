ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B020602001_06_000");
MsgDisp("Himuro","You're wearing that Hazuki Kei T-shirt again?");
MsgDisp("主人公","Yeah, I like wearing it.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
VoicePlay("B020602001_06_010");
MsgDisp("Himuro","Do you like him or something...?");
MsgDisp("主人公","Of course!
He's the pride of Habataki City.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B020602001_06_020");
MsgDisp("Himuro","...That's not what I meant.
Oh well.");
MsgDisp("主人公","(What did he mean by it then?)");
