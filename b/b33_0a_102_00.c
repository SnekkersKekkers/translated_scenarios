BGOpen("wf310",0);
BGMPlay("BGM_PLACE_MUSE_AQUA_GARDEN",0.01);
ChLayout(2);
ChEye(1,2);
ChMouth(1,0);
ChMotion(1,0);
ChEyeOpenLevel(1,8);
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ScrFadeIn(0);
VoicePlay("B330A10200_01_000");
MsgDisp("Kazama","I like the aquarium but...
but what's with us all...
lined up like this?");
MsgDisp("主人公","Hehe, everyone looks like they're
having fun. I think it's good?");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,3);
ChEyeOpenLevel(1,8);
VoicePlay("B330A10200_01_010");
MsgDisp("Kazama","You're laughing saying that huh.");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0);
VoicePlay("B330A10200_02_000");
MsgDisp("Sassa","I think it's good also.
It must look funny from
your perspective.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,1);
VoicePlay("B330A10200_06_000");
MsgDisp("Himuro","Well, the three of us are
lined up, reflected in the tank 
...It's definitely laughable.");
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,4);
VoicePlay("B330A10200_02_010");
MsgDisp("Sassa","Well...");
ChEye(2,3);
ChMouth(2,3);
ChMotion(2,5);
ChEyeOpenLevel(2,0);
VoicePlay("B330A10200_02_020");
MsgDisp("Sassa","Hahaha!");
ChEye(1,2);
ChMouth(1,2);
ChMotion(1,0);
ChEyeOpenLevel(1,0);
VoicePlay("B330A10200_01_020");
MsgDisp("Kazama","Haa... you're so loud.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
VoicePlay("B330A10200_06_010");
MsgDisp("Himuro","...Maybe...");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0);
ChEyeOpenLevel(1,8);
VoicePlay("B330A10200_01_030");
MsgDisp("Kazama","What is it this time, Inori.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B330A10200_06_020");
MsgDisp("Himuro","Ryota-senpai likes aquariums because 
he can see his reflection 
in the water tank.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B330A10200_06_030");
MsgDisp("Himuro","Since he's a narcissist.");
ChEye(1,2);
ChMouth(1,1);
ChMotion(1,3);
VoicePlay("B330A10200_01_040");
MsgDisp("Kazama","Haa?");
ChEye(1,2);
ChMouth(1,2);
ChMotion(1,0);
ChEyeOpenLevel(1,0);
VoicePlay("B330A10200_01_050");
MsgDisp("Kazama","Wrong.
I just wanted to talk 
quietly and calmly.");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,3);
VoicePlay("B330A10200_02_030");
MsgDisp("Sassa","That's impossible.
You thought you could talk calmly
with the four of us here?");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B330A10200_06_040");
MsgDisp("Himuro","He's right.");
MsgDisp("主人公","(Yeah, that's right...
If everyone's here, no matter 
where we go It'll get lively...)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,7);
ChEyeOpenLevel(1,#1);
ChMouthOpenLevel(1,#1);
ChCheek(1,0);
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,7);
ChEyeOpenLevel(2,#1);
ChMouthOpenLevel(2,#1);
ChCheek(2,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,7);
ChEyeOpenLevel(6,#1);
ChMouthOpenLevel(6,#1);
ChCheek(6,0);
