MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
BGOpen("wf310",0);
BGMPlay("BGM_PLACE_MUSE_AQUA_GARDEN",0.01);
ChLayout(1);
MsgClose();
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,2,1);
ChEyeOpenLevel(6,0);
ScrFadeIn(0);
VoicePlay("B360603101_06_000");
MsgDisp("Himuro","We're finally alone.");
MsgDisp("主人公","｛Himuro＊＊｝?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,1);
VoicePlay("B360603101_06_010");
MsgDisp("Himuro","What? You don't want to be alone with me?");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,0);
ChEyeOpenLevel(6,0);
VoicePlay("B360603101_06_020");
MsgDisp("Himuro","I just want to get ahead of the others as
soon as I can.");
MsgDisp("主人公","Really?");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360603101_06_030");
MsgDisp("Himuro","Dense.");
MsgDisp("主人公","Ugh...");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
VoicePlay("B360603101_06_040");
MsgDisp("Himuro","It's torture seeing everyone be so
familiar with you.");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("B360603101_06_050");
MsgDisp("Himuro","I wanted to tell them not to get close.");
MsgDisp("主人公","Eh!");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,2);
VoicePlay("B360603101_06_060");
MsgDisp("Himuro","...But I didn't want to be the kind of guy
who can't read the room and says stuff
like that.");
MsgDisp("主人公","...｛Himuro＊＊｝, did you not enjoy it?");
ChEye(6,0);
ChMouth(6,1);
ChMotion(6,5);
VoicePlay("B360603101_06_070");
MsgDisp("Himuro","I didn't!");
MsgDisp("主人公","I see...");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360603101_06_080");
MsgDisp("Himuro","Well, overall it wasn't bad.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360603101_06_090");
MsgDisp("Himuro","You were there.
And all things considered, it wasn't so bad.
...But it was so noisy.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B360603101_06_100");
MsgDisp("Himuro","I prefer it when we're alone.");
MsgDisp("主人公","I see.");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,1);
VoicePlay("B360603101_06_110");
MsgDisp("Himuro","So I'm making up for lost time now.
Be prepared.");
MsgDisp("主人公","Hehe, okay!");
BGMStop();
MsgClose();
ScrFadeOut(0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ChCheek(6,0);
