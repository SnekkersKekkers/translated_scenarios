ChLayout(1);
BGOpen("ex280",1);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ScrFadeIn(0);
MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
BGMPlay("BGM_C06_INORI_B",0.01);
Wait(60,0);
VoicePlay("B070601000_06_000");
MsgDisp("Himuro","Hey.");
MsgDisp("主人公","What is it?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B070601000_06_010");
MsgDisp("Himuro","...Before, 
you asked me what I thought about love?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B070601000_06_020");
MsgDisp("Himuro","What was that?");
MsgDisp("主人公","Eh?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B070601000_06_030");
MsgDisp("Himuro","What would have been the 
right answer that time?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
VoicePlay("B070601000_06_040");
MsgDisp("Himuro","When I was suddenly asked that question, 
my mind went blank and I answered 
that question on the spur of the moment...");
ChEye(6,2);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("B070601000_06_050");
MsgDisp("Himuro","Ever since then, I've been 
thinking back on that exchange
many times and regretting
it over and over again.");
MsgDisp("主人公","Ehh, really?");
ChEye(6,2);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B070601000_06_060");
MsgDisp("Himuro","Really you say...
Did it matter that little to you...?");
ChEye(6,2);
ChMouth(6,4);
ChMotion(6,4);
VoicePlay("B070601000_06_070");
MsgDisp("Himuro","Haa... looks like it was stupid of me
to worry about it for that long.");
MsgDisp("主人公","T-that's not what I——");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("B070601000_06_080");
MsgDisp("Himuro","Shut up.");
ChEye(6,2);
ChMouth(6,2);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("B070601000_06_090");
MsgDisp("Himuro","That's enough, don't
stir things up anymore.");
MsgDisp("主人公","｛氷室＊＊｝...");
BGMStop();
MsgClose();
ScrFadeOut(0);
BGOpen("ho000",2);
MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
Wait(40,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ScrFadeIn(0);
MsgDisp("主人公","Thanks for walking me home.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B070601000_06_100");
MsgDisp("Himuro","Yeah.");
MsgDisp("主人公","｛氷室＊＊｝, I--");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B070601000_06_110");
MsgDisp("Himuro","It's fine. I'm alright/
I just need to cool my
head for a bit.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B070601000_06_120");
MsgDisp("Himuro","Then, see you.");
MsgClose();
SEPlay("EV_SE_FOOT_WALK_AWAY_ALONE");
ChClose(6);
MsgDisp("主人公","(｛氷室＊＊｝...");
MsgClose();
ScrFadeOut(0);
