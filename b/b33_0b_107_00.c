ChClose(1,0,0);
ChClose(3,0,0);
ChClose(4,0,0);
BGOpen("tr300",0);
ScrFadeIn(0);
Wait(40,1);
ScrFadeOut(1,0,40);
BGOpen("tr310",0);
BGMPlay("BGM_PLACE_CASTLE",0.01);
ChLayout(2);
ChOpen(1,253,0,0,0,#1,#1,0,3);
ChOpen(3,253,0,0,0,#1,#1,0,3);
ChNanaType(0);
ChOpen(4,253,0,1,2,#1,#1,0,3);
ScrFadeIn(0);
ChMotion(4,1,1);
VoicePlay("B330B10700_04_000");
MsgDisp("Nanatsumori","...So.
What should we do here?");
ChEye(3,2);
ChMouth(3,4);
ChMotion(3,4,1);
VoicePlay("B330B10700_03_000");
MsgDisp("Honda","Mi-kun, you look discontent.");
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0);
VoicePlay("B330B10700_01_000");
MsgDisp("Kazama","Nanatsumori, the exhibition corner is
nice, you know?");
ChEye(4,2);
ChMouth(4,1);
ChMotion(4,0,1);
VoicePlay("B330B10700_04_010");
MsgDisp("Nanatsumori","Which part?");
ChEye(3,0);
ChMouth(3,4);
ChMotion(3,0,1);
ChEye(1,4);
ChMouth(1,3);
ChMotion(1,3);
VoicePlay("B330B10700_01_010");
MsgDisp("Kazama","Just look.
Things from hundreds of years ago are
right before our eyes.");
ChEye(4,2);
ChMouth(4,0);
ChMotion(4,2,1);
VoicePlay("B330B10700_04_020");
MsgDisp("Nanatsumori","Haa.");
ChEye(3,0);
ChMouth(3,3);
ChMotion(3,1,1);
VoicePlay("B330B10700_03_010");
MsgDisp("Honda","Right right.
Most of the items here are masterpieces
made by artisans from that era.");
ChEye(4,2);
ChMouth(4,4);
ChMotion(4,0,1);
VoicePlay("B330B10700_04_030");
MsgDisp("Nanatsumori","I know that.");
ChEye(1,4);
ChMouth(1,0);
ChMotion(1,0,1);
ChEyeOpenLevel(1,0);
VoicePlay("B330B10700_01_020");
MsgDisp("Kazama","There's even something connected to my
family...");
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,3,1);
VoicePlay("B330B10700_03_020");
MsgDisp("Honda","Oh that! The folding screen that was
directly bestowed to a Kazama family
ancestor by the Lord of Habataki Castle.");
MsgDisp("主人公","Even though it's so old, it's beautiful
even now.");
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,0,1);
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,3);
VoicePlay("B330B10700_01_030");
MsgDisp("Kazama","Of course.
It was kept carefully in our storehouse.");
ChEye(4,2);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("B330B10700_04_040");
MsgDisp("Nanatsumori","Yeah yeah.
I got it, Young Master.");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0);
ChEyeOpenLevel(1,8);
VoicePlay("B330B10700_01_040");
MsgDisp("Kazama","Stop calling me that.");
ChEye(3,0);
ChMouth(3,3);
ChMotion(3,4,1);
VoicePlay("B330B10700_03_030");
MsgDisp("Honda","The old lady in my neighborhood also calls
Ryo-kun young master.");
ChEye(1,0);
ChMouth(1,4);
ChMotion(1,1);
ChEyeOpenLevel(1,8);
VoicePlay("B330B10700_01_050");
MsgDisp("Kazama","It's fine for the elderly to do it.
Let's move onto the next one.");
MsgDisp("主人公","(｛Kazama＊＊｝, are you feeling a bit
embarrassed?)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,7);
ChEyeOpenLevel(1,#1);
ChMouthOpenLevel(1,#1);
ChCheek(1,0);
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,7);
ChEyeOpenLevel(3,#1);
ChMouthOpenLevel(3,#1);
ChCheek(3,0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,7);
ChEyeOpenLevel(4,#1);
ChMouthOpenLevel(4,#1);
ChCheek(4,0);
