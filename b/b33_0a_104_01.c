...
ChMotion(6,0);
ScrFadeIn(0);
MsgDisp("主人公","The firefly habitat is good 
...
...But, wouldn't things get 
interesting if that rumor was true?");
ChEye(2,0);
ChMouth(2,3);
ChMotion(2,2);
...
Heeh, how surprising.
Inori you seem like the type to 
make fun of that sort of stuff.");
ChEye(1,4);
ChMouth(1,3);
ChMotion(1,0);
ChEyeOpenLevel(1,0);
VoicePlay("B330A10401_01_010");
MsgDisp("Kazama","Inori is the type to have
lots of hidden voices.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
...
But he's also the type to normally 
let out his hidden voices.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,1);
VoicePlay("B330A10401_06_020");
MsgDisp("Himuro","Hm, that's good isn't it? It's honesty.");
...
As for who appears interesting...
...
MsgDisp("Kazama","...Even I've not really 
gotten anything to hide.");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0);
...
...Geez, I'm not supposed 
to be type to get teased.");
MsgClose();
SEPlay("EV_SE_626");
ChClose(1,0,30);
MsgDisp("主人公","(｛風真＊＊｝ is 
just well-loved by everyone huh?)");
MsgClose();
ScrFadeOut(0,0);
ChLayout(2);
