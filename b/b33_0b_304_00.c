BGOpen("fp200",0);
ChLayout(2);
ChNanaType(0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0,1);
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,0,1);
ScrFadeIn(0);
MsgDisp("主人公","Huh? There's a lot of people over there.");
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
VoicePlay("B330B30400_04_000");
MsgDisp("Nanatsumori","Ah, maybe they're shooting something.");
MsgDisp("主人公","Shooting?
Like a model photoshoot?");
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("B330B30400_04_010");
MsgDisp("Nanatsumori","Right.
This is a good location all year round.");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0,1);
VoicePlay("B330B30400_01_000");
MsgDisp("Kazama","Ehhh. Is that woman the model?");
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,4,1);
VoicePlay("B330B30400_03_000");
MsgDisp("Honda","She's pretty.
I'm sure she is.");
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
VoicePlay("B330B30400_04_020");
MsgDisp("Nanatsumori","That's a stylist.
She's from our agency.");
ChEye(1,0);
ChMouth(1,2);
ChEyeOpenLevel(1,8);
MsgDisp("主人公","Eh?");
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,2,1);
VoicePlay("B330B30400_04_030");
MsgDisp("Nanatsumori","The model is the one sitting in
that chair.");
ChEyeOpenLevel(1,0);
Wait(8,0);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0,1);
VoicePlay("B330B30400_01_010");
MsgDisp("Kazama","A kid?");
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("B330B30400_04_040");
MsgDisp("Nanatsumori","Kids photos will look nice here right?");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0);
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,3,1);
VoicePlay("B330B30400_03_010");
MsgDisp("Honda","Ah, I see!
Children love nature!");
ChEye(4,2);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("B330B30400_04_050");
MsgDisp("Nanatsumori","...I kinda get the feeling Dahon means
something different when he says kids
love nature.");
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("B330B30400_04_060");
MsgDisp("Nanatsumori","But, the fact that there's a stylist here
means they're putting a lot of effort into
today's shoot.");
MsgDisp("主人公","(Does that mean that woman is an
incredible stylist?)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,7);
ChEyeOpenLevel(1,#1);
ChMouthOpenLevel(1,#1);
ChCheek(1,0);
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,7);
ChEyeOpenLevel(3,#1);
ChMouthOpenLevel(3,#1);
ChCheek(3,0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,7);
ChEyeOpenLevel(4,#1);
ChMouthOpenLevel(4,#1);
ChCheek(4,0);
