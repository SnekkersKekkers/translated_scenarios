ChEye(2,0);
ChMouth(2,0);
ChMotion(2,0,1);
VoicePlay("B020202000_02_000");
MsgDisp("Sassa","Uhh, that person is...");
MsgDisp("主人公","Yeah, my Hazuki Kei T-Shirt.
Do you know him?");
ChEye(2,0);
ChMouth(2,2);
ChMotion(2,4);
VoicePlay("B020202000_02_010");
MsgDisp("Sassa","Hmm, I feel like I've seen him somewhere?");
MsgDisp("主人公","He's a famous model and alumni of Habataki
High.");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0);
VoicePlay("B020202000_02_020");
MsgDisp("Sassa","I see.
I'm sure I've seen him in magazines.
He's our senior, then.");
ChEye(2,0);
ChMouth(2,2);
ChMotion(2,4);
ChEyeOpenLevel(2,7);
VoicePlay("B020202000_02_030");
MsgDisp("Sassa","But he sure has a strong gaze...");
MsgDisp("主人公","(｛Sassa＊＊｝ seems to be a little
troubled?)");
