BGOpen("ne320",0);
BGMPlay("BGM_PLACE_BALLING_DARTS",0.01);
ChLayout(2);
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,7);
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0,1);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0,1);
ScrFadeIn(0);
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,1);
VoicePlay("B330C20501_06_000");
MsgDisp("Himuro","Then, let's have a four-person contest.");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("B330C20501_05_000");
MsgDisp("Hiiragi","I accept your challenge.");
MsgDisp("主人公","Yeah, let's do it.");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,5,1);
VoicePlay("B330C20501_07_000");
MsgDisp("Mikage","Sounds good.
Then, if you beat me I'll give you a
\"Habataki Mixed juice\" as a gift.");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
SEPlay("EV_SE_086");
Wait(15,1);
SEPlay("EV_SE_869");
SEPlay("EV_SE_061",0.5);
SEWait();
ChClose(6,0,0);
ChClose(5,0,0);
ChClose(7,0,0);
MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
BGOpen("ne300",0);
BGMPlay("BGM_PLACE_SKI_SKAT",0.01);
ChLayout(1);
MsgClose();
ChOpen(6,255,0,0,4,#1,#1,0,3,0,30);
ChOpen(5,255,0,0,4,#1,#1,0,3,0,30);
ScrFadeIn(0);
VoicePlay("B330C20501_05_010");
MsgDisp("Hiiragi","Mikage sensei was last place.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B330C20501_06_010");
MsgDisp("Himuro","I wonder if he let us win...
what do you think?");
MsgDisp("主人公","I think ｛御影＊＊｝ was serious...");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0,1);
VoicePlay("B330C20501_05_020");
MsgDisp("Hiiragi","If that bitterness of his was an act, I'll
let him join t he theater troupe.");
MsgDisp("主人公","Hehe.");
VoicePlay("B330C20501_07_010");
MsgDisp("Mikage","O-i, I bought it.");
MsgClose();
ChClose(5,0,30);
ChClose(6,0,30);
ChLayout(2);
MsgClose();
SEPlay("EV_SE_FOOT_WALK_COME_ALONE",0,0.9);
ChOpen(6,255,7,0,4,#1,#1,0,3,0,30);
ChOpen(5,255,0,0,4,#1,#1,0,3,0,30);
ChOpen(7,255,5,0,0,#1,#1,0,3);
VoicePlay("B330C20501_07_020");
MsgDisp("Mikage","Here you go, 
Habataki mixed juice.");
SEPlay("EV_SE_823",0,1);
SEWait();
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("B330C20501_05_030");
MsgDisp("Hiiragi","Thank you very much.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B330C20501_06_020");
MsgDisp("Himuro","Thank you.");
MsgDisp("主人公","Looks good!
Thank you.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,4);
VoicePlay("B330C20501_06_030");
MsgDisp("Himuro","We were talking about whether
Kojiro-sensei purposefully let us steal
the thunder.");
ChEye(7,0);
ChMouth(7,4);
ChMotion(7,2);
VoicePlay("B330C20501_07_030");
MsgDisp("Mikage","Haa?
I wouldn't do something so boring.");
ChEye(5,0);
ChMouth(5,2);
ChMotion(5,4);
MsgDisp("主人公","Eh?");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,1);
VoicePlay("B330C20501_07_040");
MsgDisp("Mikage","Let's have one last tearful game; a
bowling match.
I definitely won't lose?");
ChEye(5,4);
ChMouth(5,4);
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,5);
VoicePlay("B330C20501_06_040");
MsgDisp("Himuro","Pfft... sounds good. Not going easy on
each other, and keep going until we win.
Bring it on.");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("B330C20501_05_040");
MsgDisp("Hiiragi","Sounds good.");
MsgDisp("主人公","(I can understand why ｛御影＊＊｝ is
popular with the male students too... Both
｛柊＊＊＊｝ and ｛氷室＊＊｝ look like
they're having fun...)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(5,0);
ChMouth(5,0);
ChMotion(5,7);
ChEyeOpenLevel(5,#1);
ChMouthOpenLevel(5,#1);
ChCheek(5,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,7);
ChEyeOpenLevel(6,#1);
ChMouthOpenLevel(6,#1);
ChCheek(6,0);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,7);
ChEyeOpenLevel(7,#1);
ChMouthOpenLevel(7,#1);
ChCheek(7,0);
