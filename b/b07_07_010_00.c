BGOpen("ex280",1);
ChLayout(1);
ChEye(7,0);
ChMouth(7,4);
ChMotion(7,0,1);
ScrFadeIn(0);
MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
BGMPlay("BGM_C07_MIKAGE_B",0.01);
Wait(60,0);
ChEye(7,0);
ChMouth(7,4);
VoicePlay("B070701000_07_000");
MsgDisp("Mikage","Hey, you——");
MsgDisp("主人公","Yes, what is it?");
ChEye(7,0);
ChMouth(7,4);
ChMotion(7,3);
VoicePlay("B070701000_07_010");
MsgDisp("Mikage","No, 
You think about everything so seriously.");
ChEye(7,0);
ChMouth(7,0);
VoicePlay("B070701000_07_020");
MsgDisp("Mikage","That's fine though.
That's your charm.");
MsgDisp("主人公","Ummm...?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("B070701000_07_030");
MsgDisp("Mikage","But you know? 
The other day you asked me 
what I thought about love.");
MsgDisp("主人公","Ah, yeah.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,4);
VoicePlay("B070701000_07_040");
MsgDisp("Mikage","Thanking about things seriously, 
and coming up with an answer isn't 
the only solution.——");
ChEye(7,0);
ChMouth(7,0);
ChEyeOpenLevel(7,8);
VoicePlay("B070701000_07_050");
MsgDisp("Mikage","...That's how it goes sometimes.
Especially in love.");
MsgDisp("主人公","｛御影＊＊｝...?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
ChEyeOpenLevel(7,10);
VoicePlay("B070701000_07_060");
MsgDisp("Mikage","Love is something that suddenly grows 
bigger, gets heavier, and gets hotter?
It changes in an instant.");
MsgDisp("主人公","...What do you mean by that?");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,0);
VoicePlay("B070701000_07_070");
MsgDisp("Mikage","That's why, you shouldn't take your 
feelings in this moment so seriously.
Right?");
ChEye(7,2);
ChMouth(7,0);
ChEyeOpenLevel(7,0);
VoicePlay("B070701000_07_080");
MsgDisp("Mikage","Even I, if I were told to think 
about romance right now, would end 
up making you the subject.");
MsgDisp("主人公","Eh∋");
ChEye(7,2);
ChMouth(7,3);
ChMotion(7,2);
VoicePlay("B070701000_07_090");
MsgDisp("Mikage","Dangerous, right?");
MsgDisp("主人公","…………");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("B070701000_07_100");
MsgDisp("Mikage","So, you shouldn't think about 
it so seriously right?");
MsgDisp("主人公","Yes...");
MsgDisp("主人公","(It was weighing on ｛御影＊＊｝'s mind
that I had asked him about love before...)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChClose(7,0,0);
