BGOpen("ne100",0);
ChLayout(2);
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0,1);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0,1);
ScrFadeIn(0);
VoicePlay("B330C10500_06_000");
MsgDisp("Himuro","Yanosuke-senpai.
Is there something you wanted to buy?");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("B330C10500_05_000");
MsgDisp("Hiiragi","No, it's fun just hanging out with
everyone.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("B330C10500_07_000");
MsgDisp("Mikage","Speaking of, we need a theme...");
MsgDisp("主人公","How about buying things the theatre
troupe needs?");
ChEye(5,0);
ChMouth(5,2);
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,3);
VoicePlay("B330C10500_07_010");
MsgDisp("Mikage","That's nice isn't it?
Seems fun.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,4);
VoicePlay("B330C10500_06_010");
MsgDisp("Himuro","Yeah.
Seems like it would be helpful for
Yanosuke-senpai too.");
ChEye(5,2);
ChMouth(5,4);
ChMotion(5,4);
ChEyeOpenLevel(5,9);
VoicePlay("B330C10500_05_010");
MsgDisp("Hiiragi","Is that alright?
Even though the four of us were finally
able to go out together?");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,3);
VoicePlay("B330C10500_07_020");
MsgDisp("Mikage","Of course it's fine.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,3);
VoicePlay("B330C10500_06_020");
MsgDisp("Himuro","But what would you need for the theatre
troupe...stuff like props?");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("B330C10500_05_020");
MsgDisp("Hiiragi","There's someone who specialises in that.
I buy everyday items.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("B330C10500_07_030");
MsgDisp("Mikage","Being chairman is tough huh.
Today I'll do anything, even carry
your bags.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,1);
VoicePlay("B330C10500_06_030");
MsgDisp("Himuro","Then, is that supermarket okay?");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("B330C10500_05_030");
MsgDisp("Hiiragi","Yes. But is it really okay...?");
MsgDisp("主人公","Yeah, let's go.");
MsgClose();
ScrFadeOut(0,0);
MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
SEPlay("EV_SE_818",0,0.9);
Wait(50,0);
VoicePlay("B330C10500_36_000");
MsgDisp("Staff","Congratulations!
First prize: A year's supply of instant
noodles!");
SEStop("EV_SE_818",2);
SEPlay("EV_SE_511",0.3,0.6);
SEPlay("EV_SE_842",0.1,0.7);
Wait(30,0);
VoicePlay("B330C10500_05_040");
MsgDisp("Hiiragi","T-Thank you very much!");
VoicePlay("B330C10500_07_040");
MsgDisp("Mikage","That's great, Yanosuke!");
SEStop("EV_SE_511",1.5);
SEStop("EV_SE_842",1.5);
VoicePlay("B330C10500_06_040");
MsgDisp("Himuro","I-I want it...");
ChLayout(2);
ChEye(5,3);
ChMouth(5,4);
ChMotion(5,0);
ChPosition(5,3);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ChPosition(6,3);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
ChPosition(7,3);
ScrFadeIn(0);
MsgDisp("主人公","Amazing!");
VoicePlay("B330C10500_05_050");
MsgDisp("Hiiragi","I received some good fortune from
everyone.
I'll share it with you all.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,5);
VoicePlay("B330C10500_06_050");
MsgDisp("Himuro","Eh, is that okay?
Yanosuke-senpai, thank you very much.");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("B330C10500_05_060");
MsgDisp("Hiiragi","No, not even the theatre company can eat
this much.");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,3);
VoicePlay("B330C10500_07_050");
MsgDisp("Mikage","That's good, Inori.
You like cup noodles don't you.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,3);
VoicePlay("B330C10500_06_060");
MsgDisp("Himuro","I look forward to it.");
MsgDisp("主人公","But, you can't eat too much okay?");
ChEye(5,0);
ChMouth(5,2);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
ChEyeOpenLevel(6,8);
VoicePlay("B330C10500_06_070");
MsgDisp("Himuro","You always tell me that.");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("B330C10500_07_060");
MsgDisp("Mikage","Won't you say it to me too?");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0,1);
VoicePlay("B330C10500_05_070");
MsgDisp("Hiiragi","Me too, please.");
MsgDisp("主人公","Ehh∋
｛Hiiragi＊＊＊｝ and ｛Mikage＊＊｝, you guys
don't even like instant noodles, do you?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("B330C10500_07_070");
MsgDisp("Mikage","We just wanted you to fuss over us too.
Right, Yanosuke?");
ChEye(5,3);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("B330C10500_05_080");
MsgDisp("Hiiragi","Yes.
But, I shouldn't push my luck more than this.
I'll incur divine punishment.");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("B330C10500_05_090");
MsgDisp("Hiiragi","Today was a first-prize kind of day right
to the very end.");
MsgDisp("主人公","(Hehe, I'm glad.
Seeing ｛Hiiragi＊＊＊｝ enjoy himself makes me
super happy!)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(5,0);
ChMouth(5,0);
ChMotion(5,7);
ChEyeOpenLevel(5,#1);
ChMouthOpenLevel(5,#1);
ChCheek(5,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,7);
ChEyeOpenLevel(6,#1);
ChMouthOpenLevel(6,#1);
ChCheek(6,0);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,7);
ChEyeOpenLevel(7,#1);
ChMouthOpenLevel(7,#1);
ChCheek(7,0);
