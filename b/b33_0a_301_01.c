BGOpen("wf330",0);
BGMPlay("BGM_PLACE_AQUA_DEEP",0.01);
ChLayout(2);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0,1);
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,0,1);
ScrFadeIn(0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B330A30101_06_000");
MsgDisp("Himuro","The staging here is really well done.");
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,4);
ChEyeOpenLevel(2,7);
VoicePlay("B330A30101_02_000");
MsgDisp("Sassa","It's a bit overdone...
I can't concentrate on the fish.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,4);
VoicePlay("B330A30101_06_010");
MsgDisp("Himuro","...I see.
To Nozomu-senpai, this place is similar to
a haunted house.");
ChMouth(1,2);
MsgDisp("主人公","Eh...a haunted house...if you say it
like that...");
ChEye(1,2);
ChMouth(1,0);
ChMotion(1,1);
ChEyeOpenLevel(1,0);
VoicePlay("B330A30101_01_000");
MsgDisp("Kazama","Inori, stop.
There's another coward here.");
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,0);
VoicePlay("B330A30101_02_010");
MsgDisp("Sassa","I'm not a coward, I'm just delicate.
Right?");
ChEye(1,2);
ChMouth(1,0);
ChMotion(1,0);
MsgDisp("主人公","Y-Yeah.");
ChEye(2,2);
ChMouth(2,0);
ChMotion(2,2);
VoicePlay("B330A30101_02_020");
MsgDisp("Sassa","Because Inori is saying weird things,
that boat is starting to look like a
phantom ship.");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0);
VoicePlay("B330A30101_01_010");
MsgDisp("Kazama","Hm? Isn't it just a sunken ship?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
ChEyeOpenLevel(6,8);
VoicePlay("B330A30101_06_020");
MsgDisp("Himuro","Yes.
It is, but if it sank the crew must be...");
ChEyeOpenLevel(1,8);
ChEye(2,2);
ChMouth(2,0);
ChMotion(2,0);
VoicePlay("B330A30101_02_030");
MsgDisp("Sassa","Ughh... S-Stop it.");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,0);
VoicePlay("B330A30101_06_030");
MsgDisp("Himuro","Heheh, sorry.");
ChEye(1,2);
ChMouth(1,0);
ChEyeOpenLevel(1,0);
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,0);
VoicePlay("B330A30101_06_040");
MsgDisp("Himuro","Even though Nozomu-senpai and Ryota-senpai
are superhuman, it's nice that their weak
points are so easy to understand.");
ChEye(1,0);
ChMouth(1,2);
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,4);
VoicePlay("B330A30101_02_040");
MsgDisp("Sassa","Huh?");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,3);
VoicePlay("B330A30101_06_050");
MsgDisp("Himuro","That's why people like them.");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,1);
VoicePlay("B330A30101_01_020");
MsgDisp("Kazama","I don't have something like that.");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,0);
VoicePlay("B330A30101_06_060");
MsgDisp("Himuro","Your weakness is the easiest to
understand...
Right?");
MsgDisp("主人公","Eh? Me...?");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0);
ChEyeOpenLevel(1,8);
VoicePlay("B330A30101_01_030");
MsgDisp("Kazama","Ugh...
Today's pace was completely set by Inori.");
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,2);
ChEyeOpenLevel(2,7);
VoicePlay("B330A30101_02_050");
MsgDisp("Sassa","Yeah.
Inori's affinity with the deep sea corner
is definitely good.");
MsgDisp("主人公","(｛Himuro＊＊｝, maybe you're overwhelming
those two more than usual today?)");
BGMStop();
MsgClose();
ScrFadeOut(0);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,7);
ChEyeOpenLevel(1,#1);
ChMouthOpenLevel(1,#1);
ChCheek(1,0);
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,7);
ChEyeOpenLevel(2,#1);
ChMouthOpenLevel(2,#1);
ChCheek(2,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,7);
ChEyeOpenLevel(6,#1);
ChMouthOpenLevel(6,#1);
ChCheek(6,0);
