MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
BGOpen("ex300",2);
BGMPlay("BGM_XMAS_TRHEE",0.01);
ScrFadeIn(0);
MsgDisp("主人公","Waa...pretty!
It's a blue illumination.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0,1);
VoicePlay("B140600000_06_000");
MsgDisp("Himuro","Yeah, not bad.
It reminds me of the sea.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B140600000_06_010");
MsgDisp("Himuro","...It's strange.");
MsgDisp("主人公","Eh?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B140600000_06_020");
MsgDisp("Himuro","Why have we been together for two days
straight?");
MsgDisp("主人公","Ah, I see!
It's because yesterday was the school
christmas party.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,2);
VoicePlay("B140600000_06_030");
MsgDisp("Himuro","Right.
We met yesterday, and today too...");
MsgDisp("主人公","Do you dislike it...?");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,1);
VoicePlay("B140600000_06_040");
MsgDisp("Himuro","If that was the case, I wouldn't have come.
Rather...
I'm having fun.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B140600000_06_050");
MsgDisp("Himuro","The illuminations are nice, but what I really
enjoy is seeing you watching and enjoying
them.");
MsgDisp("主人公","Umm...thank you.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B140600000_06_060");
MsgDisp("Himuro","Having said all that, well, Merry Christmas
right?");
MsgDisp("主人公","Heheh, yeah!
｛Himuro＊＊｝, Merry Christmas.");
MsgDisp("主人公","(I see, both today and yesterday, I've been
with ｛Himuro＊＊｝.
My heart is racing a bit...)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
