BGOpen("wf330",0);
BGMPlay("BGM_PLACE_AQUA_DEEP",0.01);
ChLayout(2);
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,4,1);
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0,1);
ChNanaType(0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,1,1);
ScrFadeIn(0);
VoicePlay("B330B20100_03_000");
MsgDisp("Honda","She's beautiful...");
ChEye(4,5);
ChMouth(4,0);
ChMotion(4,0,1);
VoicePlay("B330B20100_04_000");
MsgDisp("Nanatsumori","Eh∋
That's not like you to say that, Dahon.");
ChEye(1,2);
ChMouth(1,3);
ChMotion(1,0);
ChEyeOpenLevel(1,0);
VoicePlay("B330B20100_01_000");
MsgDisp("Kazama","Look closely.
Dahon's line of sight is...");
ChMotion(4,1,1);
MsgDisp("主人公","Looking at the display case right?");
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,3,1);
ChEyeOpenLevel(3,0);
VoicePlay("B330B20100_03_010");
MsgDisp("Honda","A frilled shark!
They call them living fossils.
It's super cute, huh!");
ChEye(4,2);
ChMouth(4,2);
ChMotion(4,2,1);
VoicePlay("B330B20100_04_010");
MsgDisp("Nanatsumori","Dahon's standards for beauty
and cuteness is too weird...");
ChEye(1,2);
ChMouth(1,3);
ChMotion(1,0,1);
VoicePlay("B330B20100_01_010");
MsgDisp("Kazama","If I gave it the benefit
of the doubt, would I think it's cute?
No, I wouldn't.");
MsgDisp("主人公","Hehe!
｛本多＊＊｝, are you satisified?");
ChEye(3,0);
ChMouth(3,3);
ChMotion(3,1,1);
VoicePlay("B330B20100_03_020");
MsgDisp("Honda","I'm ver satisfied, this is wonderful!");
ChMouth(1,0);
ChMotion(1,1);
ChEyeOpenLevel(1,0);
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,0,1);
VoicePlay("B330B20100_03_030");
MsgDisp("Honda","Next, let's go see the deep
sea corner's popular giant isopod.");
ChEye(4,5);
ChMouth(4,1);
ChMotion(4,0,1);
VoicePlay("B330B20100_04_020");
MsgDisp("Nanatsumori","Giant isopod...");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,1);
ChEyeOpenLevel(1,8);
VoicePlay("B330B20100_01_020");
MsgDisp("Kazama","Nanatsumori might 
faint when he sees it.");
ChEye(4,2);
ChMouth(4,1);
ChMotion(4,5,1);
VoicePlay("B330B20100_04_030");
MsgDisp("Nanatsumori","∋");
ChEye(3,0);
ChMouth(3,3);
ChMotion(3,1,1);
VoicePlay("B330B20100_03_040");
MsgDisp("Honda","It's alright!
It's super cool. Mii-kun will
also definitely come to like it.");
ChEye(1,2);
ChEyeOpenLevel(1,0);
MsgDisp("主人公","(I'm pretty sure it'll b
e a no-go for ｛七ツ森＊｝ ...");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,7);
ChEyeOpenLevel(1,-1);
ChMouthOpenLevel(1,-1);
ChCheek(1,0);
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,7);
ChEyeOpenLevel(3,-1);
ChMouthOpenLevel(3,-1);
ChCheek(3,0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,7);
ChEyeOpenLevel(4,-1);
ChMouthOpenLevel(4,-1);
ChCheek(4,0);
