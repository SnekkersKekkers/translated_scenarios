BGOpen("wf100",0);
BGMPlay("BGM_PLACE_SHOP",0.01);
ChLayout(2);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0,1);
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0,1);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
ScrFadeIn(0);
VoicePlay("B330C30100_07_000");
MsgDisp("Mikage","This place is massive.
Just how many stores are there?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B330C30100_06_000");
MsgDisp("Himuro","According to the guide, more than 100.");
MsgDisp("主人公","This is Habataki City's largest shopping
mall.");
ChEye(5,2);
ChMouth(5,4);
ChMotion(5,0);
ChEyeOpenLevel(5,9);
VoicePlay("B330C30100_05_000");
MsgDisp("Hiiragi","No matter how many times I come here, I
end up losing my sense of direction.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
VoicePlay("B330C30100_06_010");
MsgDisp("Himuro","Yanosuke-senpai, please don't get lost.");
ChEye(5,2);
ChMouth(5,4);
ChMotion(5,0,1);
ChEyeOpenLevel(5,0);
VoicePlay("B330C30100_05_010");
MsgDisp("Hiiragi","I can't promise I won't.");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,2);
VoicePlay("B330C30100_07_010");
MsgDisp("Mikage","Then shall we all hold hands while we
walk?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B330C30100_06_020");
MsgDisp("Himuro","Ha?");
MsgDisp("主人公","Eh?");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("B330C30100_05_020");
MsgDisp("Hiiragi","Yes.
How should we line up?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,2);
VoicePlay("B330C30100_06_030");
MsgDisp("Himuro","Standing order...");
ChEye(6,0);
ChMouth(6,2);
ChMotion(6,2);
VoicePlay("B330C30100_06_040");
MsgDisp("Himuro","Next to her...");
ChEye(5,4);
ChMouth(5,2);
ChMotion(5,4);
VoicePlay("B330C30100_05_030");
MsgDisp("Hiiragi","Someone will have a hard time.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("B330C30100_07_020");
MsgDisp("Mikage","Ohh, you're thinking about it huh～ Male
high school students, man! 4 people
walking side by side. So, how should we do
this?");
MsgDisp("主人公","We would be a nuisance if the four of us
walked side-by-side, so we can't do it.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ChEye(5,2);
ChMouth(5,4);
ChMotion(5,0);
ChEyeOpenLevel(5,9);
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("B330C30100_07_030");
MsgDisp("Mikage","Don't get so mad.
Since the four of us are together, I
wanted to make things more heart-pounding.");
MsgDisp("主人公","(Hey...but it's rare to see ｛Hiiragi＊＊＊｝
and ｛Himuro＊＊｝ both go along with it
today?)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(5,0);
ChMouth(5,0);
ChMotion(5,7);
ChEyeOpenLevel(5,#1);
ChMouthOpenLevel(5,#1);
ChCheek(5,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,7);
ChEyeOpenLevel(6,#1);
ChMouthOpenLevel(6,#1);
ChCheek(6,0);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,7);
ChEyeOpenLevel(7,#1);
ChMouthOpenLevel(7,#1);
ChCheek(7,0);
