ChClose(5,0,0);
ChClose(6,0,0);
ChClose(7,0,0);
BGOpen("tr500",0);
ScrFadeIn(0);
Wait(40,1);
ScrFadeOut(1,0,40);
BGOpen("tr520",0);
ChLayout(1);
ChOpen(6,255,0,0,0,#1,#1,0,3,0,30);
ChOpen(5,255,0,0,4,#1,#1,0,3,0,30);
ScrFadeIn(0);
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0);
ChEyeOpenLevel(5,0);
VoicePlay("B330C20801_05_000");
MsgDisp("Hiiragi","Haa, feels nice.
I'm glad Inori-kun decided on this place.");
MsgDisp("主人公","I see.
Does ｛氷室＊＊｝ like it here?");
ChEye(5,4);
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,4);
VoicePlay("B330C20801_06_000");
MsgDisp("Himuro","It's not just me.");
VoicePlay("B330C20801_07_000");
MsgDisp("Mikage","O-i!
Come here you guys.
There's a good spot!");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B330C20801_06_010");
MsgDisp("Himuro","...Right?");
MsgDisp("主人公","Hehe! Maybe ｛御影＊＊｝ is having the
most fun?");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0,1);
VoicePlay("B330C20801_05_010");
MsgDisp("Hiiragi","Even I like the atmosphere here.");
MsgDisp("主人公","Then, me too.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B330C20801_06_020");
MsgDisp("Himuro","There's no point 
if you don't like it.");
MsgClose();
ChClose(6,0,30);
ChClose(5,0,30);
ChLayout(2);
ChOpen(6,255,7,0,4,#1,#1,0,3,0,30);
SEPlay("EV_SE_625");
ChOpen(5,255,0,0,4,#1,#1,0,3,0,30);
MsgClose();
Wait(30,0);
ChOpen(7,255,0,0,4,#1,#1,0,3);
VoicePlay("B330C20801_07_010");
MsgDisp("Mikage","What's up?
We have to catch enough fish for everyone!");
MsgDisp("主人公","｛御影＊＊｝, we're fishing this time?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("B330C20801_07_020");
MsgDisp("Mikage","Char and trout were released this morning,
so let's catch and eat some.");
ChEye(5,4);
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,1);
VoicePlay("B330C20801_06_030");
MsgDisp("Himuro","We have to catch enough for everyone.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("B330C20801_07_030");
MsgDisp("Mikage","That's right.
I also bought some good bait.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,2);
ChEyeOpenLevel(7,8);
VoicePlay("B330C20801_07_040");
MsgDisp("Mikage","It's grape-worm chan.");
ChEye(5,0);
ChMouth(5,2);
MsgDisp("主人公","Grape worm...");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B330C20801_06_040");
MsgDisp("Himuro","It's a moth larvae.");
MsgDisp("主人公","Eh...");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,3);
VoicePlay("B330C20801_07_050");
MsgDisp("Mikage","Right, it works for catching fish, Put the
hook into its bottom, then out the head.");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("B330C20801_05_020");
MsgDisp("Hiiragi","Will it be easier to catch
them this way?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("B330C20801_07_060");
MsgDisp("Mikage","Yeah, if the needle is parallel to the
body, the needle will be hidden right?");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("B330C20801_05_030");
MsgDisp("Hiiragi","I see... interesting.");
MsgDisp("主人公","Y-Yeah...");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("B330C20801_07_070");
MsgDisp("Mikage","It might be too much for a girl.
Inori, Yanosuke, help me out.");
MsgDisp("主人公","(...That means we'll alsoeat the fish that
ate this bug... Yeah, let's not think
about it so much and just enjoy it.)");
MsgClose();
ScrFadeOut(0,0);
ChEye(5,0);
ChMouth(5,0);
ChMotion(5,7);
ChEyeOpenLevel(5,#1);
ChMouthOpenLevel(5,#1);
ChCheek(5,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,7);
ChEyeOpenLevel(6,#1);
ChMouthOpenLevel(6,#1);
ChCheek(6,0);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,7);
ChEyeOpenLevel(7,#1);
ChMouthOpenLevel(7,#1);
ChCheek(7,0);
