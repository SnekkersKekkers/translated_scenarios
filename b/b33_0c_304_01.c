BGOpen("fp000",0);
ChLayout(2);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0,1);
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0,1);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
ScrFadeIn(0);
VoicePlay("B330C30401_07_000");
MsgDisp("Mikage","So, what do you want to do today?");
ChMouth(5,2);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("B330C30401_06_000");
MsgDisp("Himuro","It was Kojiro-sensei who decided on
this place. You must've had 
some intention in mind right?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,4);
VoicePlay("B330C30401_07_010");
MsgDisp("Mikage","There's many things you can do here?
The botanical garden, the museum, 
the pool...you can also just relax.");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("B330C30401_05_000");
MsgDisp("Hiiragi","All the facilities sound appealing...");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("B330C30401_07_020");
MsgDisp("Mikage","Well, let's get going.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
VoicePlay("B330C30401_06_010");
MsgDisp("Himuro","It's pointless just 
standing around here.");
MsgClose();
ScrFadeOut(0,0);
MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
BGOpen("fp200",0);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,7);
ScrFadeIn(0);
VoicePlay("B330C30401_07_030");
MsgDisp("Mikage","Phew...
It's like I was guided here.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,1);
VoicePlay("B330C30401_06_020");
MsgDisp("Himuro","Drawn to a power spot...
Have you been weakened, Kojiro-sensei?");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("B330C30401_05_010");
MsgDisp("Hiiragi","That may be true.
Mikage-sensei, I've heard that the spring 
water here is the source of its
power. Please try touching it.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("B330C30401_06_030");
MsgDisp("Himuro","Maybe it would be better to
sprinkle some over his head?");
ChEye(7,0);
ChMouth(7,4);
ChMotion(7,1);
VoicePlay("B330C30401_07_040");
MsgDisp("Mikage","Hey, Inori.
Are you serious?");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("B330C30401_05_020");
MsgDisp("Hiiragi","That's right, Inori. Instead of
sprinkling, it would be more effective if
Mikage-sensei took it directly into his
body.");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("B330C30401_05_030");
MsgDisp("Hiiragi","Mikage-sensei, please drink it.");
MsgDisp("主人公","Ehhh∋");
ChEye(7,0);
ChMouth(7,4);
ChMotion(7,5);
VoicePlay("B330C30401_07_050");
MsgDisp("Mikage","Hey, Yanosuke!");
ChEye(7,2);
ChMouth(7,4);
ChMotion(7,2);
ChEye(5,3);
ChMouth(5,4);
ChMotion(5,4,1);
VoicePlay("B330C30401_05_040");
MsgDisp("Hiiragi","Hehe, it is a joke.");
ChEyeOpenLevel(5,#1);
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,3);
VoicePlay("B330C30401_06_040");
MsgDisp("Himuro","But, he does look more energised?");
MsgDisp("主人公","(Are these two actually worried about
｛御影＊＊｝ or are they just teasing
him....)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(5,0);
ChMouth(5,0);
ChMotion(5,7);
ChEyeOpenLevel(5,#1);
ChMouthOpenLevel(5,#1);
ChCheek(5,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,7);
ChEyeOpenLevel(6,#1);
ChMouthOpenLevel(6,#1);
ChCheek(6,0);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,7);
ChEyeOpenLevel(7,#1);
ChMouthOpenLevel(7,#1);
ChCheek(7,0);
