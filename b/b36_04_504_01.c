MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
BGOpen("ex300",2);
BGMPlay("BGM_XMAS_TRHEE",0.01);
ChLayout(1);
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,0,1);
ScrFadeIn(0);
VoicePlay("B360450401_04_000");
MsgDisp("Nanatsumori","It's really beautiful.");
MsgDisp("主人公","Yeah.
It's amazing...!");
VoicePlay("B360450401_04_010");
MsgDisp("Nanatsumori","I've known about the lights in Habataki City
since I lived in the neighboring town.
One day...");
ChEye(4,4);
ChMotion(4,1,1);
VoicePlay("B360450401_04_020");
MsgDisp("Nanatsumori","I always hoped I'd get to see them with a
girlfriend someday.");
MsgDisp("主人公","I see.");
VoicePlay("B360450401_04_030");
MsgDisp("Nanatsumori","Yeah.");
ChEye(4,2);
ChMouth(4,2);
VoicePlay("B360450401_04_040");
MsgDisp("Nanatsumori","...Huh?");
MsgDisp("主人公","Hm?");
ChMotion(4,0,1);
VoicePlay("B360450401_04_050");
MsgDisp("Nanatsumori","Are you just going to ignore that?");
MsgDisp("主人公","Eh?");
VoicePlay("B360450401_04_060");
MsgDisp("Nanatsumori","I thought I said something really shocking.");
MsgDisp("主人公","Umm...");
ChEye(4,2);
ChMouth(4,4);
ChMotion(4,2,1);
VoicePlay("B360450401_04_070");
MsgDisp("Nanatsumori","Whoa—...
Seems like it was casually brushed aside...");
ChEye(4,2);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("B360450401_04_080");
MsgDisp("Nanatsumori","Well, that's part of your charm.
I'll accept it.");
MsgDisp("主人公","?");
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("B360450401_04_090");
MsgDisp("Nanatsumori","Next time, I'll say it differently.
I'll make sure my meaning is clearer.");
MsgDisp("主人公","(???)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0);
ChCheek(4,0);
