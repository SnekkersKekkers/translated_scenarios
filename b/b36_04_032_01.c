MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
BGOpen("wf310",0);
BGMPlay("BGM_PLACE_MUSE_AQUA_GARDEN",0.01);
ChLayout(1);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
ScrFadeIn(0);
VoicePlay("B360403201_04_000");
MsgDisp("Nanatsumori","At this time of day,
there are more couples here.");
MsgDisp("主人公","Hmm...");
ChEye(4,2);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("B360403201_04_010");
MsgDisp("Nanatsumori","It's cool...");
MsgDisp("主人公","Eh...?
T-That's right.");
ChEye(4,0);
ChMouth(4,3);
VoicePlay("B360403201_04_020");
MsgDisp("Nanatsumori","It's different this time.
Interesting.");
MsgDisp("主人公","Ugh...");
ChEye(4,3);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("B360403201_04_030");
MsgDisp("Nanatsumori","Haha!
Your expressions change all the time,
I really never get bored of you.");
MsgDisp("主人公","Hey.");
ChEye(4,4);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("B360403201_04_040");
MsgDisp("Nanatsumori","Don't sulk.
I'm praising you.");
ChEye(4,0);
ChMouth(4,0);
VoicePlay("B360403201_04_050");
MsgDisp("Nanatsumori","Couples huh...
What do you think we are?");
MsgDisp("主人公","Eh?");
ChMotion(4,1,1);
VoicePlay("B360403201_04_060");
MsgDisp("Nanatsumori","A couple? Friends?
Something else?");
MsgDisp("主人公","｛七ツ森＊｝, 
are you messing with me?");
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("B360403201_04_070");
MsgDisp("Nanatsumori","You got itー.
Just watching your facial
expressions is fun.");
MsgDisp("主人公","Hey!
We came to the aquarium,
so go look at the fish?");
ChEye(4,3);
ChMouth(4,3);
VoicePlay("B360403201_04_080");
MsgDisp("Nanatsumori","You're right.
Then, let's continue to enjoy our date.");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0);
ChCheek(4,0);
