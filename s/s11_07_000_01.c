ChEyeOpenLevel(7,#1);
ChMouthOpenLevel(7,#1);
ChCheek(7,0);
SEPlay("EV_SE_836");
SEWait();
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,3);
VoicePlay("S110700001_07_000");
MsgDisp("Mikage","Ooh, Excellent Luck!");
MsgDisp("主人公","｛Mikage＊＊｝, that's amazing!");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("S110700001_07_010");
MsgDisp("Mikage","Well, it's because I'm spending New Years
with a real beauty.");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,2);
VoicePlay("S110700001_07_020");
MsgDisp("Mikage","What a great fortune indeed.");
MsgDisp("主人公","Hehe, I'm glad.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,4);
VoicePlay("S110700001_07_030");
MsgDisp("Mikage","I know. I guess I should go apologise to
Vice Principal Himuro before the Excellent
Luck wears off...");
MsgDisp("主人公","Ehh∋ Did you do something?");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("S110700001_07_040");
MsgDisp("Mikage","...Nope, not yet.");
MsgDisp("主人公","(His Excellent Luck will run out too
quickly...)");
