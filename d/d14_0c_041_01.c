ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0);
ChCheek(5,0);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
ChCheek(7,0);
VoicePlay("D140C04101_05_000");
MsgDisp("Hiiragi","Inori-kun, you said there was an 
event being held in your class.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("D140C04101_07_000");
MsgDisp("Mikage","Apparently there's someone transferring.
There's even a farewell lunch.");
ChMouth(5,2);
MsgDisp("主人公","Transferring huh?
That seems rather sad.");
ChEye(5,2);
ChMouth(5,4);
ChMotion(5,4);
ChEyeOpenLevel(5,0);
VoicePlay("D140C04101_05_010");
MsgDisp("Hiiragi","I've been transferring schools 
over and over since around
middle school. So, I've become
numb to that sort of feeling.");
MsgDisp("主人公","I see...");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("D140C04101_05_020");
MsgDisp("Hiiragi","But now, if I were to transfer
from Haba Academy it would be
completely different.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("D140C04101_07_010");
MsgDisp("Mikage","I'm happy that you 
think that Yanosuke.");
MsgDisp("主人公","Yes.");
ChEye(5,2);
ChMouth(5,4);
ChMotion(5,4);
ChEyeOpenLevel(5,9);
VoicePlay("D140C04101_05_030");
MsgDisp("Hiiragi","This is the first time I've 
attended the same school for this long. 
It was once common for me to 
transfer after a month.");
ChEye(7,0);
ChMouth(7,4);
ChMotion(7,5);
VoicePlay("D140C04101_07_020");
MsgDisp("Mikage","Hey, so it's an issue 
of time for you?");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("D140C04101_05_040");
MsgDisp("Hiiragi","Ah, no.
It's because of everyone here.
This is the first time I've 
had this sort of school life.");
MsgDisp("主人公","｛柊＊＊＊｝……");
ChEye(5,2);
ChMouth(5,4);
ChMotion(5,4);
ChEyeOpenLevel(5,0);
VoicePlay("D140C04101_05_050");
MsgDisp("Hiiragi","That's why it's scary.
For me to think about what it would 
be like if I had to leave Haba 
Academy like I did in the past.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("D140C04101_07_030");
MsgDisp("Mikage","It's alright.
You can stay at Haba Academy 
until your graduation.");
ChEye(5,4);
ChEye(7,3);
ChMouth(7,3);
ChMotion(7,1);
ChEyeOpenLevel(7,0);
VoicePlay("D140C04101_07_040");
MsgDisp("Mikage","If you can't graduate, 
you can stay as many 
years as you want?");
ChEye(5,2);
ChMouth(5,4);
ChMotion(5,2);
ChEyeOpenLevel(5,9);
VoicePlay("D140C04101_05_060");
MsgDisp("Hiiragi","Ahh, yes. That's right.
If I don't study hard now, 
that's yet another problem.");
MsgDisp("主人公","(Hehe.
｛柊＊＊＊｝, let's do our best together!)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChClose(5,0,0);
ChClose(7,0,0);
