ChEye(6,0);
ChMouth(6,0);
ChMotion(6,7);
ChCheek(6,0);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
ChCheek(7,0);
VoicePlay("D140C06000_06_000");
MsgDisp("Himuro","Looks like Yanosuke-senpai has work with the
theatre troupe.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("D140C06000_07_000");
MsgDisp("Mikage","Yeah.
He puts on a normal face around us, but he's
actually really busy.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("D140C06000_06_010");
MsgDisp("Himuro","Hey, is he like that even in front of you.");
MsgDisp("主人公","Eh?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("D140C06000_07_010");
MsgDisp("Mikage","Does he ever show his troubled face in front
of you?");
MsgDisp("主人公","Ummm...
I wonder.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,2);
VoicePlay("D140C06000_06_020");
MsgDisp("Himuro","I would hope so.
Otherwise, he's too much of a superman.");
MsgDisp("主人公","Ehhh?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("D140C06000_06_030");
MsgDisp("Himuro","Nothing...
Just talking to myself.");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,1);
VoicePlay("D140C06000_07_020");
MsgDisp("Mikage","Hmmm.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
ChEyeOpenLevel(6,8);
VoicePlay("D140C06000_06_040");
MsgDisp("Himuro","What is it.");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,3);
VoicePlay("D140C06000_07_030");
MsgDisp("Mikage","This is good isn't it?
I think there's still a chance to turn things
around.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("D140C06000_06_050");
MsgDisp("Himuro","......");
MsgDisp("主人公","(These two are saying that ｛Hiiragi＊＊＊｝ and I
get along really well, right?
But a turnaround...?)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChClose(6,0,0);
ChClose(7,0,0);
