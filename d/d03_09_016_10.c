BGOpen("sc710",0);
ChEye(9,3);
ChMouth(9,4);
ChMotion(9,5,1);
ChEyeOpenLevel(9,10);
ChMouthOpenLevel(9,5);
VoicePlay("D030901610_40_000");
MsgDisp("Male Student A","Yo, heroine!
Nice work!");
VoicePlay("D030901610_48_000");
MsgDisp("Female Student A","The play was really amazing～!
The story had me captivated!");
MsgDisp("主人公","Thank you!");
ChEye(9,0);
ChMouth(9,4);
ChMotion(9,0,1);
ChEyeOpenLevel(9,8);
ChMouthOpenLevel(9,0);
VoicePlay("D030901610_09_000");
MsgDisp("Kuya","…………");
VoicePlay("D030901610_41_020");
MsgDisp("Male Student B","Hey, let's take a picture with the
heroine!");
VoicePlay("D030901610_45_000");
MsgDisp("School Girl B","Nice idea!
It'll make a good memory.");
ChEye(9,3);
ChMouth(9,4);
ChMotion(9,0,1);
ChEyeOpenLevel(9,10);
VoicePlay("D030901610_40_040");
MsgDisp("Male Student A","I want a photo with just the two
of us!");
VoicePlay("D030901610_41_050");
MsgDisp("Male Student B","Me too!");
MsgDisp("主人公","W-Wait a minute......");
ChEye(9,0);
ChMouth(9,4);
ChMotion(9,4,1);
ChEyeOpenLevel(9,8);
VoicePlay("D030901610_09_010");
MsgDisp("Kuya","Let's go over here.");
MsgDisp("主人公","Eh?");
ChEye(9,3);
ChMouth(9,0);
ChMotion(9,0,1);
ChEyeOpenLevel(9,8);
VoicePlay("D030901610_09_020");
MsgDisp("Kuya","I'm your customer, aren't I?");
MsgDisp("主人公","Ah......True.");
ChEye(9,3);
ChMotion(9,4,1);
VoicePlay("D030901610_09_030");
MsgDisp("Kuya","Sorry, but can you guys
wait until later for a photo session?");
MsgDisp("主人公","See you all later......!");
ChEye(9,0);
ChMouth(9,4);
ChMotion(9,0,1);
ChEyeOpenLevel(9,8);
ChMouthOpenLevel(9,0);
VoicePlay("D030901610_09_040");
MsgDisp("Kuya","…………");
SEPlay("EV_SE_FOOT_WALK_AWAY_TWO");
MsgClose();
ScrFadeOut(0);
MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
ChEye(9,2);
ChMouth(9,4);
ChMotion(9,0,1);
ScrFadeIn(0);
ChMotion(9,2,1);
VoicePlay("D030901610_09_050");
MsgDisp("Kuya","How pathetic......");
MsgDisp("主人公","Eh, what's wrong?");
ChEye(9,2);
ChMouth(9,0);
ChMotion(9,1,1);
VoicePlay("D030901610_09_060");
MsgDisp("Kuya","My jealousy. I thought you were going 
to be taken away by that big group of 
students so I did something immature.");
MsgDisp("主人公","It's fine......
Even though you came, I wasn't able to
pay attention to you......");
ChEye(9,2);
ChMouth(9,0);
ChMotion(9,0,1);
VoicePlay("D030901610_09_070");
MsgDisp("Kuya","......Hehe.
Jealousy, huh.");
ChMotion(9,2,1);
VoicePlay("D030901610_09_080");
MsgDisp("Kuya","If I was a student at Haba High,
maybe I would feel jealous everyday.");
ChEye(9,3);
ChMouth(9,0);
ChEyeOpenLevel(9,8);
VoicePlay("D030901610_09_090");
MsgDisp("Kuya","......Yeah.
I think Hanegasaki High School was a good
decision.");
MsgDisp("主人公","???");
ChEye(9,3);
ChMouth(9,0);
ChMotion(9,4,1);
ChEyeOpenLevel(9,0);
VoicePlay("D030901610_09_100");
MsgDisp("Kuya","Don't worry about it.
I'm just talking to myself.");
MsgClose();
ScrFadeOut(0,0);
