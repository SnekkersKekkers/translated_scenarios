ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0);
ChCheek(5,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ChCheek(6,0);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
ChCheek(7,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,2);
VoicePlay("D140C05100_06_000");
MsgDisp("Himuro","…………");
ChEye(7,0);
ChMouth(7,4);
ChMotion(7,1);
VoicePlay("D140C05100_07_000");
MsgDisp("Mikage","…………");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("D140C05100_05_000");
MsgDisp("Hiiragi","What's up with you two?");
MsgDisp("主人公","Yeah, is there 
something bothering you?");
ChEye(7,0);
ChMouth(7,4);
ChMotion(7,1);
VoicePlay("D140C05100_07_010");
MsgDisp("Mikage","Inori is being considerate.");
ChMouth(5,2);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("D140C05100_06_010");
MsgDisp("Himuro","Phew... I conpletely forgot.
What were we talking about
until now?");
MsgDisp("主人公","Until now?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("D140C05100_06_020");
MsgDisp("Himuro","Before you and Yanosuke-senpai got 
so close.");
MsgDisp("主人公","Eh∋");
ChEye(5,3);
ChMotion(5,0);
VoicePlay("D140C05100_05_010");
MsgDisp("Hiiragi","Close?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,4);
VoicePlay("D140C05100_07_020");
MsgDisp("Mikage","I understand how Inori feels.
In times like this, isn't it 
best for the cozy pair to provide
us with a conversation topic?");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,3);
VoicePlay("D140C05100_06_030");
MsgDisp("Himuro","That's right.
Please, Yanosuke-senpai.
Give us your impressions on the date
you two went on.");
MsgDisp("主人公","Ehh..? ");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("D140C05100_05_020");
MsgDisp("Hiiragi","I understand.");
MsgDisp("主人公","Ehhh∋");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoiceEVSPlay(7);
VoicePlay("D140C05100_07_030");
MsgDisp("Mikage","｛主人公｝,
Why is it only you who 
is that surprised?");
ChEye(5,3);
ChMouth(5,2);
ChMotion(5,0);
VoicePlay("D140C05100_05_030");
MsgDisp("Hiiragi","Would it be wrong of me 
to tell them?");
MsgDisp("主人公","It's not wrong but...");
ChEye(5,4);
ChMouth(5,4);
ChEyeOpenLevel(5,0);
Wait(90,1);
ChMotion(5,2);
VoicePlay("D140C05100_05_040");
MsgDisp("Hiiragi","It was a clear day, and it felt 
nice. When you showed up at the 
meeting place, your clothes were 
more mature than usual——");
MsgDisp("主人公","∋");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("D140C05100_07_040");
MsgDisp("Mikage","Heehー, as expected of Yanosuke.
I can picture the scene.
Nice.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("D140C05100_06_040");
MsgDisp("Himuro","Yeah, and then?");
ChEye(5,3);
ChMouth(5,2);
ChMotion(5,5);
MsgDisp("主人公","Wait a second,｛柊＊＊＊｝!
You can't!");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
ChEyeOpenLevel(6,8);
VoicePlay("D140C05100_06_050");
MsgDisp("Himuro","Why?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("D140C05100_07_050");
MsgDisp("Mikage","Then, shall we continue 
this when you're not here?");
MsgDisp("主人公","You can't do that either!");
ChEye(5,2);
ChMouth(5,4);
ChMotion(5,4);
ChEyeOpenLevel(5,9);
VoicePlay("D140C05100_05_050");
MsgDisp("Hiiragi","It appears that I cannot.");
MsgDisp("主人公","(Haa, that caught me off guard.
What was ｛柊＊＊＊｝ saying so suddenly?)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChClose(5,0,0);
ChClose(6,0,0);
ChClose(7,0,0);
