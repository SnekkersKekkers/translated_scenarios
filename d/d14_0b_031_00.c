ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0,1);
ChEyeOpenLevel(1,8);
ChCheek(1,0);
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,0,1);
ChCheek(3,0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
ChCheek(4,0);
MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
VoicePlay("D140B03100_03_000");
MsgDisp("Honda","Yes.
That's why ramen is profound.");
ChEye(1,2);
ChMouth(1,1);
ChMotion(1,0);
ChEyeOpenLevel(1,0);
VoicePlay("D140B03100_01_000");
MsgDisp("Kazama","I got it already.");
ChEye(1,2);
ChMouth(1,2);
ChMotion(1,1);
VoicePlay("D140B03100_01_010");
MsgDisp("Kazama","That's why ramen is a dangerous topic with
Honda...");
MsgDisp("主人公","Hehe.
Wasn't ｛Honda＊＊｝'s lecture on ramen
interesting?");
ChMotion(4,0,1);
VoicePlay("D140B03100_04_000");
MsgDisp("Nanatsumori","I understand it was amazing.
...But, I don't eat ramen.");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0);
ChEye(3,0);
ChMouth(3,4);
ChMotion(3,4,1);
VoicePlay("D140B03100_03_010");
MsgDisp("Honda","Why?
How?
Tell me why you dislike it that much.");
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0);
VoicePlay("D140B03100_01_020");
MsgDisp("Kazama","I want to know too.
Why do you dislike it so much?");
ChMotion(4,2,1);
VoicePlay("D140B03100_04_010");
MsgDisp("Nanatsumori","It's not really that.
I don't hate ramen.
I ate it when I still lived at home.");
MsgDisp("主人公","Then is there another reason for it?");
ChEye(4,4);
ChMouth(4,4);
VoicePlay("D140B03100_04_020");
MsgDisp("Nanatsumori","When you slurp...");
ChEye(1,4);
ChMouth(1,2);
ChEye(3,4);
ChMouth(3,4);
ChMotion(3,0,1);
VoicePlay("D140B03100_03_020");
MsgDisp("Honda","When you slurp?");
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
VoicePlay("D140B03100_04_030");
MsgDisp("Nanatsumori","The broth scatters everywhere.");
ChEye(1,2);
ChMouth(1,2);
ChMotion(1,0);
VoicePlay("D140B03100_01_030");
MsgDisp("Kazama","...Yes?");
VoicePlay("D140B03100_04_040");
MsgDisp("Nanatsumori","In other words, I'm not good at slurping.");
ChEye(4,1);
ChMouth(4,1);
ChMotion(4,2,1);
VoicePlay("D140B03100_04_050");
MsgDisp("Nanatsumori","...Is that bad?");
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0,1);
ChEyeOpenLevel(1,8);
VoicePlay("D140B03100_01_040");
MsgDisp("Kazama","There's something cute about that.");
VoicePlay("D140B03100_04_060");
MsgDisp("Nanatsumori","...I can't eat ramen after all.
Never in front of other people.");
ChEye(1,0);
ChMouth(1,3);
ChEye(3,0);
ChMouth(3,3);
ChMotion(3,3,1);
VoicePlay("D140B03100_03_030");
MsgDisp("Honda","Mi-kun, I'll teach you how to slurp ramen
well.
Next time it's decided, we'll have ramen!");
ChEye(4,2);
ChMouth(4,0);
ChMotion(4,0,1);
VoicePlay("D140B03100_04_070");
MsgDisp("Nanatsumori","No thank you.");
MsgDisp("主人公","(That's for sure...
It's hard to eat ramen well...)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChClose(1,0,0);
ChClose(3,0,0);
ChClose(4,0,0);
