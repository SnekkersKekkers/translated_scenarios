ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
ChCheek(7,0);
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0);
ChCheek(5,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ChCheek(6,0);
ChMotion(5,4);
VoicePlay("D140C03400_05_000");
MsgDisp("Hiiragi","Mikage-sensei. You grow plants 
at school and at home, correct?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("D140C03400_07_000");
MsgDisp("Mikage","Yeah.
What's this all of a sudden, Yanosuke?");
VoicePlay("D140C03400_05_010");
MsgDisp("Hiiragi","You own pets right?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("D140C03400_06_000");
MsgDisp("Himuro","Now that you mention it, I don't think
I've heard anything on that.");
MsgDisp("主人公","Molly-chan lives at 
your parent's house, right?");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,1);
VoicePlay("D140C03400_07_010");
MsgDisp("Mikage","Hahaha, well, you can't 
keep a cow in an apartment.");
ChEye(5,0);
ChMouth(5,2);
ChMotion(5,4);
VoicePlay("D140C03400_05_020");
MsgDisp("Hiiragi","Could you not own a 
dogs or a cat?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("D140C03400_07_020");
MsgDisp("Mikage","Isn't it kind of sad for 
them to be alone all day?");
ChMouth(5,4);
MsgDisp("主人公","That's for sure...");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,2);
ChEyeOpenLevel(7,8);
VoicePlay("D140C03400_07_030");
MsgDisp("Mikage","And there's lots of them 
at school anyways, right?");
ChEye(5,3);
ChMouth(5,2);
MsgDisp("主人公","Eh?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
VoicePlay("D140C03400_06_010");
MsgDisp("Himuro","You mean the chickens
we keep at school?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("D140C03400_07_040");
MsgDisp("Mikage","Well, let's go with that.");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("D140C03400_05_030");
MsgDisp("Hiiragi","Heheh.
You mean us, right?");
MsgDisp("主人公","Eh∋");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,4);
VoicePlay("D140C03400_06_020");
MsgDisp("Himuro","Haa... to Kojiro-sensei,
we look like plants ans animals?");
ChMotion(5,4);
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,3);
VoicePlay("D140C03400_07_050");
MsgDisp("Mikage","Hahaha! Everyone's cute, lively and
steadily growing. Wouldn't you say you're
alike?");
MsgDisp("主人公","(I know he cares for us but... To be the
same as an animal or vegetable...)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChClose(5,0,0);
ChClose(6,0,0);
ChClose(7,0,0);
