ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0);
VoicePlay("D140A06001_01_000");
MsgDisp("Kazama","What is up with Sassa today?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("D140A06001_06_000");
MsgDisp("Himuro","Did something happen?");
MsgDisp("主人公","Eh, are you asking me?");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0,1);
ChEyeOpenLevel(1,8);
VoicePlay("D140A06001_01_010");
MsgDisp("Kazama","Who else would we be asking other than
you... geez.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("D140A06001_06_010");
MsgDisp("Himuro","You play dumb too much.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
VoicePlay("D140A06001_06_020");
MsgDisp("Himuro","We know that you and Nozomu-senpai are
quickly growing close.");
ChEye(1,4);
ChMouth(1,2);
ChMotion(1,1);
ChEyeOpenLevel(1,0);
VoicePlay("D140A06001_01_020");
MsgDisp("Kazama","...It's fine.
It's not good, but it's fine.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
VoicePlay("D140A06001_06_030");
MsgDisp("Himuro","What are you saying.");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0);
VoicePlay("D140A06001_01_030");
MsgDisp("Kazama","I'm not saying it's an especially bad
thing. That the two of them are going out,
I guess.");
MsgDisp("主人公","Umm...yeah.
Maybe we have been going out recently.");
ChEye(1,2);
ChMouth(1,2);
ChMotion(1,0,1);
ChEyeOpenLevel(1,0);
VoicePlay("D140A06001_01_040");
MsgDisp("Kazama","Haa... that's why we asked you ] if
anything had happened today.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("D140A06001_06_040");
MsgDisp("Himuro","It's rare for Nozomu-senpai to not come to
lunch.");
ChEye(1,0);
MsgDisp("主人公","Ah!
In that case...");
ChEye(1,2);
ChMouth(1,2);
ChMotion(1,0);
VoicePlay("D140A06001_01_050");
MsgDisp("Kazama","...What's up?");
MsgDisp("主人公","Maybe he has no pocket money...");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0,1);
ChEyeOpenLevel(1,8);
VoicePlay("D140A06001_01_060");
MsgDisp("Kazama","...Oi.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("D140A06001_06_050");
MsgDisp("Himuro","Now we want to return
our concern for him.");
ChEye(1,2);
ChMouth(1,2);
ChMotion(1,1);
ChEyeOpenLevel(1,0);
VoicePlay("D140A06001_01_070");
MsgDisp("Kazama","Tell him to spend his allowance more
wisely. ...Geez.");
MsgDisp("主人公","(I see, both of them are worried about me
and ｛颯砂＊＊｝... )");
BGMStop();
MsgClose();
ScrFadeOut(0);
ChClose(1,0,0);
ChClose(6,0,0);
