MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0,1);
VoicePlay("D140A06002_01_000");
MsgDisp("Kazama","Where's Inori today?");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0);
VoicePlay("D140A06002_02_000");
MsgDisp("Sassa","I guess there's lots of things going on
since he's in a different year?");
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0);
VoicePlay("D140A06002_01_010");
MsgDisp("Kazama","Well, if he wants to come, he'll be late.");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,4);
VoicePlay("D140A06002_02_010");
MsgDisp("Sassa","Do you think he's worried about what we
say about him when he's not around?");
MsgDisp("主人公","Hehe, I wonder if he worries about stuff
like that?");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("D140A06002_01_020");
MsgDisp("Kazama","He doesn't seem like he cares, but
he does.
That's what's cute about him.");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,4,1);
VoicePlay("D140A06002_02_020");
MsgDisp("Sassa","Maybe.
Then, do you like that part about him too?");
MsgDisp("主人公","Eh?");
ChEye(1,2);
ChMouth(1,2);
ChMotion(1,3);
VoicePlay("D140A06002_01_030");
MsgDisp("Kazama","What are you saying, Sassa?");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0);
VoicePlay("D140A06002_02_030");
MsgDisp("Sassa","It's fine.
Inori isn't here after all.
I wanted to hear it from you once.");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,4);
VoicePlay("D140A06002_02_040");
MsgDisp("Sassa","what you like about Inori.");
ChEye(1,2);
ChMouth(1,2);
ChMotion(1,1);
ChEyeOpenLevel(1,0);
VoicePlay("D140A06002_01_040");
MsgDisp("Kazama","Ugh, you do what you like.
I don't want to hear it.");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0);
VoicePlay("D140A06002_02_050");
MsgDisp("Sassa","Then, cover your ears.");
MsgDisp("主人公","I can't answer something like that so
suddenly...
I don't know.");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,1);
ChEyeOpenLevel(1,8);
VoicePlay("D140A06002_01_050");
MsgDisp("Kazama","If you say, \"I don't know\", then you can't
take any counter measures right?");
ChEye(2,0);
ChMouth(2,3);
ChMotion(2,4);
VoicePlay("D140A06002_02_060");
MsgDisp("Sassa","Ryota, you are listening right?");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0);
ChEyeOpenLevel(1,8);
VoicePlay("D140A06002_01_060");
MsgDisp("Kazama","Shut up.
So, how about it?");
MsgDisp("主人公","Ehhh? Hmm...");
ChEye(1,2);
ChMouth(1,1);
ChMotion(1,1);
ChEyeOpenLevel(1,0);
VoicePlay("D140A06002_01_070");
MsgDisp("Kazama","Ah, it's fine after all.");
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,4,1);
ChEyeOpenLevel(2,7);
VoicePlay("D140A06002_02_070");
MsgDisp("Sassa","What is?");
MsgDisp("主人公","(? I wonder what's up with ｛Kazama＊＊｝?)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChClose(1,0,0);
ChClose(2,0,0);
