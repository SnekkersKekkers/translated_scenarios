MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("D150006701_06_000");
MsgDisp("Himuro","What factors help you choose a menu?");
MsgDisp("主人公","Uhm.
I guess I just ask my stomach.");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,2);
VoicePlay("D150006701_07_000");
MsgDisp("Mikage","I have to consult my wallet too.
The food here is good so I always get the
large size.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,1);
VoicePlay("D150006701_06_010");
MsgDisp("Himuro","So you factor in your cravings and your
budget.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("D150006701_07_010");
MsgDisp("Mikage","How does Inori decide?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,2);
VoicePlay("D150006701_06_020");
MsgDisp("Himuro","I'm okay with just instant ramen so all of
these options make it hard to choose.");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,3);
VoicePlay("D150006701_07_020");
MsgDisp("Mikage","Instant Ramen can be good sometimes.
Recommend me one.");
ChEye(6,0);
ChMouth(6,5);
ChMotion(6,5);
VoicePlay("D150006701_06_030");
MsgDisp("Himuro","Eh?");
MsgDisp("主人公","I want to know too.");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,4);
VoicePlay("D150006701_06_040");
MsgDisp("Himuro","...Uh, okay.
I like all of them but seafood flavor is my
favorite.");
MsgDisp("主人公","(Hehe, ｛Himuro＊＊｝ seems to be enjoying
himself. I'm looking forward to trying his
recommendations!)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChClose(6,0,0);
ChClose(7,0,0);
