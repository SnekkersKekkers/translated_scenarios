MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0);
VoiceEVSPlay(1);
VoicePlay("D140A03400_01_000");
MsgDisp("Kazama","｛主人公｝, what class 
do we have in the afternoon?");
MsgDisp("主人公","Biology.");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0,1);
VoicePlay("D140A03400_01_010");
MsgDisp("Kazama","ＯＫ.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
VoicePlay("D140A03400_06_000");
MsgDisp("Himuro","...Ryota-senpai, you 
knew already when you asked 
that, didn't you?");
ChMouth(1,2);
MsgDisp("主人公","Eh?");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0);
VoicePlay("D140A03400_01_020");
MsgDisp("Kazama","What are you saying?");
ChEye(2,0);
ChMouth(2,3);
ChMotion(2,3);
VoicePlay("D140A03400_02_000");
MsgDisp("Sassa","Haha!
That's just like Ryota.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,3);
VoicePlay("D140A03400_06_010");
MsgDisp("Himuro","Well, yeah.
His possessiveness is that strong
it's almost refreshing.");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0,1);
VoicePlay("D140A03400_01_030");
MsgDisp("Kazama","It's just a normal conversation.
Right?");
MsgDisp("主人公","Yeah, was there 
something weird about it?");
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,4);
VoicePlay("D140A03400_02_010");
MsgDisp("Sassa","Huh...
Is it actually us who's 
too conscious over it?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("D140A03400_06_020");
MsgDisp("Himuro","Eh... no.
I felt like there was some
hidden meaning to it.");
MsgDisp("主人公","?");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0,1);
VoicePlay("D140A03400_01_040");
MsgDisp("Kazama","I see. 
We're childhood friends, we're in the 
same classes, Inori feels jealous.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
ChEyeOpenLevel(6,8);
VoicePlay("D140A03400_06_030");
MsgDisp("Himuro","Haa... he really does do it unconsciously.
It's actually quite impressive.");
ChEyeOpenLevel(1,0);
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,2);
VoicePlay("D140A03400_02_020");
MsgDisp("Sassa","Don't mind it, Inori.");
ChEye(1,0);
ChMouth(1,2);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,5);
VoicePlay("D140A03400_06_040");
MsgDisp("Himuro","Eh?");
ChEye(2,0);
ChMouth(2,3);
ChMotion(2,2);
VoicePlay("D140A03400_02_030");
MsgDisp("Sassa","Sorry.
I mean, I'm their childhood
friend also, after all.");
ChEye(1,2);
ChMouth(1,0);
ChEyeOpenLevel(1,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,5);
VoicePlay("D140A03400_06_050");
MsgDisp("Himuro","Ugh...");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,4);
MsgDisp("主人公","｛氷室＊＊｝, you're in
the same school as us right?");
ChEye(1,2);
ChMouth(1,3);
ChMotion(1,0,1);
VoiceEVSPlay(1);
VoicePlay("D140A03400_01_050");
MsgDisp("Kazama","｛主人公｝, 
that's kind of forced.");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,1);
ChEyeOpenLevel(6,0);
VoicePlay("D140A03400_06_060");
MsgDisp("Himuro","It's nothing.
I don't really mind, you know");
MsgDisp("主人公","(Let's all get along
and have lunch, okay...?)");
BGMStop();
MsgClose();
ScrFadeOut(0);
ChClose(1,0,0);
ChClose(2,0,0);
ChClose(6,0,0);
