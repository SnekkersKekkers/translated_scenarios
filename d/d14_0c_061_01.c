ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0);
ChCheek(5,0);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
ChCheek(7,0);
VoicePlay("D140C06101_07_000");
MsgDisp("Mikage","Inori sure seems busy, huh?");
MsgDisp("主人公","He said he had class duty and was helping the
teacher.");
ChEye(5,3);
ChMouth(5,2);
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("D140C06101_07_010");
MsgDisp("Mikage","Oh, you have a firm grasp of things.");
ChMotion(5,0,1);
VoicePlay("D140C06101_05_000");
MsgDisp("Hiiragi","Does Inori-kun report all that to you?");
MsgDisp("主人公","Yeah, not report per se...but he told me.");
ChEye(5,0);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("D140C06101_05_010");
MsgDisp("Hiiragi","He's pretty meticulous about those things.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,1);
VoicePlay("D140C06101_07_020");
MsgDisp("Mikage","It's surprising right?
I thought he was more of a free spirit.");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("D140C06101_05_020");
MsgDisp("Hiiragi","You're good at managing him.");
ChEye(7,0);
ChMouth(7,3);
ChMotion(7,1);
VoicePlay("D140C06101_07_030");
MsgDisp("Mikage","I see, is this him wearing golden sandals?");
MsgDisp("主人公","Golden sandals?");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,4);
ChEyeOpenLevel(5,0);
VoicePlay("D140C06101_05_030");
MsgDisp("Hiiragi","There's a saying: \"Search for an older wife
even in golden sandals.\"
That's what you are to Inori-kun.");
MsgDisp("主人公","Eh?
Wife?");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("D140C06101_05_040");
MsgDisp("Hiiragi","Why are you that surprised?
Everyone knows that you two have been getting
along recently.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,3);
VoicePlay("D140C06101_07_040");
MsgDisp("Mikage","Since Inori-kun is having so much fun.
Isn't that nice.");
MsgDisp("主人公","(Ugh...this is a little embarrassing?)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChClose(5,0,0);
ChClose(7,0,0);
