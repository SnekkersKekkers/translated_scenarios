ChEye(5,0);
ChMouth(5,4);
ChMotion(5,0);
ChCheek(5,0);
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,7);
ChCheek(6,0);
MsgDisp("主人公","It's kind of quiet today isn't it?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("D140C04002_06_000");
MsgDisp("Himuro","Have I or Yanosuke-senpai ever been noisy?");
MsgDisp("主人公","...Ah, it's cause
｛御影＊＊｝ isn't here
today.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
ChEyeOpenLevel(6,8);
VoicePlay("D140C04002_06_010");
MsgDisp("Himuro","So?");
ChEye(5,2);
ChMouth(5,4);
ChMotion(5,4);
ChEyeOpenLevel(5,9);
VoicePlay("D140C04002_05_000");
MsgDisp("Hiiragi","Inori-kun, you sure are in a bad mood,
aren't you?");
MsgDisp("主人公","Isn't he more irritable than usual?");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("D140C04002_05_010");
MsgDisp("Hiiragi","Heheh, being irritable is a key part of
who he is.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("D140C04002_06_020");
MsgDisp("Himuro","Haa...there was something I wanted to talk
to Kojiro sensei about too today.");
ChEye(5,0);
ChMotion(5,4);
MsgDisp("主人公","I see.
What was it?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ChEyeOpenLevel(6,8);
VoicePlay("D140C04002_06_030");
MsgDisp("Himuro","I wanted to talk to Kojiro-sensei, not
you.");
ChEye(5,0);
ChMouth(5,3);
ChMotion(5,3);
VoicePlay("D140C04002_05_020");
MsgDisp("Hiiragi","Ah, this way!");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("D140C04002_06_040");
MsgDisp("Himuro","Eh?");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("D140C04002_05_030");
MsgDisp("Hiiragi","It's Mikage-sensei.
Looks like he's finished his work.");
MsgDisp("主人公","That's good.
You'll be able to talk to him now won't
you?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,1);
ChEyeOpenLevel(6,0);
VoicePlay("D140C04002_06_050");
MsgDisp("Himuro","I'm good for today after all.
Disregard what I said just now.");
ChEye(5,3);
ChMouth(5,2);
ChMotion(5,4);
VoicePlay("D140C04002_05_040");
MsgDisp("Hiiragi","Eh, why?");
MsgDisp("主人公","Is that so?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
ChEyeOpenLevel(6,8);
ChCheek(6,5);
VoicePlay("D140C04002_06_060");
MsgDisp("Himuro","He'll probably say something like, \"Oh,
you missed me Inori?\", that's why, I'm
good for today.");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("D140C04002_05_050");
MsgDisp("Hiiragi","Heheh, understood.
Alright, with this it'll surely become
lively, won't it?");
MsgDisp("主人公","(Looks like because
｛御影＊＊｝ is coming,
both of them suddenly perked up?)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChClose(5,0,0);
ChClose(6,0,0);
