ChEye(3,0);
ChMouth(3,0);
ChMotion(3,0,1);
ChCheek(3,0);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0);
ChCheek(1,0);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
ChCheek(4,0);
ChEye(1,2);
ChMouth(1,3);
ChMotion(1,3);
ChEyeOpenLevel(1,0);
VoicePlay("D140B03300_01_000");
MsgDisp("Kazama","Ah--, it's finally lunch time.");
MsgDisp("主人公","We had PE in the morning, 
so it's gotten us quite 
hungry, hasn't it?");
ChMotion(1,0);
ChEyeOpenLevel(1,-1);
VoicePlay("D140B03300_01_010");
MsgDisp("Kazama","Yeah. 
PE for fourth period is rough.");
ChEye(3,0);
ChMouth(3,3);
ChMotion(3,1,1);
VoicePlay("D140B03300_03_000");
MsgDisp("Honda","Our class kept having to move between 
classrooms, so I'm now really hungry.
What about you, Mii-kun?");
ChEye(4,0);
ChMouth(4,0);
VoicePlay("D140B03300_04_000");
MsgDisp("Nanatsumori","Not really.");
VoicePlay("D140B03300_03_010");
MsgDisp("Honda","Even though Mii-kun's body
is so big, he doesn't get hungry.");
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,2,1);
VoicePlay("D140B03300_04_010");
MsgDisp("Nanatsumori","I don't think this is related
to how big your body is.");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,3);
VoicePlay("D140B03300_01_020");
MsgDisp("Kazama","Come on. 
Let's quickly eat.");
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,3,1);
VoicePlay("D140B03300_03_020");
MsgDisp("Honda","Ah, those, Ryo-kun!
Give me one!");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0);
VoicePlay("D140B03300_01_030");
MsgDisp("Kazama","Why.");
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("D140B03300_04_020");
MsgDisp("Nanatsumori","Ah, Dahon. 
Give me one of those.");
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,4,1);
VoicePlay("D140B03300_03_030");
MsgDisp("Honda","This?
Then, I'll trade you this 
for that, Mii-kun.");
ChEye(4,4);
ChMouth(4,4);
ChMotion(4,0,1);
VoicePlay("D140B03300_04_030");
MsgDisp("Nanatsumori","No way.
I won't hand this over.");
ChEye(1,2);
ChEye(3,1);
ChMouth(3,4);
ChMotion(3,5,1);
VoicePlay("D140B03300_03_040");
MsgDisp("Honda","Eh!
Then, the negotiations
have broken down.");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,0);
VoicePlay("D140B03300_01_040");
MsgDisp("Kazama","Both of you, just eat already.");
ChEye(4,2);
ChMouth(4,2);
ChMotion(4,2,1);
VoicePlay("D140B03300_04_040");
MsgDisp("Nanatsumori","It's Dahon's fault
for making me mad.");
ChEye(3,2);
ChMouth(3,4);
ChMotion(3,2,1);
VoicePlay("D140B03300_03_050");
MsgDisp("Honda","...It's Mii-kun's fault.");
ChEye(1,2);
ChMouth(1,2);
ChMotion(1,0,1);
ChEyeOpenLevel(1,0);
VoicePlay("D140B03300_01_050");
MsgDisp("Kazama","Haa...");
MsgDisp("主人公","(Hehe! 
We're kind of like a family!)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChClose(1,0,0);
ChClose(3,0,0);
ChClose(4,0,0);
