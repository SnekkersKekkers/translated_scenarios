ChEye(21,0);
ChEye(22,0);
ChMouth(22,3);
ChMotion(22,4,1);
VoicePlay("D140D07040_22_000");
MsgDisp("Hikaru","Hey, hey did you see this month's Habacha
feature on Minoru-kun's accessories?");
MsgDisp("主人公","Eh?
I haven't seen it yet...");
ChEye(21,0);
ChMouth(21,4);
ChMotion(21,0);
VoicePlay("D140D07040_21_000");
MsgDisp("Michiru","Hikaru, you got it wrong.
Habacha goes on sale tomorrow.");
ChEye(22,3);
ChMouth(22,0);
ChMotion(22,0,1);
ChEyeOpenLevel(22,10);
VoicePlay("D140D07040_22_010");
MsgDisp("Hikaru","Huh? Is that so?");
ChEye(21,2);
ChMouth(21,4);
ChMotion(21,5);
ChEyeOpenLevel(21,0);
VoicePlay("D140D07040_21_010");
MsgDisp("Michiru","Hey.");
ChFace(21,0);
MsgDisp("主人公","A feature on ｛七ツ森＊｝.
Both the Hanatsubaki twins 
and ｛七ツ森＊｝ are so cool...!");
ChEye(22,3);
ChMouth(22,3);
ChMotion(22,4,1);
VoicePlay("D140D07040_22_020");
MsgDisp("Hikaru","The cool one is Minoru-kun.
We're cute▼");
MsgDisp("主人公","Ah, is that right?");
ChEye(21,0);
ChMouth(21,0);
ChMotion(21,0);
VoicePlay("D140D07040_21_020");
MsgDisp("Michiru","But, Minori kun at school and at work are
like two completely different people.
I respect him for that.");
ChEye(22,3);
ChMouth(22,3);
ChMotion(22,5,1);
VoicePlay("D140D07040_22_030");
MsgDisp("Hikaru","I totally get it!
It would be impossible for Hikaru; 
that multi-faceted sort of like.");
MsgDisp("主人公","(Multi-faceted? Not a \"Double life\"?
You are talking about the four
eyes ｛七ツ森＊｝ at school
And the model \"Nana\" right...?)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChClose(21,0,0);
ChClose(22,0,0);
