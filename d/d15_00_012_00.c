MsgDisp ( "","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・" ) ;
ChEye ( 2,0 ) ;
ChMouth ( 2,3 ) ;
ChMotion ( 2,0 ) ;
VoicePlay ( "D150001200_02_000" ) ;
MsgDisp ( "颯砂","I wonder if we said \"Itadakimasu!\"
together back when we were in
Kindergarten." ) ;
ChEye ( 1,0 ) ;
ChMouth ( 1,2 ) ;
ChEyeOpenLevel ( 1,8 ) ;
VoicePlay ( "D150001200_01_000" ) ;
MsgDisp ( "風真","Yes, we did." ) ;
ChEye ( 2,0 ) ;
ChMouth ( 2,3 ) ;
ChMotion ( 2,3 ) ;
VoicePlay ( "D150001200_02_010" ) ;
MsgDisp ( "颯砂","Ryota, didn't you answer that too quickly?
So cold." ) ;
ChMotion ( 1,1 ) ;
ChEyeOpenLevel ( 1, - 1 ) ;
VoicePlay ( "D150001200_01_010" ) ;
MsgDisp ( "風真","Shut up." ) ;
MsgDisp ( "主人公","Hehe.
I'm so happy we get to say \"Itadakimasu!\"
together as high school students." ) ;
ChEye ( 1,4 ) ;
ChMouth ( 1,4 ) ;
ChMotion ( 1,0 ) ;
ChEyeOpenLevel ( 1, - 1 ) ;
VoicePlay ( "D150001200_01_020" ) ;
MsgDisp ( "風真","Right?" ) ;
ChEye ( 2,3 ) ;
ChMouth ( 2,3 ) ;
ChMotion ( 2,1 ) ;
ChEyeOpenLevel ( 2,0 ) ;
VoicePlay ( "D150001200_02_020" ) ;
MsgDisp ( "颯砂","After we finish eating we should go
outside and play like kindergarteners,
then." ) ;
ChEye ( 1,0 ) ;
ChMouth ( 1,0 ) ;
ChMotion ( 1,3 ) ;
VoicePlay ( "D150001200_01_030" ) ;
MsgDisp ( "風真","......Geez.
What the hell are you talking about?" ) ;
MsgDisp ( "主人公","That sounds fun. What should we play?" ) ;
ChEye ( 2,0 ) ;
ChMouth ( 2,3 ) ;
ChMotion ( 2,4 ) ;
ChEyeOpenLevel ( 2, - 1 ) ;
VoicePlay ( "D150001200_02_030" ) ;
MsgDisp ( "颯砂","Just like I thought, you're into the idea.
What about you, Ryota?" ) ;
ChEye ( 1,0 ) ;
ChMouth ( 1,0 ) ;
ChMotion ( 1,1 ) ;
ChEyeOpenLevel ( 1,0 ) ;
VoicePlay ( "D150001200_01_040" ) ;
MsgDisp ( "風真","Please choose something that doesn't
involve running." ) ;
MsgDisp ( "主人公","(Hehe!)" ) ;
BGMStop (  ) ;
MsgClose (  ) ;
ScrFadeOut ( 0,0 ) ;
ChClose ( 1,0,0 ) ;
ChClose ( 2,0,0 ) ;
