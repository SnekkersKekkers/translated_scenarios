MsgClose();
ScrFadeOut(0);
MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
BGOpen("sc771",2);
ChLayout(2);
MsgClose();
ChOpen(1,34,0,0,3,-1,-1,0,0,0,30);
ChNanaType(0);
ChOpen(4,34,0,0,0,-1,-1,0,1,0,30);
ChOpen(3,34,0,0,0,-1,-1,0,2,0,30);
ScrFadeIn(0);
VoiceEVSPlay(1);
VoicePlay("D130B00101_01_000");
MsgDisp("Kazama","｛主人公｝.");
MsgDisp("主人公","Ah, you guys.
Merry Christmas!");
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,3,1);
VoicePlay("D130B00101_03_000");
MsgDisp("Honda","Merry Merry Christmas!");
ChEye(4,0);
ChMouth(4,0);
VoicePlay("D130B00101_04_000");
MsgDisp("Nanatsumori","The gift exchange is done and now we
have some free time. This is the last
Christmas Party of high school, huh.");
ChEye(1,2);
ChMotion(1,0,1);
ChEyeOpenLevel(1,0);
MsgDisp("主人公","You're right......");
ChEye(3,0);
ChMouth(3,3);
ChMotion(3,1,1);
VoicePlay("D130B00101_03_010");
MsgDisp("Honda","But the party isn't over yet.
Let's all have fun hanging out, and make
some good memories today!");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,3);
VoicePlay("D130B00101_01_010");
MsgDisp("Kazama","Yeah, that's right.
For the first time I feel like I
understand Honda's value.");
ChEye(3,5);
ChMouth(3,4);
ChMotion(3,5,1);
VoicePlay("D130B00101_03_020");
MsgDisp("Honda","Ehh∋　The first time?");
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,2,1);
VoicePlay("D130B00101_04_010");
MsgDisp("Nanatsumori","I get it.
Kazama and I are two people who can't
really change the mood.");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0);
ChEyeOpenLevel(1,0);
VoicePlay("D130B00101_01_020");
MsgDisp("Kazama","That's exactly it.");
ChEyeOpenLevel(1,-1);
ChEye(3,2);
ChMouth(3,4);
ChMotion(3,2,1);
VoicePlay("D130B00101_03_030");
MsgDisp("Honda","Well, that's fine then I guess.");
MsgDisp("主人公","Hehe.");
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,0,1);
VoicePlay("D130B00101_03_040");
MsgDisp("Honda","You know, the four of us made a lot of
good memories together...... I'm really 
grateful for Haba High.");
ChEye(1,2);
ChMouth(1,2);
ChMotion(1,0,1);
VoicePlay("D130B00101_01_030");
MsgDisp("Kazama","What are you saying now?");
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("D130B00101_04_020");
MsgDisp("Nanatsumori","It's not like Dahon to say that kind
of thing.
But I agree with you.");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,1);
ChEyeOpenLevel(1,8);
VoicePlay("D130B00101_01_040");
MsgDisp("Kazama","You two can't be serious......");
ChEye(1,2);
ChMouth(1,1);
ChMotion(1,3);
ChEyeOpenLevel(1,0);
VoicePlay("D130B00101_01_050");
MsgDisp("Kazama","Sighh!
I feel the same way then!");
ChEye(1,4);
ChMouth(1,2);
ChMotion(1,0);
ChEye(4,0);
ChMouth(4,0);
MsgDisp("主人公","Me too.
I'm so happy I got to make good memories
together with you all.");
ChEye(1,2);
ChMouth(1,3);
ChMotion(1,0);
ChEyeOpenLevel(1,8);
VoicePlay("D130B00101_01_060");
MsgDisp("Kazama","......When you say it all pretty like
that——");
ChEye(3,0);
ChMouth(3,4);
ChMotion(3,2,1);
ChEyeOpenLevel(3,0);
VoicePlay("D130B00101_03_050");
MsgDisp("Honda","......It's making me tear up a little——");
ChEye(4,2);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("D130B00101_04_030");
MsgDisp("Nanatsumori","......Should we all shed a few tears of
joy together?");
ChEye(1,4);
ChMouth(1,2);
ChMotion(1,0,1);
ChEyeOpenLevel(1,0);
ChMouthOpenLevel(1,0);
VoicePlay("D130B00101_01_070");
MsgDisp("Kazama","…………");
ChEye(4,3);
ChMouth(4,3);
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,1,1);
ChEyeOpenLevel(3,0);
ChEye(1,3);
ChMouth(1,3);
ChMotion(1,3);
VoicePlay("D130B00101_01_080");
MsgDisp("Kazama","No way!
Ahahaha!");
MsgDisp("主人公","(Hehe!
I have so many good memories with these
guys, just now was a new one!)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChClose(1,0,0);
ChClose(4,0,0);
ChClose(3,0,0);
