MsgDisp ( "","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・" ) ;
ChEye ( 2,0 ) ;
ChMouth ( 2,3 ) ;
ChMotion ( 2,4 ) ;
VoicePlay ( "D150002401_02_000" ) ;
MsgDisp ( "颯砂","Everyone's greeting each other.
Feels like elementary school lunches.." ) ;
MsgDisp ( "主人公","Hehe, true." ) ;
ChEye ( 4,0 ) ;
ChMouth ( 4,4 ) ;
ChMotion ( 4,1 ) ;
VoicePlay ( "D150002401_04_000" ) ;
MsgDisp ( "七ツ森","It doesn't matter if it's a bento lunch or
a cafeteria lunch. It's important to be
grateful to the food and the person who
made it." ) ;
ChEye ( 2,0 ) ;
ChMouth ( 2,3 ) ;
ChMotion ( 2,3 ) ;
VoicePlay ( "D150002401_02_010" ) ;
MsgDisp ( "颯砂","Nanatsumori is really polite and
considerate." ) ;
ChEye ( 4,0 ) ;
ChMouth ( 4,4 ) ;
ChMotion ( 4,2 ) ;
VoicePlay ( "D150002401_04_010" ) ;
MsgDisp ( "七ツ森","Not really......
This is normal to me." ) ;
MsgDisp ( "主人公","Yeah, he's got his uniform on properly." ) ;
ChEye ( 4,0 ) ;
ChMouth ( 4,3 ) ;
ChMotion ( 4,0 ) ;
VoicePlay ( "D150002401_04_020" ) ;
MsgDisp ( "七ツ森","That's such a weird way to compliment me." ) ;
ChEye ( 2,1 ) ;
ChMouth ( 2,4 ) ;
ChMotion ( 2,4 ) ;
VoicePlay ( "D150002401_02_020" ) ;
MsgDisp ( "颯砂","I'd wear a tie too if you gave me
compliments." ) ;
MsgDisp ( "主人公","(I'd like to see ｛颯砂＊＊｝ wearing a
necktie, actually.)" ) ;
BGMStop (  ) ;
MsgClose (  ) ;
ScrFadeOut ( 0,0 ) ;
ChClose ( 2,0,0 ) ;
ChClose ( 4,0,0 ) ;
