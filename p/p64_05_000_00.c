ChEye(5,4);
ChMouth(5,4);
ChMotion(5,4,1);
ChEyeOpenLevel(5,0);
ScrFadeIn(0);
VoicePlay("P640500000_05_000");
MsgDisp("Hiiragi","Phew...
Yes, it's calming.");
MsgDisp("主人公","Ah,｛柊＊＊＊｝.
Is it?");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,4);
VoicePlay("P640500000_05_010");
MsgDisp("Hiiragi","Yes, it's relaxing and 
delicious at the same time.");
MsgDisp("主人公","Hehe, it's relaxing because 
of the herbs used, huh?");
ChEye(5,4);
ChMouth(5,4);
ChMotion(5,0);
VoicePlay("P640500000_05_020");
MsgDisp("Hiiragi","Seems like it, yes.
Thanks to this, it seems like
I'll sleep soundly today.");
MsgDisp("主人公","(Seems like ｛柊＊＊＊｝ liked it!
Yay, it wa s huge success∈)");
MsgClose();
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChClose(5,0,0);
