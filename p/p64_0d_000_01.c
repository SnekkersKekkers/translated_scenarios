BGMPlay("BGM_PLACE_SHOP",0.01);
ScrFadeIn(0);
ChEye(22,5);
ChMouth(22,1);
ChMotion(22,0);
VoicePlay("P640D00001_22_000");
MsgDisp("Hikaru","Hmm?");
ChEye(21,0);
ChMouth(21,4);
ChMotion(21,4);
VoicePlay("P640D00001_21_000");
MsgDisp("Michiru","This...");
MsgDisp("主人公","How is it?");
ChEye(22,2);
ChMouth(22,0);
ChMotion(22,2,1);
VoicePlay("P640D00001_22_010");
MsgDisp("Hikaru","Mari, can Hikaru be honest?
This is something you shouldn't put out.");
MsgDisp("主人公","Eh?");
ChEye(21,0);
ChMouth(21,4);
ChMotion(21,3);
VoicePlay("P640D00001_21_010");
MsgDisp("Michiru","The stimulus of the herbs are too strong, and
with one mouthful, a shock runs through you.");
ChEye(22,0);
ChMouth(22,3);
ChMotion(22,5,1);
VoicePlay("P640D00001_22_020");
MsgDisp("Hikaru","Hikaru got it!
It's some what like an electric shock～!");
MsgDisp("主人公","(Maybe I mixed the herbs wrong...
Uugh, a huge fail...)");
MsgClose();
ScrFadeOut(0,0);
