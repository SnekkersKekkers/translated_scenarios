MsgClose();
ChOpen(2,255,2,0,0,#1,#1,0,0);
VoicePlay("G020230201_02_000");
MsgDisp("Sassa","The fireflies' light sure is beautiful
like this.");
MsgDisp("主人公","Yeah...");
ChEye(2,4);
ChMouth(2,4);
ChMotion(2,4);
VoicePlay("G020230201_02_010");
MsgDisp("Sassa","It's not about what you see but who you see
it with?
Right?");
MsgDisp("主人公","Eh?");
ChEye(2,0);
ChMouth(2,2);
ChMotion(2,0);
VoicePlay("G020230201_02_020");
MsgDisp("Sassa","What do you mean by 'Eh? '.
I took the trouble to say something nice.");
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,4);
ChEyeOpenLevel(2,7);
VoicePlay("G020230201_02_030");
MsgDisp("Sassa","I'm saying it looks beautiful because I'm
seeing it with you.");
MsgDisp("主人公","Y-Yeah.
But the fireflies here are always beautiful?");
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,0);
ChEyeOpenLevel(2,7);
VoicePlay("G020230201_02_040");
MsgDisp("Sassa","Ehhh—?
I don't know who else you've been seeing them
with, but.");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,4);
VoicePlay("G020230201_02_050");
MsgDisp("Sassa","Sigh...they look prettier than I thought.
I really think it's because of you.");
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,4);
VoicePlay("G020230201_02_060");
MsgDisp("Sassa","Is that bad?");
MsgDisp("主人公","No, hearing you say that makes me happy.");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,1);
VoicePlay("G020230201_02_070");
MsgDisp("Sassa","Alright, just as planned.");
MsgDisp("主人公","Plan?");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0);
VoicePlay("G020230201_02_080");
MsgDisp("Sassa","Actually, I've been practising what to say to
you here since yesterday.");
MsgDisp("主人公","Hehe, training just like you do for
track, huh?");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,3);
VoicePlay("G020230201_02_090");
MsgDisp("Sassa","Exactly.
Always plan ahead, that's my motto.");
MsgDisp("主人公","(He's always thinking about so much different
things for me...
Thank you, ｛Sassa＊＊｝)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChLayout(1);
ChEyeOpenLevel(2,#1);
ChMouthOpenLevel(2,#1);
ChCheek(2,0);
