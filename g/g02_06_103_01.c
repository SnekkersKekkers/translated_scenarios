MsgClose();
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
ChEyeOpenLevel(6,0);
VoicePlay("G020610301_06_000");
MsgDisp("Himuro","Lately, I've been sleep-deprived. It's
your fault.");
MsgDisp("主人公","Eh! Why?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020610301_06_010");
MsgDisp("Himuro","You came to my place the other day, right?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
ChCheek(6,10);
VoicePlay("G020610301_06_020");
MsgDisp("Himuro","......After you left, your scent lingered.");
MsgDisp("主人公","∋");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020610301_06_030");
MsgDisp("Himuro","Since then, at random moments, I feel like
I smell your scent......");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020610301_06_040");
MsgDisp("Himuro","As if you're always in my room......");
ChEye(6,1);
ChMouth(6,1);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("G020610301_06_050");
MsgDisp("Himuro","Argh, this is really troubling!");
MsgDisp("主人公","I-I'm sorry!");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,1);
ChCheek(6,0);
VoicePlay("G020610301_06_060");
MsgDisp("Himuro","Could you not apologize?
I know I'm just taking it out on you.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020610301_06_070");
MsgDisp("Himuro","know it's childish, and I hate myself for
it, but I don't know what to do..");
MsgDisp("主人公","｛氷室＊＊｝……");
ChEye(6,2);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("G020610301_06_080");
MsgDisp("Himuro","......I'll probably be sleep-deprived
again today. Because I'll remember your
scent from now when I'm in my room.");
MsgDisp("主人公","(I feel like I'm being told something
great......)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChLayout(1);
ChEyeOpenLevel(6,-1);
ChMouthOpenLevel(6,-1);
ChCheek(6,0);
