MsgClose();
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0,1);
VoicePlay("G020630001_06_000");
MsgDisp("Himuro","Lately it's been awfully lively.");
MsgDisp("主人公","Isn't it because everyone's together?");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("G020630001_06_010");
MsgDisp("Himuro","Right.
You and me.
And also Ryota-senpai and Nozomu-senpai.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("G020630001_06_020");
MsgDisp("Himuro","It's kind of an unbelievable combination.");
MsgDisp("主人公","Is it?");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("G020630001_06_030");
MsgDisp("Himuro","Of course.
Without you, 
there'd be no connection at all.");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,3);
VoicePlay("G020630001_06_040");
MsgDisp("Himuro","So I must be grateful to you.");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,3);
VoicePlay("G020630001_06_050");
MsgDisp("Himuro","Because I met those two,
my world has expanded.
I've learned new perspectives.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("G020630001_06_060");
MsgDisp("Himuro","......Though I wouldn't say that to 
Ryouta-senpai or Nozomu-senpai.");
MsgDisp("主人公","Hehe.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("G020630001_06_070");
MsgDisp("Himuro","I'm sure the me from middle school
would be shocked.
\"What are you doing, you idiot?\"");
MsgDisp("主人公","Eh, why?");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,4);
VoicePlay("G020630001_06_080");
MsgDisp("Himuro","Because it's unproductive.
......But, it's fun.
It's complete nonsense.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("G020630001_06_090");
MsgDisp("Himuro","If these relationships continued forever
......that would be nice.");
MsgDisp("主人公","It'll definitely last.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("G020630001_06_100");
MsgDisp("Himuro","I wonder?
That might be problematic too.");
MsgDisp("主人公","Eh......?");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0,1);
VoicePlay("G020630001_06_110");
MsgDisp("Himuro","You don't need to understand.
Let's just enjoy this moment for now.");
MsgDisp("主人公","(Moment......
Does ｛氷室＊＊｝ think our relationship
is only temporary?)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChLayout(1);
ChEyeOpenLevel(6,#1);
ChMouthOpenLevel(6,#1);
ChCheek(6,0);
