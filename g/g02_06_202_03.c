MsgClose();
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("G020620203_06_000");
MsgDisp("Himuro","A guy with adult charm?");
MsgDisp("主人公","Eh?");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020620203_06_010");
MsgDisp("Himuro","I'm talking about Kojiro-sensei.");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020620203_06_020");
MsgDisp("Himuro","It's common for students to admire their
teachers, and on top of that,
Kojirou-sensei is quite close to the
students.");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,0,1);
VoicePlay("G020620203_06_030");
MsgDisp("Himuro","To me, he's too innocent, and it feels
like I'm taking care of a younger brother.
Well, that gap is probably what makes him
popular.");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,0);
ChEyeOpenLevel(6,0);
VoicePlay("G020620203_06_040");
MsgDisp("Himuro","You feel the same way, right?");
MsgDisp("主人公","That's......");
ChEye(6,2);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("G020620203_06_050");
MsgDisp("Himuro","You don't have to hide it.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020620203_06_060");
MsgDisp("Himuro","All in all, you're spending a lot
of time with Kojirou-sensei these days.");
MsgDisp("主人公","Y-Yeah......");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,1);
VoicePlay("G020620203_06_070");
MsgDisp("Himuro","But, I won't lose to Kojiro-sensei.");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,0);
VoicePlay("G020620203_06_080");
MsgDisp("Himuro","In fact, I'm glad it was Kojiro-sensei 
who took the place by your side.");
MsgDisp("主人公","Eh......?");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,0);
VoicePlay("G020620203_06_090");
MsgDisp("Himuro","I'd rather it be someone much older and
more mature than someone mediocre. It will
help me grow further without being
complacent.");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,1);
VoicePlay("G020620203_06_100");
MsgDisp("Himuro","So expect to see a more grown-up version
of me.");
MsgDisp("主人公","Y-Yeah. I got it.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020620203_06_110");
MsgDisp("Himuro","Don't forget what you just said.
Because it's a promise.");
MsgDisp("主人公","(I feel like I made a really big
promise......)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChLayout(1);
ChEyeOpenLevel(6,-1);
ChMouthOpenLevel(6,-1);
ChCheek(6,0);
