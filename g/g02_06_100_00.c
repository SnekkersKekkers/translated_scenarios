MsgClose();
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020610000_06_000");
MsgDisp("Himuro","......Beautiful");
MsgDisp("主人公","Yeah......");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("G020610000_06_010");
MsgDisp("Himuro","It's a strange feeling.");
MsgDisp("主人公","Eh?");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("G020610000_06_020");
MsgDisp("Himuro","No......it's just that having a genuine 
reaction like 'beautiful' is a bit 
surprising to me.");
MsgDisp("主人公","Is it?");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,3);
VoicePlay("G020610000_06_030");
MsgDisp("Himuro","Yeah. Even if I thought so in my heart, I
could never really say it out loud..");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
ChCheek(6,5);
VoicePlay("G020610000_06_040");
MsgDisp("Himuro","It felt frustrating, like admitting it was
embarrassing......");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ChCheek(6,0);
VoicePlay("G020610000_06_050");
MsgDisp("Himuro","But recently, I've realized that those
feelings are trivial.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020610000_06_060");
MsgDisp("Himuro","Maybe it's thanks to you.");
MsgDisp("主人公","Eh, me?");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,3);
VoicePlay("G020610000_06_070");
MsgDisp("Himuro","Yes. You show all your emotions.");
MsgDisp("主人公","I-is that so......?");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,5);
VoicePlay("G020610000_06_080");
MsgDisp("Himuro","Even right now. 
Whenever you speak.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("G020610000_06_090");
MsgDisp("Himuro","But I think it's good. Honestly. 
Because you're like that, I——");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,1);
ChCheek(6,10);
VoicePlay("G020610000_06_100");
MsgDisp("Himuro","......What am I saying? Let's head back.
I'll walk you home.");
MsgDisp("主人公","(｛氷室＊＊｝……?)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChLayout(1);
ChEyeOpenLevel(6,-1);
ChMouthOpenLevel(6,-1);
ChCheek(6,0);
