MsgClose();
ChOpen(1,255,0,0,3,#1,#1,0,0);
ChMotion(1,1);
VoicePlay("G020110304_01_000");
MsgDisp("Kazama","Night parades are great, but this place is
amazing too.");
MsgDisp("主人公","Yeah, it's so peaceful here.");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("G020110304_01_010");
MsgDisp("Kazama","Yeah, the other place has a lot of noise
too.");
ChEye(1,4);
ChMouth(1,4);
ChMotion(1,2);
VoicePlay("G020110304_01_020");
MsgDisp("Kazama","A quiet night parade...yeah, that would be
nice, huh.");
MsgDisp("主人公","That's right, huh.");
ChEye(1,4);
ChMouth(1,0);
ChMotion(1,1);
ChEyeOpenLevel(1,0);
VoicePlay("G020110304_01_030");
MsgDisp("Kazama","Shh.
Listen carefully.");
MsgDisp("主人公","Eh, okay. ......");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,1);
ChMouthOpenLevel(1,0);
VoicePlay("G020110304_01_040");
MsgDisp("Kazama","......");
ChMotion(1,0);
ChEyeOpenLevel(1,0);
Wait(12,0);
ChEye(1,4);
ChMouth(1,3);
VoicePlay("G020110304_01_050");
MsgDisp("Kazama","Well?");
MsgDisp("主人公","Eh?");
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0);
VoicePlay("G020110304_01_060");
MsgDisp("Kazama","I can hear it.");
MsgDisp("主人公","My heart's voice?");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,3);
VoicePlay("G020110304_01_070");
MsgDisp("Kazama","No.
The voice of the fireflies.");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("G020110304_01_080");
MsgDisp("Kazama","They're all out there, shining and flying
while shouting the names of the ones they
love.");
ChEye(1,0);
ChMouth(1,4);
ChMotion(1,1);
VoicePlay("G020110304_01_090");
MsgDisp("Kazama","It's not quiet at all.
They're all desperate.");
MsgDisp("主人公","I see.
They're all searching for a partner.");
ChEye(1,4);
ChMouth(1,0);
ChMotion(1,1);
ChEyeOpenLevel(1,0);
VoicePlay("G020110304_01_100");
MsgDisp("Kazama","Yeah, they glow and fly so that the other
person will like it.");
ChEye(1,4);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("G020110304_01_110");
MsgDisp("Kazama","Just like me, thinking only about your
happy face while choosing your birthday
present.");
MsgDisp("主人公","Yeah, it made me happy.
Thank you, as always.");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,3);
VoicePlay("G020110304_01_120");
MsgDisp("Kazama","That's it, that face of yours.
Unlike fireflies, I get to see that face
every year.");
ChEye(1,4);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("G020110304_01_130");
MsgDisp("Kazama","That's enough 'thanks' for me.");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,1);
ChEyeOpenLevel(1,0);
VoicePlay("G020110304_01_140");
MsgDisp("Kazama","Ahー, I can't wait for your birthday to
come again.");
MsgDisp("主人公","(If that were the case, I'd age
quickly...)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChLayout(1);
ChEyeOpenLevel(1,#1);
ChMouthOpenLevel(1,#1);
ChCheek(1,0);
