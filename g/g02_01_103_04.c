MsgClose();
ChOpen(1,255,0,0,3,#1,#1,0,0);
ChMotion(1,1);
VoicePlay("G020110304_01_000");
MsgDisp("Kazama","The Night Parade and stuff is great, but
this place is amazing too.");
MsgDisp("主人公","Yeah, though it's quiet over here.");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("G020110304_01_010");
MsgDisp("Kazama","True. The noise is one of the amazing
things over there.");
ChEye(1,4);
ChMouth(1,4);
ChMotion(1,2);
VoicePlay("G020110304_01_020");
MsgDisp("Kazama","But a quiet Night Parade...... Yeah, I
like it.");
MsgDisp("主人公","I agree.");
ChEye(1,4);
ChMouth(1,0);
ChMotion(1,1);
ChEyeOpenLevel(1,0);
VoicePlay("G020110304_01_030");
MsgDisp("Kazama","Shhhh. Try listening close for a bit.");
MsgDisp("主人公","Huh? Okay. .........");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,1);
ChMouthOpenLevel(1,0);
VoicePlay("G020110304_01_040");
MsgDisp("Kazama","…………");
ChMotion(1,0);
ChEyeOpenLevel(1,0);
Wait(12,0);
ChEye(1,4);
ChMouth(1,3);
VoicePlay("G020110304_01_050");
MsgDisp("Kazama","Right?");
MsgDisp("主人公","What?");
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0);
VoicePlay("G020110304_01_060");
MsgDisp("Kazama","I could hear it.");
MsgDisp("主人公","How I really feel?");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,3);
VoicePlay("G020110304_01_070");
MsgDisp("Kazama","No. What the fireflies are saying.");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("G020110304_01_080");
MsgDisp("Kazama","All of 'em are flying around, glowing,
while yelling the name of the one they're
in love with.");
ChEye(1,0);
ChMouth(1,4);
ChMotion(1,1);
VoicePlay("G020110304_01_090");
MsgDisp("Kazama","So it's actually not quiet at all.
Everyone's doing everything they can.");
MsgDisp("主人公","Oh, yeah. They're looking for partners
after all.");
ChEye(1,4);
ChMouth(1,0);
ChMotion(1,1);
ChEyeOpenLevel(1,0);
VoicePlay("G020110304_01_100");
MsgDisp("Kazama","Yeah, they're flying around and glowing so
the one they love will like them back.");
ChEye(1,4);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("G020110304_01_110");
MsgDisp("Kazama","The same goes for me. Like how all I think
about is your delighted face as I pick out
your birthday presents.");
MsgDisp("主人公","Yeah, they really make me happy. Thanks
for everything.");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,3);
VoicePlay("G020110304_01_120");
MsgDisp("Kazama","There, that's the face I'm talking about.
Unlike the fireflies, I get to see that
face every year.");
ChEye(1,4);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("G020110304_01_130");
MsgDisp("Kazama","That's enough for me to be 'grateful.'");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,1);
ChEyeOpenLevel(1,0);
VoicePlay("G020110304_01_140");
MsgDisp("Kazama","Man, why can't your birthday get here
sooner?");
MsgDisp("主人公","(But if my birthdays come too quick, I'll
be a granny in no time......)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChLayout(1);
ChEyeOpenLevel(1,#1);
ChMouthOpenLevel(1,#1);
ChCheek(1,0);
