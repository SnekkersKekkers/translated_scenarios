MsgClose();
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
VoicePlay("G020630201_06_000");
MsgDisp("Himuro","......As you probably know,
I like being alone.");
MsgDisp("主人公","Uh, yeah.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("G020630201_06_010");
MsgDisp("Himuro","But it's a bit different with you.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020630201_06_020");
MsgDisp("Himuro","But, fitting in with people is tiring. 
I don't want to do it.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020630201_06_030");
MsgDisp("Himuro","When I say that, people call me a loner,
but liking being alone doesn't mean
having no friends.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020630201_06_040");
MsgDisp("Himuro","If you're really friends, you should
respect each other. If you're truly
friends, you should respect the other
person.");
MsgDisp("Himuro","I'm sorry, but I don't want to impose
my desire to make noise on you.");
MsgDisp("主人公","Right.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0,1);
VoicePlay("G020630201_06_050");
MsgDisp("Himuro","Well, at least even I can
socialize with you.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020630201_06_060");
MsgDisp("Himuro","If we can respect each other, 
I think we can build a good relationship.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,3);
VoicePlay("G020630201_06_070");
MsgDisp("Himuro","It's also important that the timing 
of what you want to do and 
what you can do together align.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("G020630201_06_080");
MsgDisp("Himuro","If I were to have a friend,
I'd want someone like that.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("G020630201_06_090");
MsgDisp("Himuro","Well it's not easy to meet
someone like that.");
MsgDisp("主人公","That's why if you met a person like that,
you'd treasure them, right?");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("G020630201_06_100");
MsgDisp("Himuro","Well, yeah. We'd definitely be friends for
a long time.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020630201_06_110");
MsgDisp("Himuro","......What were we talking about.");
MsgDisp("主人公","(Hehe, I'm glad I finally got to hear
｛氷室＊＊｝ 's thoughts on friendships.)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChLayout(1);
ChEyeOpenLevel(6,-1);
ChMouthOpenLevel(6,-1);
ChCheek(6,0);
