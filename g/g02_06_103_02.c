MsgClose();
ChEye(6,2);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("G020610302_06_000");
MsgDisp("Himuro","Here, we don't have to worry about people
watching us......");
MsgDisp("主人公","Eh?");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,1);
VoicePlay("G020610302_06_010");
MsgDisp("Himuro","Don't get the wrong idea. It's not like
we're going to do anything shady.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020610302_06_020");
MsgDisp("Himuro","Us today.
We're dressed the same, right?");
MsgDisp("主人公","Ah, that's right.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ChCheek(6,10);
VoicePlay("G020610302_06_030");
MsgDisp("Himuro","Isn't that embarrassing for you? 
How do you think people see us?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020610302_06_040");
MsgDisp("Himuro","They might misunderstand and think 
we're a couple, you know?");
ChEye(6,0);
ChMouth(6,2);
ChMotion(6,2);
VoicePlay("G020610302_06_050");
MsgDisp("Himuro","......Well, I don't mind.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020610302_06_060");
MsgDisp("Himuro","But you're a girl. Wouldn't it be a
problem if weird rumors started?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
VoicePlay("G020610302_06_070");
MsgDisp("Himuro","......That being said, I shouldn't have to
worry about you. It's your own problem,
shouldn't you worry yourself?");
MsgDisp("主人公","S-sorry......");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
ChCheek(6,0);
VoicePlay("G020610302_06_080");
MsgDisp("Himuro","......So, how about it? Would it be a
problem if there were rumors about us?");
MsgDisp("主人公","I don't think so?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020610302_06_090");
MsgDisp("Himuro","......Really? So it's okay if we end up in
matching outfits again?");
MsgDisp("主人公","Yeah.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("G020610302_06_100");
MsgDisp("Himuro","Then, together with me......");
MsgDisp("主人公","Eh?");
ChEye(6,0);
ChMouth(6,1);
ChMotion(6,1);
VoicePlay("G020610302_06_110");
MsgDisp("Himuro","It's nothing! Just, that outfit suits you,
so you should wear it again. That's all.");
MsgDisp("主人公","(???)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChLayout(1);
ChEyeOpenLevel(6,-1);
ChMouthOpenLevel(6,-1);
ChCheek(6,0);
