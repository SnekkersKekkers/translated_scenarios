MsgClose();
ChEye(8,4);
ChMouth(8,0);
ChMotion(8,0);
VoicePlay("G020840000_08_000");
MsgDisp("Shirahane","The light of the fireflies is somewhat
warm.");
MsgDisp("主人公","Yes, that's what it feel like.");
ChMotion(8,2,1);
VoicePlay("G020840000_08_010");
MsgDisp("Shirahane","I wonder if it's because of thatー......");
MsgDisp("主人公","Hm?");
ChEye(8,4);
ChMouth(8,3);
ChMotion(8,3,1);
VoicePlay("G020840000_08_020");
MsgDisp("Shirahane","My heart is so happy right now.");
MsgDisp("主人公","Hehe.");
ChEye(8,3);
ChMouth(8,3);
ChEyeOpenLevel(8,0);
VoicePlay("G020840000_08_030");
MsgDisp("Shirahane","Hehe.");
ChEye(8,3);
ChMouth(8,3);
ChMotion(8,0,1);
VoicePlay("G020840000_08_040");
MsgDisp("Shirahane","I'm really glad I got to know you.");
MsgDisp("主人公","Really?");
ChMotion(8,3,1);
VoicePlay("G020840000_08_050");
MsgDisp("Shirahane","Really.");
ChEye(8,2);
ChMouth(8,0);
ChMotion(8,2,1);
VoicePlay("G020840000_08_060");
MsgDisp("Shirahane","But at first I really couldn't believe it.
A girl from the prestigious Haba High is
working part-time at a gas station.");
MsgDisp("主人公","I-Is that so?");
ChEye(8,3);
ChMouth(8,3);
ChEyeOpenLevel(8,0);
VoicePlay("G020840000_08_070");
MsgDisp("Shirahane","Yeah. But now I don't have that prejudice
anymore.");
ChEye(8,3);
ChMouth(8,3);
ChMotion(8,0,1);
VoicePlay("G020840000_08_080");
MsgDisp("Shirahane","If our 'fateful meeting' were to be at the
gas station, wouldn't that be really
funny?");
MsgDisp("主人公","Ahaha!");
ChEye(8,3);
ChMouth(8,3);
ChEyeOpenLevel(8,0);
VoicePlay("G020840000_08_090");
MsgDisp("Shirahane","Hehe!");
MsgDisp("主人公","(Fateful meeting, huh......)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
BGMStop();
ChLayout(1);
ChEyeOpenLevel(8,-1);
ChMouthOpenLevel(8,-1);
ChCheek(8,0);
