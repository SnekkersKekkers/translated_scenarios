MsgClose();
ChEye(8,4);
ChMouth(8,0);
ChMotion(8,0);
ChCheek(8,4);
MsgDisp("主人公","Wow......so many fireflies!
Beautiful......!");
VoicePlay("G020820300_08_000");
MsgDisp("Shirahane","You're right......!");
ChEye(8,3);
ChMouth(8,4);
ChEyeOpenLevel(8,10);
ChMouthOpenLevel(8,0);
ChCheek(8,6);
VoicePlay("G020820300_08_010");
MsgDisp("Shirahane","…………");
MsgDisp("主人公","......｛大地＊＊｝?
What's wrong?");
ChEye(8,4);
ChMouth(8,4);
ChMotion(8,2,1);
VoicePlay("G020820300_08_020");
MsgDisp("Shirahane","Do you know the rumours surrounding this
place?");
VoicePlay("G020820300_08_030");
MsgDisp("Shirahane","When you're here with someone, 
you can sometimes hear their inner voice.");
ChEye(8,4);
ChMouth(8,4);
ChMouthOpenLevel(8,0);
VoicePlay("G020820300_08_040");
MsgDisp("Shirahane","…………");
ChEye(8,2);
ChMouth(8,0);
ChMotion(8,0,1);
VoicePlay("G020820300_08_050");
MsgDisp("Shirahane","No way......
Right now, can you hear my inner voice?");
MsgDisp("主人公","Hehe, what do you think?");
ChEye(8,4);
ChMouth(8,5);
ChMotion(8,5,1);
ChEyeOpenLevel(8,10);
ChMouthOpenLevel(8,10);
ChCheek(8,10);
VoicePlay("G020820300_08_060");
MsgDisp("Shirahane","∋");
ChEye(8,2);
ChMouth(8,4);
ChMotion(8,4,1);
ChEyeOpenLevel(8,0);
VoicePlay("G020820300_08_070");
MsgDisp("Shirahane","No, this is bad......this is bad!
I need to clear my mind......");
MsgDisp("主人公","(｛大地＊＊｝...... What on earth is he
thinking about right now?)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
BGMStop();
ChLayout(1);
ChEyeOpenLevel(8,#1);
ChMouthOpenLevel(8,#1);
ChCheek(8,0);
