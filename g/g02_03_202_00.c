ChLayout(1);
MsgClose();
ChOpen(3,255,1,0,0,-1,-1,0,0);
VoicePlay("G020320200_03_000");
MsgDisp("Honda","Hey, do you like Ryo-kun?");
MsgDisp("主人公","Eh∋
W-Why do you want to hear that?");
ChEye(3,0);
ChMouth(3,4);
ChMotion(3,0);
VoicePlay("G020320200_03_010");
MsgDisp("Honda","I thought it would be quicker to ask
directly.");
ChEye(3,4);
ChMouth(3,4);
VoicePlay("G020320200_03_020");
MsgDisp("Honda","Lately, it seems like you're together with
Ryo-kun a lot.");
ChEye(3,3);
ChMouth(3,0);
ChMotion(3,1);
ChEyeOpenLevel(3,0);
VoicePlay("G020320200_03_030");
MsgDisp("Honda","Yeah! When you're with Ryo-kun, your laugh
sounds a little different.");
MsgDisp("主人公","Is that, right......?");
ChEye(3,0);
ChMouth(3,3);
ChMotion(3,4);
ChEyeOpenLevel(3,0);
VoicePlay("G020320200_03_040");
MsgDisp("Honda","Yeah.
It like, really sparkles.");
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,1);
VoicePlay("G020320200_03_050");
MsgDisp("Honda","I mean, Ryo-kun is cool, mature, and he
understands you well");
VoicePlay("G020320200_03_060");
MsgDisp("Honda","You two look great together, but——");
ChEye(3,1);
ChMouth(3,1);
ChMotion(3,5);
VoicePlay("G020320200_03_070");
MsgDisp("Honda","No, that's not it! I mean, it is, but!");
MsgDisp("主人公","｛本多＊＊｝……？");
ChEye(3,2);
ChMouth(3,0);
ChMotion(3,1);
VoicePlay("G020320200_03_080");
MsgDisp("Honda","I like both you and Ryo-kun a lot, but I
can't support you two being together.");
MsgDisp("主人公","Eh?");
ChEye(3,2);
ChMouth(3,4);
ChMotion(3,4);
VoicePlay("G020320200_03_090");
MsgDisp("Honda","I don't really get it, but my chest feels
tight and achy. It eventually starts to
throb.");
ChEye(3,4);
ChMouth(3,4);
ChMotion(3,0);
VoicePlay("G020320200_03_100");
MsgDisp("Honda","So I'm not so sure I can act the same way
in front of you two. That's what I wanted
to tell you today.");
MsgDisp("主人公","Yeah......I see.");
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,1);
VoicePlay("G020320200_03_110");
MsgDisp("Honda","Yeah. But I really do like both of you a
lot, okay?");
ChEye(3,4);
ChMouth(3,4);
ChMotion(3,2);
VoicePlay("G020320200_03_120");
MsgDisp("Honda","It's just that, right now, seeing you two
together hurts a little......");
MsgDisp("主人公","(Hehe, I'm glad I heard ｛本多＊＊｝'s
true feelings.)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChLayout(1);
ChEyeOpenLevel(3,-1);
ChMouthOpenLevel(3,-1);
ChCheek(3,0);
