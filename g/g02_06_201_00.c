MsgClose();
ChEye(6,1);
ChMouth(6,1);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("G020620100_06_000");
MsgDisp("Himuro","...Are you bored?");
MsgDisp("主人公","What do you mean?");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,1);
VoicePlay("G020620100_06_010");
MsgDisp("Himuro","Lately, I feel a distance between us.");
MsgDisp("主人公","Eh.");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,4);
VoicePlay("G020620100_06_020");
MsgDisp("Himuro","It's not just me, you're close with
someone else, aren't you?");
MsgDisp("主人公","...Could it be—");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("G020620100_06_030");
MsgDisp("Himuro","It's okay don't say it.");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("G020620100_06_040");
MsgDisp("Himuro","I don't want to hear that person's name
come out of your mouth.");
ChEye(6,0);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("G020620100_06_050");
MsgDisp("Himuro","But, it's clear who comes to mind for you.");
MsgDisp("主人公","｛Himuro＊＊｝...");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020620100_06_060");
MsgDisp("Himuro","...Okay? Keep this in mind.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020620100_06_070");
MsgDisp("Himuro","This is a declaration of war against the
guy next to you right now and a statement
of resolve toward myself.");
MsgDisp("主人公","Y-Yes.");
ChEye(6,1);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("G020620100_06_080");
MsgDisp("Himuro","For now, I admit defeat, but I have no
intention of giving up that place to him
forever.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("G020620100_06_090");
MsgDisp("Himuro","I will definitely reclaim it, both the
place next to you and your feelings.");
ChEye(6,0);
ChMouth(6,3);
ChMotion(6,0);
VoicePlay("G020620100_06_100");
MsgDisp("Himuro","So be prepared.
...this is my message to you now.");
MsgDisp("主人公","Ehhh∋");
MsgDisp("主人公","(｛Himuro＊＊｝...?)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChLayout(1);
ChEyeOpenLevel(6,#1);
ChMouthOpenLevel(6,#1);
ChCheek(6,0);
