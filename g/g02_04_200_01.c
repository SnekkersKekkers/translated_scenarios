MsgClose();
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0);
VoicePlay("G020420001_04_000");
MsgDisp("Nanatsumori","There's a shy atmosphere in groups right
now when you feel like a lone wolf.");
MsgDisp("主人公","?");
ChEye(4,4);
ChMouth(4,0);
ChMotion(4,1,1);
VoicePlay("G020420001_04_010");
MsgDisp("Nanatsumori","Hey.
You may not realise it, but you know
you're the heroine, right?");
MsgDisp("主人公","Huh, I'm the heroine∋");
ChEye(4,2);
ChMouth(4,3);
VoicePlay("G020420001_04_020");
MsgDisp("Nanatsumori","Yeah.");
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
VoicePlay("G020420001_04_030");
MsgDisp("Nanatsumori","But I'm no hero or prince.
Maybe just a knight.");
MsgDisp("主人公","A knight...");
ChEye(4,2);
ChMouth(4,4);
ChMotion(4,1,1);
VoicePlay("G020420001_04_040");
MsgDisp("Nanatsumori","...That's why I was thinking.");
MsgDisp("主人公","??");
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
VoiceEVSPlay(4);
VoicePlay("G020420001_04_050");
MsgDisp("Nanatsumori","...｛主人公｝.
Among the knights, doesn't one of them get
special treatment?");
MsgDisp("主人公","Huh...");
ChEye(4,2);
ChMouth(4,4);
ChMotion(4,1,1);
VoicePlay("G020420001_04_060");
MsgDisp("Nanatsumori","Don't worry about it.
It's just I realised something that I
didn't need to know.");
ChEye(4,2);
ChMouth(4,3);
ChMotion(4,2,1);
VoicePlay("G020420001_04_070");
MsgDisp("Nanatsumori","Haha... Is this what jealousy feels like?");
MsgDisp("主人公","(｛Nanatsumori＊｝...)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChLayout(1);
ChEyeOpenLevel(4,#1);
ChMouthOpenLevel(4,#1);
ChCheek(4,0);
