MsgClose();
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,4);
VoicePlay("G020210100_02_000");
MsgDisp("Sassa","The fireflies here are really unique. Look
at their colors and how they light up.");
MsgDisp("主人公","Yeah.");
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,2);
VoicePlay("G020210100_02_010");
MsgDisp("Sassa","We're all pretty different too......");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0);
VoicePlay("G020210100_02_020");
MsgDisp("Sassa","I mean, Ryota, Inori, and me.
We all went out together before.");
MsgDisp("主人公","Yeah, that's right.");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,4);
VoicePlay("G020210100_02_030");
MsgDisp("Sassa","Ryota can do anything,
It's like he's superman......");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,4);
VoicePlay("G020210100_02_040");
MsgDisp("Sassa","And Inori......
I can't even figure him out at all. 
He's a funny guy.");
MsgDisp("主人公","Hehe.");
ChEye(2,0);
ChMouth(2,0);
ChMotion(2,0);
VoicePlay("G020210100_02_050");
MsgDisp("Sassa","Compared to them, 
I'm just the serious one, right?");
MsgDisp("主人公","｛颯砂＊＊｝ is also a funny superman.");
ChEye(2,0);
ChMouth(2,2);
ChMotion(2,4);
ChEyeOpenLevel(2,7);
VoicePlay("G020210100_02_060");
MsgDisp("Sassa","Are you complimenting me?");
MsgDisp("主人公","Yeah.");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,0);
VoicePlay("G020210100_02_070");
MsgDisp("Sassa","Well, that's fine. But I just think of
myself as a super serious, straight-laced
guy.");
ChEye(2,0);
ChMouth(2,4);
ChMotion(2,3);
VoicePlay("G020210100_02_080");
MsgDisp("Sassa","So I need to keep improving myself
, so I don't fall behind those two. 
And so you don't get bored of me!");
MsgDisp("主人公","I don't think that'll happen though......");
ChEye(2,2);
ChMouth(2,4);
ChMotion(2,4);
VoicePlay("G020210100_02_090");
MsgDisp("Sassa","Ah, sorry, sorry.");
MsgDisp("主人公","(｛颯砂＊＊｝ also has his own individual
charm though)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
ChLayout(1);
ChEyeOpenLevel(2,-1);
ChMouthOpenLevel(2,-1);
ChCheek(2,0);
