BGOpen("sc211",0);
ScrFadeIn(0);
MsgDisp("主人公","(It's time for the entrance ceremony...)");
MsgDisp("主人公","(...Huh? This boy right here...)");
BGMPlay("BGM_C06_INORI_I",0.01);
MsgClose();
ChOpen(6,254,0,1,1,0,#1,0,0);
VoicePlay("A020601100_06_000");
MsgDisp("Himuro?","......");
MsgDisp("主人公","(A new student?
Even so, his vibe seems familiar.
Reminds me of someone...)");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
VoicePlay("A020601100_06_010");
MsgDisp("Himuro?","What do you want?");
MsgDisp("主人公","Eh?");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("A020601100_06_020");
MsgDisp("Himuro?","If you've got something to ask, just
do it.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("A020601100_06_030");
MsgDisp("Himuro?","Or should I answer first?
I know exactly what Habataki High students
want to ask me.");
MsgDisp("主人公","? Err...");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
VoicePlay("A020601100_06_040");
MsgDisp("Himuro","Name's Himuro Inori.
I'm about to be a first year student.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("A020601100_06_050");
MsgDisp("Himuro","And as you may have guessed, a relative of
Reiichi-sa... of Vice Principal Himuro.");
MsgDisp("主人公","Oh... I see!");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("A020601100_06_060");
MsgDisp("Himuro","You didn't know, did you?
I shouldn't have told you...");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("A020601100_06_070");
MsgDisp("Himuro","And who might you be?");
MsgDisp("主人公","Ah, sorry, I'm ｛主人公姓名｝.
I'm in second year.
Nice to meet you.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,4);
VoicePlay("A020601100_06_080");
MsgDisp("Himuro","You're a year above me?
Sorry for being so rude.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("A020601100_06_090");
MsgDisp("Himuro","But don't bring up the Himuro thing too
much please.
...'cause I'll get angry.");
MsgDisp("主人公","A-Alright.");
ChEye(6,0);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("A020601100_06_100");
MsgDisp("Himuro","Alright.
That's all.");
SEPlay("EV_SE_FOOT_WALK_AWAY_ALONE");
BGMStop();
ChClose(6);
MsgDisp("主人公","(｛Himuro＊＊｝...
Seems like I should avoid bringing up his
relationship with the Vice Principal.)");
MsgClose();
ScrFadeOut(0);
