ChLayout(2);
MsgClose();
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,4);
ChEyeOpenLevel(3,0);
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0);
ChNanaType(0);
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,0);
ScrFadeIn(0);
VoicePlay("Q110B00000_03_000");
MsgDisp("Honda","Phew...
The castella was so yummy!");
MsgDisp("主人公","Yeah!");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("Q110B00000_01_000");
MsgDisp("Kazama","The texture of the coarse sugar was great.");
ChMotion(4,1,1);
VoicePlay("Q110B00000_04_000");
MsgDisp("Nanatsumori","Was I able to satisfy you all?");
ChMotion(1,0,1);
MsgDisp("主人公","Very satisfied!");
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,3,1);
VoicePlay("Q110B00000_03_010");
MsgDisp("Honda","Me too, me too!
When you eat something sweet——");
ChEye(1,0);
ChMouth(1,2);
ChEye(3,0);
ChMouth(3,3);
ChMotion(3,1,1);
VoicePlay("Q110B00000_03_020");
MsgDisp("Honda","Don't you also get hungry for some ramen?");
ChEye(4,2);
ChMouth(4,2);
ChEye(1,2);
ChMouth(1,2);
ChMotion(1,1);
ChEyeOpenLevel(1,0);
VoicePlay("Q110B00000_01_010");
MsgDisp("Kazama","Don't get us started again...");
ChEye(4,2);
ChMouth(4,4);
ChMotion(4,2,1);
ChMouthOpenLevel(4,5);
VoicePlay("Q110B00000_04_010");
MsgDisp("Nanatsumori","......");
MsgDisp("主人公","Hey, may I go to that craft shop over
there?");
ChEye(1,0);
Wait(60,1);
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("Q110B00000_01_020");
MsgDisp("Kazama","Yeah, looks nice.
Let's go together.");
MsgClose();
ScrFadeOut(0,0);
MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
ChEye(1,0);
ChMouth(1,0);
ChMotion(1,0);
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,0,1);
ScrFadeIn(0);
MsgDisp("主人公","There was a lot of stuff, even pottery and
tortoiseshell accessories. But they were
all too expensive for a high-schooler.");
ChEye(1,2);
ChMouth(1,3);
VoicePlay("Q110B00000_01_030");
MsgDisp("Kazama","Each piece is crafted with great care, so
the high price is understandable.");
MsgDisp("主人公","Yeah, that's true...
They were all so beautiful.");
ChEye(1,0);
ChEye(3,1);
ChMouth(3,3);
ChMotion(3,4,1);
VoicePlay("Q110B00000_03_030");
MsgDisp("Honda","Well, how about this?");
MsgDisp("主人公","...Ah, that's beautiful!
This is a beadlo, right?");
ChEye(4,0);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("Q110B00000_04_020");
MsgDisp("Nanatsumori","Yeah.
Our masterpiece.");
ChEye(1,4);
ChEyeOpenLevel(1,0);
MsgDisp("主人公","Eh?
By \"our\", you mean...");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0,1);
VoicePlay("Q110B00000_01_040");
MsgDisp("Kazama","While you were looking at souvenirs we've
been doing beadlo painting lessons.");
ChEye(3,0);
ChMouth(3,3);
ChMotion(3,1,1);
VoicePlay("Q110B00000_03_040");
MsgDisp("Honda","Yepyep.
We all painted that one.
Can you guess which is which?");
ChEye(4,3);
ChMouth(4,3);
ChMotion(4,0,1);
VoicePlay("Q110B00000_04_030");
MsgDisp("Nanatsumori","With our strong personalities, it ended up
being quite flashy.");
ChEye(1,4);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("Q110B00000_01_050");
MsgDisp("Kazama","Do you like it?");
MsgDisp("主人公","Yes...!
Thanks, everyone!");
MsgDisp("主人公","(I got gifted a wonderful memory!)");
BGMStop();
MsgClose();
ScrFadeOut(0,0);
MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
