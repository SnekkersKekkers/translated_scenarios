StlNear(0,0,15);
StlCheek(22,0);
StlEye(21,0);
StlMouth(21,0);
StlEye(22,0);
StlMouth(22,0);
VoicePlay("U140D00001_22_000");
MsgDisp("Hikaru","Come to think of it, Mari has the aura of an
intelligent lady lately♪");
MsgDisp("主人公","Really?
I wonder if it's because I've been studying
a lot.");
StlEyeOpenLevel(21,5,1);
VoicePlay("U140D00001_21_000");
MsgDisp("Michiru","Hehe. Aren't you a model student?");
StlEye(22,2);
StlMouth(22,0);
VoicePlay("U140D00001_22_010");
MsgDisp("Hikaru","Teach Hikaru how to study some day～!
Just summarise all the important parts♪");
StlMouth(21,1);
VoicePlay("U140D00001_21_010");
MsgDisp("Michiru","Hikaru, no. Follow Mari's example.");
StlEye(22,1);
StlMouth(22,1);
VoicePlay("U140D00001_22_020");
MsgDisp("Hikaru","Ehh...");
StlMouth(21,0);
StlEyeOpenLevel(22,5,1);
StlMouth(22,2);
MsgDisp("主人公","(I'm so happy they noticed how hard I've been
studying lately!)");
MsgClose();
