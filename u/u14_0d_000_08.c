StlNear(0,0,15);
StlCheek(22,0);
StlEye(21,0);
StlMouth(21,0);
StlEye(22,0);
StlMouth(22,1);
VoicePlay("U140D00008_22_000");
MsgDisp("Hikaru","That reminds me. Mari.
You've been working hard on your club
activities lately!");
MsgDisp("主人公","Hehe.
I feel like I've been enjoying my youth.");
StlEyeOpenLevel(21,5,1);
VoicePlay("U140D00008_21_000");
MsgDisp("Michiru","It's good to have something you're
devoted to. Don't overwork yourself and
give it your all.");
StlEyeOpenLevel(22,5,1);
StlMouth(22,0);
VoicePlay("U140D00008_22_010");
MsgDisp("Hikaru","Enjoying your youth, huh～.
Hikaru will try her best to do that too▼");
StlEye(21,3);
VoicePlay("U140D00008_21_010");
MsgDisp("Michiru","Hehe.
I'm cheering for both of you.");
MsgDisp("主人公","(I'm so happy they noticed how hard 
I've been working on my club activities!)");
MsgClose();
