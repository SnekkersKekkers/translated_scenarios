ChEyeOpenLevel(6,#1);
ChCheek(6,0);
BGMStop();
MsgDisp("主人公","I'm sorry...");
ChEye(6,2);
ChMouth(6,4);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("T020600000_06_530");
MsgDisp("Himuro","Ah...
I see I guess, that was just my own
selfish expectation.");
MsgDisp("主人公","｛氷室＊＊｝...");
BGMPlay("BGM_C06_INORI_B",0.01);
Wait(40,0);
VoicePlay("T020600000_06_540");
ChEye(6,2);
ChMouth(6,4);
ChMotion(6,0);
MsgDisp("Himuro","Why do you look so sad after rejecting
someone else?");
MsgDisp("主人公","Even so...");
VoicePlay("T020600000_06_550");
ChEye(6,2);
ChMouth(6,4);
ChMotion(6,0);
MsgDisp("Himuro","It's okay.
At least finish it nicely.");
MsgDisp("主人公","......");
VoicePlay("T020600000_06_560");
ChEye(6,2);
ChMouth(6,3);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
MsgDisp("Himuro","Truly a good person until the very end.");
VoicePlay("T020600000_06_570");
ChEye(6,2);
ChMouth(6,4);
ChMotion(6,2);
ChEyeOpenLevel(6,8);
MsgDisp("Himuro","That's why...
I fell in love with you.");
VoicePlay("T020600000_06_580");
ChEye(6,4);
ChMouth(6,4);
ChMotion(6,0);
MsgDisp("Himuro","I will never forget the memories I have
with you from our school life.");
VoicePlay("T020600000_06_590");
ChEye(6,2);
ChMouth(6,3);
ChMotion(6,2);
MsgDisp("Himuro","...No, there is no way I could forget it.");
VoicePlay("T020600000_06_600");
ChEye(6,2);
ChMouth(6,1);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
MsgDisp("Himuro","Because I love you so much...");
VoicePlay("T020600000_06_610");
ChEye(6,2);
ChMouth(6,3);
ChMotion(6,0);
MsgDisp("Himuro","Thank you for everything.
Goodbye...");
BGMStop(3.6);
MsgClose();
ScrFadeOut(0,1,120);
ChClose(6,0,0);
Wait(120);
