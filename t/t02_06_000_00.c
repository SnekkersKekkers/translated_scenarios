MsgType(1);
BGOpen("ex980",0);
ChLayout(0);
ScrFadeIn(0);
VoiceEVSPlay(6);
VoicePlay("T020600000_06_000");
MsgDisp("Himuro?","｛主人公｝.");
MsgClear();
Wait(60);
SEPlay("EV_SE_FOOT_WALK_COME_ALONE");
SEWait();
BGMPlay("BGM_C06_INORI_KOKU",0.01);
ChOpen(6,254,2,4,0,8,-1,0,0);
VoicePlay("T020600000_06_010");
MsgDisp("Himuro","Thank goodness.");
MsgDisp("主人公","｛氷室＊＊｝...");
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("T020600000_06_020");
MsgDisp("Himuro","To be honest, until I saw your face...
I had no confidence at all.");
ChEye(6,2);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("T020600000_06_030");
MsgDisp("Himuro","You're here now of course,
but there's the matter of our 
relationship so far.");
ChEye(6,2);
ChMouth(6,0);
ChMotion(6,2);
VoicePlay("T020600000_06_040");
MsgDisp("Himuro","I'll truly get what I deserve.
I've been treating you horribly
all this time...");
MsgDisp("主人公","That's not...");
ChEye(6,2);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
Wait(60,0);
VoicePlay("T020600000_06_050");
MsgDisp("Himuro","Back then, I felt trapped by my own
surname.");
ChEye(6,4);
ChMouth(6,0);
ChMotion(6,0);
ChEyeOpenLevel(6,8);
VoicePlay("T020600000_06_060");
MsgDisp("Himuro","The expectations tied to the Himuro name
were too high, and I couldn't bear them
all on my own...");
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("T020600000_06_070");
MsgDisp("Himuro","I just kept rejecting everything and
everyone around me.");
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,8);
VoicePlay("T020600000_06_080");
MsgDisp("Himuro","It was stupid of me, wasn't it...
If I'd stayed that way, I'd surely be
alone right now.");
ChEye(6,4);
ChMouth(6,4);
ChMotion(6,0);
ChEyeOpenLevel(6,8);
Wait(60,0);
VoicePlay("T020600000_06_090");
MsgDisp("Himuro","...But hey.
You're right here, in front of me.");
MsgDisp("主人公","Oh...");
ChEye(6,4);
ChMouth(6,4);
ChMotion(6,0);
ChEyeOpenLevel(6,9);
VoicePlay("T020600000_06_100");
MsgDisp("Himuro","You didn't expect anything of me,
and didn't force anything onto me.
You just always stayed by my side.");
ChEye(6,4);
ChMouth(6,4);
ChMotion(6,2);
ChEyeOpenLevel(6,0);
VoicePlay("T020600000_06_110");
MsgDisp("Himuro","It all seemed so nonchalant at first,
it even annoyed me sometimes.");
ChEye(6,4);
ChMouth(6,4);
ChMotion(6,2);
ChEyeOpenLevel(6,8);
VoicePlay("T020600000_06_120");
MsgDisp("Himuro","But before I knew it...
All I could see was you.");
ChEye(6,4);
ChMouth(6,4);
ChMotion(6,3,1);
ChEyeOpenLevel(6,9);
VoicePlay("T020600000_06_130");
MsgDisp("Himuro","I felt like that last summer.
Do you remember when I invited you
out to the sea in the morning?");
MsgDisp("主人公","Yes.");
ChEye(6,4);
ChMouth(6,4);
ChMotion(6,0);
VoicePlay("T020600000_06_140");
MsgDisp("Himuro","In that moment too, I wanted to see you
happy, and I wanted you all for myself.");
ChEye(6,4);
ChMouth(6,4);
ChMotion(6,2);
ChEyeOpenLevel(6,8);
VoicePlay("T020600000_06_150");
MsgDisp("Himuro","That's when I finally got it.
How much of a blessing it is to be able
to spend time together——");
MsgClear();
ChClose(6);
ChCustomLayout(6,5,0,-2.5);
Wait(60);
SEPlay("EV_SE_544");
ChOpen(6,254,0,4,0,9,-1,0,0,0,120);
ChMotion(6,7);
VoicePlay("T020600000_06_160");
MsgDisp("Himuro","And that's when I...
I knew I liked you.");
MsgDisp("主人公","!");
ChEye(6,4);
ChMouth(6,0);
ChMotion(6,2);
ChEyeOpenLevel(6,9);
VoicePlay("T020600000_06_170");
MsgDisp("Himuro","It was a feeling that I never could
have imagined two years ago.
You gave me this feeling.");
ChEye(6,4);
ChMouth(6,0);
ChMotion(6,0);
VoiceEVSPlay(6);
VoicePlay("T020600000_06_180");
MsgDisp("Himuro","｛主人公｝...
From now on, I want us to be together.
I want you by my side.");
ChEye(6,4);
ChMouth(6,0);
ChMotion(6,0);
VoicePlay("T020600000_06_190");
MsgDisp("Himuro","Please... be my girlfriend.");
