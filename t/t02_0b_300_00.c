BGOpen("sc211",0);
ChLayout(1);
ScrFadeIn(0);
MsgDisp("主人公","(......｛風真＊＊｝,
missed the graduation ceremony......)");
SEPlay("EV_SE_FOOT_RUN_COME_ALONE");
SEWait();
VoiceEVSPlay(1);
VoicePlay("T020B30000_01_000");
MsgDisp("Kazama?","｛主人公｝.");
BGMPlay("BGM_HANYOU_D",0.01);
ChOpen(1,254,0,4,3,-1,-1,0,0);
MsgDisp("主人公","｛風真＊＊｝!
You're back!");
MsgDisp("主人公","But...the graduation ceremony is 
already over.");
ChClose(1,0,30);
ChLayout(0);
SEPlay("EV_SE_544");
ChOpen(1,254,0,2,3,8,-1,0,0,0,30);
VoicePlay("T020B21000_01_010");
MsgDisp("Kazama","Sorry. I did try to hurry though.");
MsgDisp("主人公","Yeah.
I'm glad we could graduate together......");
ChEyeOpenLevel(1,0);
Wait(8,0);
ChEye(1,4);
VoicePlay("T020B21000_01_020");
MsgDisp("Kazama","Ah, congratulations on graduating.");
MsgDisp("主人公","｛風真＊＊｝ You too.");
ChEye(1,4);
ChMouth(1,3);
ChMotion(1,0,1);
ChEyeOpenLevel(1,0);
VoicePlay("T020B21000_01_030");
MsgDisp("Kazama","I remember waiting for you on 
that hill on the day of
the entrance ceremony.");
ChEye(1,2);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("T020B21000_01_040");
MsgDisp("Kazama","We haven't changed
much since then, huh?");
ChEyeOpenLevel(1,0);
Wait(8,0);
ChEye(1,4);
ChMouth(1,2);
ChMotion(1,0);
ChMouthOpenLevel(1,0);
ChCheek(1,5);
VoicePlay("T020B21000_01_050");
MsgDisp("Kazama","…………");
SEPlay("EV_SE_FOOT_RUN_COME_ALONE");
SEWait();
ChEye(1,2);
ChMouth(1,3);
ChMotion(1,0,1);
ChEyeOpenLevel(1,0);
ChCheek(1,0);
VoicePlay("T020B21000_01_060");
MsgDisp("Kazama","This is......");
ChClose(1,0,30);
ChLayout(1);
ChOpen(1,254,0,2,3,0,-1,0,1,0,30);
SEPlay("EV_SE_544");
ChOpen(3,254,1,3,3,-1,-1,0,2);
VoicePlay("T020B21000_03_000");
MsgDisp("Honda","Welcome back Ryota-kun! I'm glad we were
able to graduate together.");
ChEye(1,4);
ChMouth(1,3);
ChMotion(1,0);
VoicePlay("T020B21000_01_070");
MsgDisp("Kazama","That's right. I'm back.");
MsgDisp("主人公","｛本多＊＊｝.
Congratualitions on your graduation!");
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,3,1);
ChEyeOpenLevel(3,0);
VoicePlay("T020B21000_03_010");
MsgDisp("Honda","Yes, congratulations!");
ChEye(1,0);
ChMouth(1,2);
ChEye(3,4);
ChMouth(3,4);
ChMotion(3,4,1);
VoicePlay("T020B21000_03_020");
MsgDisp("Honda","Huh? 
The only one left is Mii-kun.
I wonder where he went.");
MsgClose();
ScrFadeOut(0,0);
ChClose(1,0,0);
ChClose(3,0,0);
MsgDisp("","　　　　　　　　　　 ・
　　　　　　　　　　 ・
　　　　　　　　　　 ・");
MsgClose();
BGOpen("sc130",1);
ChLayout(1);
ChNanaType(0);
ChOpen(4,254,0,2,0,-1,-1,0,0);
ScrFadeIn(0);
VoicePlay("T020B21000_04_000");
MsgDisp("Nanatsumori","Haa... you're finally here.");
ChClose(4,0,30);
ChLayout(2);
ChNanaType(0);
ChOpen(4,254,0,2,0,-1,-1,0,1,0,30);
SEPlay("EV_SE_FOOT_WALK_COME_TWO");
ChOpen(1,254,0,0,3,-1,-1,0,0,0,30);
ChOpen(3,254,0,0,0,-1,-1,0,2,0,30);
VoicePlay("T020B21000_01_080");
MsgDisp("Kazama","I'm late, but I made it just in time,
right?");
ChEye(4,2);
ChMouth(4,3);
ChMotion(4,1,1);
VoicePlay("T020B21000_04_010");
MsgDisp("Nanatsumori","Well, I guess that's typical Kazama 
until the end.");
ChEye(3,0);
ChMouth(3,4);
ChMotion(3,4,1);
VoicePlay("T020B21000_03_030");
MsgDisp("Honda","Mi-kun, you've been waiting here all this
time?");
ChMouth(1,2);
ChEye(4,0);
ChMouth(4,0);
ChMotion(4,0,1);
VoicePlay("T020B21000_04_020");  
MsgDisp("Nanatsumori","Ah, well if I wait here it can't go wrong
right? the worst thing would be to pass
each other.");
ChEye(4,0);
ChMouth(4,3);
ChEye(3,0);
ChMouth(3,0);
ChMotion(3,0,1);
VoicePlay("T020B21000_04_030");
MsgDisp("Nanatsumori","Today is the last day.");
MsgDisp("主人公","Right...today is the last day");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0,1);
ChEyeOpenLevel(1,0);
VoicePlay("T020B21000_01_090");
MsgDisp("Kazama","Yes, it's Nanatsumori's fault.");
ChEye(4,2);
ChMouth(4,4);
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,1);
VoicePlay("T020B21000_01_100");
MsgDisp("Kazama","Since you said that, it's making me
depressed.");
ChEye(3,0);
ChMouth(3,3);
ChMotion(3,2,1);
ChEyeOpenLevel(3,0);
VoicePlay("T020B21000_03_040");
MsgDisp("Honda","That's no good, Mii-kun.");
ChEye(4,2);
ChMouth(4,1);
ChMotion(4,2,1);
VoicePlay("T020B21000_04_040");
MsgDisp("Nanatsumori","Huh?
Wait a minute.
Did I do something wrong?");
ChEye(1,0);
ChMouth(1,3);
ChMotion(1,0);
ChEyeOpenLevel(1,0);
VoicePlay("T020B21000_01_110");
MsgDisp("Kazama","Well, did you?");
ChMouth(3,0);
ChMotion(3,1,1);
VoicePlay("T020B21000_03_050");
MsgDisp("Honda","Yes you did, Haha!");
ChEye(1,4);
ChEye(3,0);
MsgDisp("主人公","Hehe!");
ChEye(4,2);
ChMouth(4,4);
ChMotion(4,0,1);
VoicePlay("T020B21000_04_050");
MsgDisp("Nanatsumori","Even you... I was just waiting here for
everyone to arrive......");
ChEye(1,0);
ChMouth(1,0);
ChEye(4,2);
ChMouth(4,3);
VoicePlay("T020B21000_04_060");
MsgDisp("Nanatsumori","I mean, what were you doing?
How long were you going to make me wait?");
ChMouth(1,2);
ChSet(3,3);
VoicePlay("T020B21000_03_060");
MsgDisp("Honda","Ah, that's right.
Ryou-kun was caught up in the emotional
reunion and secretly he-——");
ChEye(1,0);
ChMouth(1,2);
ChMotion(1,3);
ChEyeOpenLevel(1,8);
ChCheek(1,5);
VoicePlay("T020B21000_01_120");
MsgDisp("Kazama","......∈
W-What is it?");
ChEye(4,3);
ChMouth(4,3);
ChMotion(4,2,1);
VoicePlay("T020B21000_04_070");
MsgDisp("Nanatsumori","Oh? Can you tell me that
story slowly?");
ChMouth(1,1);
ChMotion(1,0);
ChCheek(1,7);
VoicePlay("T020B21000_01_130");
MsgDisp("Kazama","There's no such thing. 
...Honda, were you watching?∋");
ChEye(3,3);
ChMouth(3,3);
ChMotion(3,1,1);
ChEyeOpenLevel(3,0);
VoicePlay("T020B21000_03_070");
MsgDisp("Honda","Yes yes, I heard you had arrived. 
Now, let's look back on the last
three years together");
ChEye(1,2);
ChMouth(1,2);
ChEyeOpenLevel(1,0);
MsgDisp("主人公","Hehe, yes!");
BGMStop(5);
MsgClose();
ScrFadeOut(0,0,120);
ChClose(1,0,0);
ChClose(3,0,0);
ChClose(4,0,0);
Wait(300,0);
