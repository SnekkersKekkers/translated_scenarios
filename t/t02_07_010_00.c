MsgType(1);
BGOpen("ex980",0);
ScrFadeIn(0);
VoiceEVSPlay(7);
VoicePlay("T020701000_07_000");
MsgDisp("Mikage?","｛主人公｝.");
MsgClear();
Wait(60);
SEPlay("EV_SE_FOOT_WALK_COME_ALONE");
SEWait();
BGMPlay("BGM_C07_MIKAGE_KOKU",0.01);
ChLayout(0);
ChOpen(7,254,0,0,0,#1,#1,0,0);
MsgDisp("主人公","｛Mikage＊＊｝...
Why are you here?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,4);
VoicePlay("T020701000_07_010");
MsgDisp("Mikage","I wonder why.
I had a feeling you were here.");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("T020701000_07_020");
MsgDisp("Mikage","Will you go with me?
It's my graduation ceremony.");
MsgDisp("主人公","｛Mikage＊＊｝'s graduation ceremony?
Are you going to quit your job as a teacher
at Haba High?");
ChEye(7,2);
ChMouth(7,4);
ChMotion(7,2);
ChEyeOpenLevel(7,0);
VoicePlay("T020701000_07_030");
MsgDisp("Mikage","Originally I wasn't meant to be a teacher.");
MsgDisp("主人公","Eh...?");
ChEye(7,2);
ChMouth(7,4);
ChMotion(7,0);
ChEyeOpenLevel(7,8);
VoicePlay("T020701000_07_040");
MsgDisp("Mikage","I came to Haba High to fill in the void left
from my high school days.");
ChEye(7,2);
ChMouth(7,4);
ChMotion(7,0);
ChEyeOpenLevel(7,0);
VoicePlay("T020701000_07_050");
MsgDisp("Mikage","I'm not the kind of person who's suited to be
called a teacher...");
MsgDisp("主人公","That...
I was very happy to spend the last three
years with ｛Mikage＊＊｝.");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,0,1);
ChEyeOpenLevel(7,8);
VoicePlay("T020701000_07_060");
MsgDisp("Mikage","...Thank you.
You always help me out like that.");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("T020701000_07_070");
MsgDisp("Mikage","Thanks to you, I no longer have an excuse to
run away from the family business.");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,2);
ChEyeOpenLevel(7,0);
VoicePlay("T020701000_07_080");
MsgDisp("Mikage","The gap from my high school days...
There was always a huge void there for me
you know?");
MsgDisp("主人公","｛Mikage＊＊｝...");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("T020701000_07_090");
MsgDisp("Mikage","Hey, do you remember when we went bowling?");
MsgDisp("主人公","Yes.
You took me there when our extracurricular
lesson was cancelled.");
ChEye(7,0);
ChMouth(7,4);
ChMotion(7,1);
VoicePlay("T020701000_07_100");
MsgDisp("Mikage","That's right.
When I think back to that time, the moment
the bowling pins flipped—");
ChEye(7,4);
ChMouth(7,4);
ChMotion(7,0);
ChEyeOpenLevel(7,0);
VoicePlay("T020701000_07_110");
MsgDisp("Mikage","the void in my heart might have been filled.
It was that much fun....");
MsgDisp("主人公","I also had a lot of fun.");
ChEye(7,4);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("T020701000_07_120");
MsgDisp("Mikage","I see... I'm glad.");
ChEye(7,4);
ChMouth(7,4);
ChMotion(7,0);
VoicePlay("T020701000_07_130");
MsgDisp("Mikage","But then the blanks were filled and there was
no space left—");
ChEye(7,4);
ChMouth(7,4);
ChMotion(7,0);
ChEyeOpenLevel(7,0);
VoicePlay("T020701000_07_140");
MsgDisp("Mikage","My feelings for you are getting stronger and
stronger.");
MsgDisp("主人公","Feelings for me...?");
MsgClear();
ChClose(7);
ChCustomLayout(7,6,0,-2.8);
Wait(60);
SEPlay("EV_SE_544");
ChEye(7,4);
ChMouth(7,4);
ChMotion(7,7);
ChEyeOpenLevel(7,8);
ChOpen(7,254,0,0,0,#1,#1,0,0,0,120);
VoicePlay("T020701000_07_150");
MsgDisp("Mikage","Yeah, what I feel for you...is love.");
MsgDisp("主人公","!");
ChEye(7,4);
ChMouth(7,4);
ChMotion(7,2);
VoicePlay("T020701000_07_160");
MsgDisp("Mikage","\"Like\" quickly grew into \"Love\" and it filled
that void within me.
There was no space left.");
ChEye(7,4);
ChMouth(7,4);
ChMotion(7,0);
VoiceEVSPlay(7);
VoicePlay("T020701000_07_170");
MsgDisp("Mikage","｛主人公｝, I love you.");
