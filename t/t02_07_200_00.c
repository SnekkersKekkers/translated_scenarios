MsgType(1);
BGOpen("ex980",0);
ChLayout(0);
ScrFadeIn(0);
VoiceEVSPlay(7);
VoicePlay("T020720000_07_000");
MsgDisp("Mikage?","｛主人公｝.");
MsgClear();
Wait(60);
SEPlay("EV_SE_FOOT_WALK_COME_ALONE");
SEWait();
BGMPlay("BGM_C07_MIKAGE_KOKU",0.01);
ChOpen(7,254,0,0,0,-1,-1,0,0);
MsgDisp("主人公","｛御影＊＊｝......
Why are you here?");
ChEye(7,0);
ChMouth(7,0);
ChMotion(7,4);
VoicePlay("T020720000_07_010");
MsgDisp("Mikage","It was them. Yanosuke and Inori gave me
the push I needed.");
MsgDisp("主人公","｛柊＊＊＊｝ and ｛氷室＊＊｝?");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,0);
VoicePlay("T020720000_07_020");
MsgDisp("Mikage","Yeah, they told me to graduate.
In the end I'm the one being looked after.");
ChEye(7,2);
ChMouth(7,3);
ChMotion(7,2);
VoicePlay("T020720000_07_030");
MsgDisp("Mikage","How pathetic.");
MsgDisp("主人公","No, that's not true.");
ChEye(7,2);
ChMouth(7,1);
ChMotion(7,5);
VoicePlay("T020720000_07_040");
MsgDisp("Mikage","That's not true. 
I'm not a teacher to begin with.");
ChEye(7,2);
ChMouth(7,4);
ChMotion(7,0);
ChEyeOpenLevel(7,8);
VoicePlay("T020720000_07_050");
MsgDisp("Mikage","I came to Haba School to fill in the void
left from my high school days");
ChEye(7,2);
ChMouth(7,4);
ChMotion(7,0);
ChEyeOpenLevel(7,0);
VoicePlay("T020720000_07_060");
MsgDisp("Mikage","I'm not the kind of person who's fit to be
called a teacher......");
MsgDisp("主人公","That......
I was really happy in the last three
years I spent with ｛御影＊＊｝.");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,0,1);
ChEyeOpenLevel(7,8);
VoicePlay("T020720000_07_070");
MsgDisp("Mikage","............Thank you.
You always help me out like that.");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,2);
ChEyeOpenLevel(7,0);
VoicePlay("T020720000_07_080");
MsgDisp("Mikage","Thanks to that, the void in my life was 
filled without me even realizing it.");
ChEye(7,4);
ChMouth(7,4);
ChMotion(7,0);
VoicePlay("T020720000_07_090");
MsgDisp("Mikage","......But then I realized I no longer had
any space to store my feelings for you.");
MsgDisp("主人公","Feelings......for me?");
MsgClear();
ChClose(7);
ChCustomLayout(7,6,0,-2.8);
Wait(60);
SEPlay("EV_SE_544");
ChOpen(7,254,7,4,4,8,-1,0,0,0,120);
VoicePlay("T020720000_07_100");
MsgDisp("Mikage","Yeah, I ......
love you.");
ChEye(7,4);
ChMouth(7,4);
ChMotion(7,2);
VoicePlay("T020720000_07_110");
MsgDisp("Mikage","My fondness for you has grown into love in
an instant. It's overflowing and I just
can't do anything about it.");
ChEye(7,2);
ChMouth(7,4);
ChMotion(7,2);
ChEyeOpenLevel(7,0);
VoicePlay("T020720000_07_120");
MsgDisp("Mikage","But I was still trying to stow it
away......");
MsgDisp("主人公","｛御影＊＊｝……");
ChEye(7,2);
ChMouth(7,0);
ChMotion(7,2);
VoicePlay("T020720000_07_130");
MsgDisp("Mikage","Those guys laughed at me.
They said that if I continued like this,
I'd never be able to graduate.");
ChEye(7,4);
ChMouth(7,4);
ChMotion(7,0);
ChEyeOpenLevel(7,0);
VoicePlay("T020720000_07_140");
MsgDisp("Mikage","It made me realize that if I leave my
feelings for you at Haba Scool, I can't
move forward.");
ChEye(7,4);
ChMouth(7,4);
ChMotion(7,0);
VoiceEVSPlay(7);
VoicePlay("T020720000_07_150");
MsgDisp("Mikage","｛主人公｝, I love you.");
